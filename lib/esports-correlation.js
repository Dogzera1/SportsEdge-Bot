...
function adjustStakesForCorrelation(tips) {
  ...
  const DISCOUNT_FACTOR = (() => {
    const e = parseFloat(process.env.ESPORTS_CORR_DISCOUNT_FACTOR);
    return Number.isFinite(e) && e > 0 && e <= 1 ? e : 0.5;
  })();
  // Quarter-Kelly safety net pra tips no mesmo match. Mesmo c/ correlação
  // baixa empírica (<0.3), múltiplas tips no mesmo evento amplificam variance
  // (rho real >0 por construção: outcome compartilhado do match). Lit Kelly:
  // 2 corr-1 tips → 0.5×Kelly cada; 3+ tips → ainda mais agressivo.
  // Floors: 1 tip = 0%, 2 = 25% (3/4 Kelly), 3+ = 50% (half Kelly).
  // Combina com pair-discount via Math.max — pair discount alto pode passar.
  const N = tips.length;

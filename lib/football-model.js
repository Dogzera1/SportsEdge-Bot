...
function calcExpectedGoals(homeAttack, homeDefense, awayAttack, awayDefense, leagueAvg, homeFactor = HOME_FACTOR) {
  ...
  // 2026-05-07 (audit P2): clamp [0.15, 5.0] era assimétrico — pra leagueAvg=1.5
  // (típico), floor 0.15 = 10x abaixo da média mas ceiling 5.0 = 3.3x acima.
  // Probability mass colapsa pro floor em mismatches, viesando favorito muito alto.
  // Fix: clamp simétrico em log-space (ratio 5x ambos lados) com hard floor/ceiling
  // de safety (0.1 / 6.0). Resulta [0.3, 6.0] pra leagueAvg=1.5 — calibrado com
  // dados reais (Man City peak xG/game ~3.5; weakest L2 teams ~0.5).
  const lo = Math.max(0.1, leagueAvg / 5);
  const hi = Math.min(6.0, leagueAvg * 5);
  const lambdaHome = Math.max(lo, Math.min(hi, homeAttack * awayDefense * leagueAvg * homeFactor));
  const lambdaAway = Math.max(lo, Math.min(hi, awayAttack * homeDefense * leagueAvg));

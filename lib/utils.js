...
function _applyKelly(p, odds, frac) {
  ...
  // 2026-05-01: HIGH_EV_THROTTLE — buckets EV>8% rodaram ROI -10% (overall),
  // -31.7% (lol 8-12), -32.1% (tennis 8-12), -31.8% (cs >12) em 30d.
  // Selection bias / linha stale / modelo overconfident em edge alto.
  // Corte multiplicativo no Kelly preserva tip (mantém amostra) mas reduz
  // exposure. Override: HIGH_EV_THROTTLE=false desliga; HIGH_EV_THROTTLE_MULT
  // ajusta corte (default 0.6); HIGH_EV_THROTTLE_THRESHOLD muda gatilho EV%.
  let evThrottle = 1;
  if (!/^(0|false|no)$/i.test(String(process.env.HIGH_EV_THROTTLE ?? ''))) {
  ...
  const kellyStake = kellyFull * frac * _kellyCal * evThrottle;

...
function calcKellyFraction(evStr, oddsStr, fraction) {
  ...
  // Kelly completo: f* = (p*odds - 1) / (odds - 1) = EV / (odds - 1)
  // Esta fórmula é matematicamente equivalente à forma canônica f* = (b*p - q) / b
  // onde b=odds-1, p=probabilidade de vitória, q=1-p
  const kellyStake = (ev / (odds - 1)) * frac;
  // Max 4u (ALTA ¼ Kelly), 3u (MÉDIA ⅙ Kelly), 1.5u (BAIXA 1/10 Kelly)
  const maxStake = frac >= 0.25 ? 4 : frac >= 0.15 ? 3 : 1.5;
  const minStake = frac >= 0.15 ? 0.5 : 0.5;
...
module.exports = { log, calcKelly, calcKellyFraction, norm, httpGet, httpsPost, safeParse, sendJson, fmtDate, fmtDateTime, fmtDuration, fuzzyName, oddsApiAllowed };
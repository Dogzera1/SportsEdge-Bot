...
function _applyKelly(p, odds, frac) {
  const kellyStake = ((p * (odds - 1) - (1 - p)) / (odds - 1)) * frac;
  // Max 4u (ALTA ¼ Kelly), 3u (MÉDIA ⅙ Kelly), 1.5u (BAIXA 1/10 Kelly)
  const maxStake = frac >= 0.25 ? 4 : frac >= 0.15 ? 3 : 1.5;
  const minStake = 0.5;
  const stake = Math.max(minStake, Math.min(maxStake, Math.round(kellyStake * 100 * 2) / 2));
  return `${stake}u`;
}
...
async function cachedHttpGet(targetUrl, opts = {}) {
  ...
  // TTL 0 -> sem cache, mas ainda dedupe in-flight
  ...
  if (_httpInFlight.has(cacheKey)) return _httpInFlight.get(cacheKey);
...
module.exports = { log, calcKelly, calcKellyFraction, calcKellyWithP, norm, httpGet, cachedHttpGet, httpsPost, fetchWithRetry, aiPost, safeParse, sendJson, fmtDate, fmtDateTime, fmtDuration, fuzzyName, oddsApiAllowed, getMetricsLite };
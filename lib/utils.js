...
// ── The Odds API monthly budget tracker (tennis only) ──
...
// ── OddsPapi monthly budget tracker (esports: LoL) ──
// Free tier: 250 req/mês. Com TTL 20min + tournament cache 24h ≈ 72 req/dia — cuidado com quota!
const ODDSPAPI_MONTHLY_BUDGET = 200; // 250 free - 50 buffer
let _oddspapiReqCount = 0;
let _oddspapiReqMonth = new Date().getMonth();

function oddspapiAllowed(tag) {
  const now = new Date();
  if (now.getMonth() !== _oddspapiReqMonth) { _oddspapiReqCount = 0; _oddspapiReqMonth = now.getMonth(); }
  if (_oddspapiReqCount >= ODDSPAPI_MONTHLY_BUDGET) {
    log('WARN', tag || 'ODDS', `OddsPapi: limite mensal atingido (${_oddspapiReqCount}/${ODDSPAPI_MONTHLY_BUDGET}). Esports sem odds.`);
    return false;
  }
  _oddspapiReqCount++;
  if (_oddspapiReqCount % 20 === 0 || _oddspapiReqCount >= ODDSPAPI_MONTHLY_BUDGET - 20) {
    log('INFO', tag || 'ODDS', `Quota OddsPapi: ${_oddspapiReqCount}/${ODDSPAPI_MONTHLY_BUDGET} no mês`);
  }
  return true;
}

module.exports = { log, calcKelly, norm, httpGet, httpsPost, safeParse, sendJson, fmtDate, fmtDateTime, fmtDuration, fuzzyName, oddsApiAllowed, oddspapiAllowed };
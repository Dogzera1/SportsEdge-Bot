...
function propagateMtResultToTips(db, shadowRow, result, profitUnits) {
  ...
  // BUG FIX 2026-04-27 #2: tip 559 (Zverev -3.5 @ 1.54) settled errado com row
  // shadow line -4.5 @ 2.29. Match_id não codifica line, propagator casava por
  // padrão (sport+market+side+pair) e qualquer shadow row da mesma combinação
  // sobrescrevia tip de line diferente. Fix: select TODAS candidatas + filtra
  // por proximidade de odd (linhas diferentes têm odds bem diferentes — line
  // -3.5 nunca tem odd parecida com line -4.5).
  const ODD_TOL_PCT = 0.07; // 7% tolerância (cobre CLV drift normal, rejeita line swap)
  const isOddMatch = (tipOdd) => {
    const a = Number(tipOdd) || 0;
    const b = Number(shadowRow.odd) || 0;
    if (a <= 1 || b <= 1) return true; // sem odds confiáveis, não bloqueia
    return Math.abs(a - b) / a <= ODD_TOL_PCT;
  };
  let tipRow = null;
  try {
    // Lookup primário: match_id sintético + pair match. Pega top 5 e escolhe
    // a com odds mais próximas (caso múltiplas linhas coexistam pra mesmo par).
    ...
    tipRow = candidates.find(c => isOddMatch(c.odds)) || null;
    if (candidates.length > 0 && !tipRow) {
      log('WARN', 'MT-PROP',
        ...
        `mas todas com odd != shadow.odd=${shadowRow.odd} (line mismatch?). Tip ids: ${candidates.map(c => `${c.id}@${c.odds}`).join(',')}`);
    }
  } catch (e) {

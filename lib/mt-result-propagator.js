...
// 2026-04-28: espelho do _MT_MARKET_TYPE_MAP em bot.js. Mantenha sincronizado!
// Markets fora deste map caem no fallback toUpperCase() — funciona pra
// camelCase consistente (doubleChance → DOUBLECHANCE) mas pode divergir se
// um lado normalize diferente. Adicionados explicitamente:
// doubleChance, correctScore, duration, mapWinner, firstBlood, total_kills_map<N>.
...
// 2026-05-06: prepared statement cache. Antes db.prepare era chamado em cada
// settle propagation × 3 lookups × dezenas de tips/ciclo = parsing custoso
// repetido. Cache amortiza prepare cost (~50-200μs SQL parse → 0).
let _stmts = null;
function _getStmts(db) {
  if (_stmts && _stmts._db === db) return _stmts;
  _stmts = {
    _db: db,
    lookupLineSuffix: db.prepare(`
...
function propagateMtResultToTips(db, shadowRow, result, profitUnits) {
  ...
  // 2026-05-10 BUG FIX: sport bucket mismatch entre market_tips_shadow e tips.
  // Convention (memory project_cs_cs2_convention):
  //   - market_tips_shadow.sport = 'cs2'  (game id)
  //   - tips.sport             = 'cs'   (bankroll bucket)
  // Propagator usava WHERE sport = shadowRow.sport (= 'cs2') → ZERO match em tips.
  // Resultado: shadow row settled mas tip real ficava pending pra sempre. Caso
  // real CS TOTAL: tips 2379 (GenOne vs Ursa) e 2383 (Bounty Hunters vs Pagode)
  // pending 12-14h enquanto shadow já tinha result=loss.
  //
  // Fix: mapeia shadowRow.sport pro bucket correspondente em tips. cs2→cs é o
  // unico caso documentado, mas dota/dota2/lol/esports legacy também podem
  // divergir. Map preferencial (game→bucket); fallback identidade.
  const _shadowSportToTipsBucket = {
    cs2: 'cs',     // game cs2 → bucket cs
    dota: 'dota2', // legacy dota → bucket dota2
  };
  const tipsSport = _shadowSportToTipsBucket[shadowRow.sport] || shadowRow.sport;
  ...
  let tipRow = null;
  let matchPath = '';
  const _stmtsCache = _getStmts(db);
  // Lookup #1 (preferido): line-suffix exato — mais robusto que odd heurística
  if (sideLinePattern) {
    try {
      const candidates = _stmtsCache.lookupLineSuffix.all(tipsSport, market_type, sideLinePattern, t1n, t2n, t2n, t1n);
  ...
  if (!tipRow) {
    try {
      const candidates = _stmtsCache.lookupPattern.all(tipsSport, market_type, sidePattern, t1n, t2n, t2n, t1n);
  ...
  if (!tipRow) {
    try {
      const candidates = _stmtsCache.lookupPair.all(tipsSport, market_type, t1n, t2n, t2n, t1n);
      tipRow = candidates.find(c => isTimeMatch(c.sent_at) && isOddMatch(c.odds)) || null;
  ...
  try {
    ...
    try {
      ...
      // 2026-05-06 FIX: result='void'/'push' antes virava -stakeR (loss).
      // Pinnacle voida em walkover, totals integer line push, totalGames push,
      // mapWinner not-played, bestOf mismatch — esses devolvem stake (profit=0).
      profitR = result === 'win'
        ? parseFloat((stakeR * (odds - 1)).toFixed(2))
        : (result === 'void' || result === 'push')
          ? 0
          : parseFloat((-stakeR).toFixed(2));
    } catch (eU) {
      // Fallback: usa unit value 1.0 se sport-unit falhar
      stakeR = stakeU;
      profitR = result === 'win' ? stakeU * (odds - 1)
              : (result === 'void' || result === 'push') ? 0
              : -stakeU;
    }

    // 2026-05-15 audit P0 architectural: idempotency guard pra prevent
    // double-credit em restore-voided → re-settle cycle. Pre-tx check evita
    // re-fire desnecessário se já creditado pelo propagator com mesmo result.
    ...
    const existingAudit = db.prepare(`
      ...
      LIMIT 1
    `).get(tipRow.id, result);
    if (existingAudit) {
      log('DEBUG', 'MT-PROP', `tip id=${tipRow.id} ${result} já creditado via propagator — idempotent skip`);
      return null;
    }

    db.transaction(() => {
      const upd = _stmtsCache.updateTipResult.run(result, stakeR, profitR, tipRow.id);
      // 2026-05-06: só toca bankroll se o UPDATE da tip realmente mudou row.
      // Antes: read+sum+write padrão deixava race com /settle paralelo (read
      // pegava valor stale, depois write sobrescrevia delta de /settle).
      // Agora: incremental delta direto via UPDATE +=, idempotente e tip-gated.
      if (upd.changes > 0) {
        // 2026-05-15 audit P0 architectural: audit trail no propagator path.
        // Era missing — só /settle gravava audit. Necessário pra idempotency
        // guard (B) + traceability cross-source.
        ...
        `).run(tipRow.id, tipRow.sport, result, profitR);
        _stmtsCache.bumpBankroll.run(profitR, tipRow.sport);
      }
    })();

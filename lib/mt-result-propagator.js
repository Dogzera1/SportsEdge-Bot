...
function propagateMtResultToTips(db, shadowRow, result, profitUnits) {
  ...
  if (sideLinePattern) {
    try {
      const candidates = _stmtsCache.lookupLineSuffix.all(shadowRow.sport, market_type, sideLinePattern, t1n, t2n, t2n, t1n);
  ...
  if (!tipRow) {
    try {
      const candidates = _stmtsCache.lookupPattern.all(shadowRow.sport, market_type, sidePattern, t1n, t2n, t2n, t1n);
  ...
  if (!tipRow) {
    try {
      const candidates = _stmtsCache.lookupPair.all(shadowRow.sport, market_type, t1n, t2n, t2n, t1n);
      tipRow = candidates.find(c => isTimeMatch(c.sent_at) && isOddMatch(c.odds)) || null;

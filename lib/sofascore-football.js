...
async function httpJson(url, { ttlMs = 5 * 60 * 1000, provider = 'sofascore' } = {}) {
  ...
  if (r.status !== 200) {
    ...
    // 2026-05-08: 404 adicionado ao filter — proxy Public-Sofascore-API
    // retorna 404 estruturado quando Sofascore upstream rejeita o IP do
    // Railway (mesmo com curl_cffi chrome110 impersonation). Sem o 404 aqui,
    // métrica sofascore_fail|path=proxy,status=404 ficava invisível.
    if (r.status === 403 || r.status === 404 || r.status === 0 || r.status === 502 || r.status === 503) {
      try {

...
// 2026-05-06: throttle global 200ms entre reqs sofascore — evita ngrok proxy
// 429/Cloudflare-Sofascore burst em listagens grandes. Memória cita "throttle
// WARN" foi adicionado mas só log; agora é gate real.
let _sofaLastReqMs = 0;
...
async function httpJson(url, { ttlMs = 5 * 60 * 1000, provider = 'sofascore' } = {}) {
  // Throttle (skip se cache hit dispensa req real — cachedHttpGet handle internamente)
  const _now = Date.now();
  const _wait = _SOFA_MIN_GAP_MS - (_now - _sofaLastReqMs);
  if (_wait > 0 && _sofaLastReqMs > 0) {
    await new Promise(r => setTimeout(r, _wait));
  }
  _sofaLastReqMs = Date.now();

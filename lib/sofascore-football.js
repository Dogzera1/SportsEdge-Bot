...
async function httpJson(url, { ttlMs = 5 * 60 * 1000, provider = 'sofascore' } = {}) {
  ...
  }).catch(e => { log('WARN', 'SOFASCORE', `httpJson err ${url}: ${e.message}`); return null; });
  log('INFO', 'SOFASCORE', `httpJson ${url} → status=${r?.status} bodyLen=${r?.body?.length || 0}`);
...
async function enrichMatch(team1, team2, commenceIso) {
  ...
  log('INFO', 'SOFASCORE', `enrichMatch start: ${team1} vs ${team2} | enabled=${enabled} commenceIso="${commenceIso}"`);
  ...
  log('INFO', 'SOFASCORE', `dates built: d0="${d0}" count=${dates.size} list=${[...dates].join(',')}`);
  ...
  for (const d of dates) {
    ...
    log('INFO', 'SOFASCORE', `schedule ${d}: ${events.length} eventos | jsonKeys=${Object.keys(json).join(',').slice(0, 100)}`);
    for (const ev of events) {
  ...
  log('INFO', 'SOFASCORE', `parse: homeForm=${homeFormData?.form?.length || 0} awayForm=${awayFormData?.form?.length || 0} h2h=${h2hData.results.length} homeJsonKeys=${homeJson ? Object.keys(homeJson).slice(0,3).join(',') : 'null'} homeEventsRaw=${homeJson?.events?.length || 0}`);

/**
 * Enriquecimento de futebol via API não oficial Sofascore (documentação:
 * https://github.com/pseudo-r/Public-Sofascore-API ).
 *
 * Env:
 * - SOFASCORE_PROXY_BASE — URL do prefixo `/api/v1/sofascore` do proxy Django (ex.: http://127.0.0.1:8000/api/v1/sofascore).
 *   Sem isso, Node costuma levar 403 em api.sofascore.com.
 * - SOFASCORE_DIRECT=true — tenta api.sofascore.com mesmo assim (só ajuda em hosts “confiáveis”).
 * - SOFASCORE_ENRICH_FOOTBALL — default: ligado se SOFASCORE_PROXY_BASE existir; senão desligado (use `true` para forçar).
 *
 * Rotas usadas no proxy: `/schedule/football/{date}/`, `/event/{id}/h2h`, `/team/{id}/events/last/0`.
 * O README oficial do proxy só lista schedule/event/team; pode ser preciso estender o Django para h2h/last.
 */
const { safeParse, cachedHttpGet, log, throttleHost, BROWSER_UA } = require('./utils');

const BROWSER_HEADERS = {
  Accept: 'application/json,text/plain,*/*',
  'User-Agent': BROWSER_UA,
  Origin: 'https://www.sofascore.com',
  Referer: 'https://www.sofascore.com/',
  'ngrok-skip-browser-warning': 'true'
};

function normTeam(s) {
  return String(s || '')
    .toLowerCase()
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .replace(/[^a-z0-9]/g, '');
}

function teamsMatchEvent(homeName, awayName, t1, t2) {
  const h = normTeam(homeName);
  const a = normTeam(awayName);
  const q1 = normTeam(t1);
  const q2 = normTeam(t2);
  if (!h || !a || !q1 || !q2) return false;
  const fwd = (h.includes(q1) || q1.includes(h)) && (a.includes(q2) || q2.includes(a));
  const rev = (h.includes(q2) || q2.includes(h)) && (a.includes(q1) || q1.includes(a));
  return fwd || rev;
}

function _scoreCurrent(ev, side) {
  const o = side === 'home' ? ev?.homeScore : ev?.awayScore;
  if (o == null) return null;
  if (typeof o === 'number') return o;
  if (o.current != null) return o.current;
  if (o.display != null) return parseInt(String(o.display), 10);
  if (o.normaltime?.current != null) return o.normaltime.current;
  return null;
}

function resultLetterForTeam(ev, teamId) {
  const hid = ev?.homeTeam?.id;
  const aid = ev?.awayTeam?.id;
  const hs = _scoreCurrent(ev, 'home');
  const as = _scoreCurrent(ev, 'away');
  if (hs == null || as == null || !teamId) return null;
  const isHome = hid === teamId;
  const isAway = aid === teamId;
  if (!isHome && !isAway) return null;
  if (hs > as) return isHome ? 'W' : 'L';
  if (as > hs) return isAway ? 'W' : 'L';
  return 'D';
}

function collectEventsFromSchedulePayload(data) {
  const out = [];
  const visit = (node) => {
    if (!node || typeof node !== 'object') return;
    if (Array.isArray(node)) {
      for (const x of node) visit(x);
      return;
    }
    if (node.homeTeam && node.awayTeam && node.id != null) {
      out.push(node);
      return;
    }
    for (const k of Object.keys(node)) visit(node[k]);
  };
  visit(data);
  return out;
}

function buildUrl(base, path) {
  const b = String(base || '').replace(/\/+$/, '');
  const p = path.startsWith('/') ? path : `/${path}`;
  return `${b}${p}`;
}

// 2026-05-14: throttle migrado pra global per-host (throttleHost em utils).
// Antes cada lib mantinha _lastReqMs separado — 5× sofascore-* paralelos
// burst-violavam 200ms gap contra mesmo proxy (Sofascore 403 burst documentado).
const _SOFA_MIN_GAP_MS = parseInt(process.env.SOFA_MIN_GAP_MS || '200', 10);
async function httpJson(url, { ttlMs = 5 * 60 * 1000, provider = 'sofascore' } = {}) {
  await throttleHost('sofascore', _SOFA_MIN_GAP_MS);
  const r = await cachedHttpGet(url, {
    ttlMs,
    provider,
    headers: BROWSER_HEADERS,
    cacheKey: `sofascore:${url}`
  }).catch(() => null);
  // 2026-05-12: r=null = transport error (timeout, DNS, ECONN). Antes silent —
  // sem isso falha de proxy Django (Railway service down/timeout) não aparecia
  // em log e diag de FOOTBALL-SYNC mostrava "no-data" sem causa identificável.
  if (!r) {
    try {
      const _hostname = new URL(url).hostname;
      const _isDirect = _hostname.includes('api.sofascore.com') || _hostname.includes('api.sofascore.app');
      const _path = _isDirect ? 'direct' : 'proxy';
      global._sofaErrLogged = global._sofaErrLogged || new Map();
      const key = `null:${_hostname}`;
      const last = global._sofaErrLogged.get(key) || 0;
      if (Date.now() - last > 60 * 60 * 1000) {
        log('WARN', 'SOFA-FB', `${url.slice(0, 80)} unreachable (transport error/timeout) path=${_path}`);
        try { require('./metrics').incr('sofascore_fail', { status: 'null', path: _path }); } catch (_) {}
        global._sofaErrLogged.set(key, Date.now());
        if (global._sofaErrLogged.size > 50) {
          const oldest = global._sofaErrLogged.keys().next().value;
          global._sofaErrLogged.delete(oldest);
        }
      }
    } catch (_) {}
    return null;
  }
  if (r.status !== 200) {
    // 2026-05-06: log throttled per-status pra surfacing — antes 403/404/timeout
    // todos viravam null indistinto. Cooldown 1h/host pra não poluir logs.
    // 2026-05-08: distingue path=proxy (Django curl_cffi) vs path=direct
    // (api.sofascore.{com,app}). Diagnóstico crítico — falha no proxy = bug
    // no serviço Django (precisa redeploy/log); falha em direct = esperado
    // quando proxy não retornou 200 e o código tentou fallback (CF block).
    // 2026-05-12: filter removido — QUALQUER non-200 agora loga (antes só 403/404/0/502/503).
    // 500/504/522/524 do proxy também precisam visibilidade. Throttle 1h/(status,host) protege volume.
    try {
      global._sofaErrLogged = global._sofaErrLogged || new Map();
      const _hostname = new URL(url).hostname;
      const _isDirect = _hostname.includes('api.sofascore.com') || _hostname.includes('api.sofascore.app');
      const _path = _isDirect ? 'direct' : 'proxy';
      const key = `${r.status}:${_hostname}`;
      const last = global._sofaErrLogged.get(key) || 0;
      if (Date.now() - last > 60 * 60 * 1000) {
        // Path-specific message: proxy 404 = upstream block; direct 403 = CF block; etc.
        // 2026-05-16: proxy 403 = curl_cffi impersonate detectado por Sofascore WAF —
        // bump SOFASCORE_IMPERSONATE env no Railway proxy service (chrome146 atual).
        const _hint = _path === 'proxy' && r.status === 404
          ? '(upstream blocked? proxy alive but Sofascore returns 404)'
          : _path === 'proxy' && r.status === 403
          ? '(upstream WAF blocked — bump SOFASCORE_IMPERSONATE no proxy Railway env)'
          : '(proxy/CF blocked?)';
        log('WARN', 'SOFA-FB', `${url.slice(0, 80)} status=${r.status} path=${_path} ${_hint}`);
        // Métrica pra dashboard /admin/p2-status surfacing
        try { require('./metrics').incr('sofascore_fail', { status: String(r.status), path: _path }); } catch (_) {}
        global._sofaErrLogged.set(key, Date.now());
        if (global._sofaErrLogged.size > 50) {
          const oldest = global._sofaErrLogged.keys().next().value;
          global._sofaErrLogged.delete(oldest);
        }
      }
    } catch (_) {}
    return null;
  }
  return safeParse(r.body, null);
}

function _directEnabled() {
  const v = String(process.env.SOFASCORE_DIRECT || '').toLowerCase();
  return v === '1' || v === 'true' || v === 'yes';
}

function scheduleUrlsForDate(isoDate) {
  const d = String(isoDate || '').slice(0, 10);
  if (!/^\d{4}-\d{2}-\d{2}$/.test(d)) return [];
  const proxy = (process.env.SOFASCORE_PROXY_BASE || '').trim().replace(/\/+$/, '');
  const direct = _directEnabled();
  const urls = [];
  if (proxy) {
    urls.push(buildUrl(proxy, `/schedule/football/${d}/`));
    urls.push(buildUrl(proxy, `/schedule/football/${d}`));
  }
  if (direct) {
    urls.push(`https://api.sofascore.com/api/v1/sport/football/scheduled-events/${d}`);
    urls.push(`https://api.sofascore.app/api/v1/sport/football/scheduled-events/${d}`);
  }
  return [...new Set(urls)];
}

function eventH2hUrls(eventId) {
  const id = parseInt(String(eventId), 10);
  if (!Number.isFinite(id)) return [];
  const proxy = (process.env.SOFASCORE_PROXY_BASE || '').trim().replace(/\/+$/, '');
  const direct = _directEnabled();
  const urls = [];
  if (proxy) {
    urls.push(buildUrl(proxy, `/event/${id}/h2h/`));
    urls.push(buildUrl(proxy, `/event/${id}/h2h`));
  }
  if (direct) {
    urls.push(`https://api.sofascore.com/api/v1/event/${id}/h2h`);
    urls.push(`https://api.sofascore.app/api/v1/event/${id}/h2h`);
  }
  return [...new Set(urls)];
}

function teamEventsUrls(teamId) {
  const id = parseInt(String(teamId), 10);
  if (!Number.isFinite(id)) return [];
  const proxy = (process.env.SOFASCORE_PROXY_BASE || '').trim().replace(/\/+$/, '');
  const direct = _directEnabled();
  const urls = [];
  if (proxy) {
    urls.push(buildUrl(proxy, `/team/${id}/events/last/0`));
  }
  if (direct) {
    urls.push(`https://api.sofascore.com/api/v1/team/${id}/events/last/0`);
    urls.push(`https://api.sofascore.app/api/v1/team/${id}/events/last/0`);
  }
  return [...new Set(urls)];
}

async function fetchFirstJson(urlList) {
  for (const u of urlList) {
    const j = await httpJson(u, { ttlMs: 3 * 60 * 1000 }).catch(() => null);
    if (j) return j;
  }
  return null;
}

function collectH2hEvents(data) {
  if (!data || typeof data !== 'object') return [];
  const out = [];
  const pushArr = a => { if (Array.isArray(a)) for (const x of a) out.push(x); };
  pushArr(data.events);
  pushArr(data?.h2h?.events);
  pushArr(data?.teamDuel?.events);
  pushArr(data?.duels?.[0]?.events);
  if (out.length) return out;
  return collectEventsFromSchedulePayload(data);
}

function parseH2hToResults(h2hJson) {
  const events = collectH2hEvents(h2hJson);
  const results = [];
  for (const ev of events) {
    const hs = _scoreCurrent(ev, 'home');
    const as = _scoreCurrent(ev, 'away');
    if (hs == null || as == null) continue;
    results.push({
      home: ev?.homeTeam?.name || '',
      away: ev?.awayTeam?.name || '',
      homeGoals: hs,
      awayGoals: as,
      date: (ev?.startTimestamp
        ? new Date(ev.startTimestamp * 1000).toISOString()
        : (ev?.time?.start || ev?.startDate || '')).slice(0, 10)
    });
    if (results.length >= 10) break;
  }
  return { results };
}

function formFromTeamEvents(json, teamId) {
  const events = json?.events || json?.results || [];
  const form = [];
  const homeForm = [];
  const awayForm = [];
  let gf = 0, ga = 0, games = 0;
  let gfHome = 0, gaHome = 0, gamesHome = 0;
  let gfAway = 0, gaAway = 0, gamesAway = 0;
  for (const ev of events) {
    const letter = resultLetterForTeam(ev, teamId);
    if (!letter) continue;
    const hs = _scoreCurrent(ev, 'home');
    const as = _scoreCurrent(ev, 'away');
    if (hs == null || as == null) continue;
    const isHome = ev?.homeTeam?.id === teamId;
    form.push(letter);
    gf += isHome ? hs : as;
    ga += isHome ? as : hs;
    games++;
    if (isHome) {
      if (homeForm.length < 10) homeForm.push(letter);
      gfHome += hs; gaHome += as; gamesHome++;
    } else {
      if (awayForm.length < 10) awayForm.push(letter);
      gfAway += as; gaAway += hs; gamesAway++;
    }
    if (form.length >= 10) break;
  }
  return {
    form,
    homeForm: homeForm.length ? homeForm : null,
    awayForm: awayForm.length ? awayForm : null,
    goalsFor:          games     ? gf / games         : null,
    goalsAgainst:      games     ? ga / games         : null,
    goalsForHome:      gamesHome ? gfHome / gamesHome : null,
    goalsAgainstHome:  gamesHome ? gaHome / gamesHome : null,
    goalsForAway:      gamesAway ? gfAway / gamesAway : null,
    goalsAgainstAway:  gamesAway ? gaAway / gamesAway : null,
    games
  };
}

/**
 * @param {string} team1 - mandante (Odds API)
 * @param {string} team2 - visitante
 * @param {string} commenceIso - ISO commence_time
 * @returns {Promise<null|{ homeFormData, awayFormData, h2hData, eventId, homeTeamId, awayTeamId }>}
 */
async function enrichMatch(team1, team2, commenceIso) {
  const hasProxy = !!(process.env.SOFASCORE_PROXY_BASE || '').trim();
  const envFb = String(process.env.SOFASCORE_ENRICH_FOOTBALL || '').toLowerCase();
  const enabled = envFb === 'true' || envFb === '1' || envFb === 'yes'
    || (envFb !== 'false' && envFb !== '0' && envFb !== 'no' && hasProxy);
  if (!enabled) return null;

  const d0 = String(commenceIso || '').slice(0, 10);
  const dates = new Set();
  if (/^\d{4}-\d{2}-\d{2}$/.test(d0)) {
    dates.add(d0);
    try {
      const t = new Date(commenceIso);
      const prev = new Date(t);
      prev.setUTCDate(prev.getUTCDate() - 1);
      dates.add(prev.toISOString().slice(0, 10));
      const next = new Date(t);
      next.setUTCDate(next.getUTCDate() + 1);
      dates.add(next.toISOString().slice(0, 10));
    } catch (_) {}
  }
  if (!dates.size) return null;

  let found = null;
  for (const d of dates) {
    const urls = scheduleUrlsForDate(d);
    const json = await fetchFirstJson(urls);
    if (!json) continue;
    const events = collectEventsFromSchedulePayload(json);
    for (const ev of events) {
      const hname = ev?.homeTeam?.name || ev?.homeTeam?.shortName || '';
      const aname = ev?.awayTeam?.name || ev?.awayTeam?.shortName || '';
      if (!teamsMatchEvent(hname, aname, team1, team2)) continue;
      found = {
        eventId: ev.id,
        homeTeamId: ev.homeTeam?.id,
        awayTeamId: ev.awayTeam?.id,
        homeName: hname,
        awayName: aname
      };
      break;
    }
    if (found) break;
  }
  if (!found?.eventId || !found.homeTeamId || !found.awayTeamId) return null;

  const h2hUrls = eventH2hUrls(found.eventId);
  const h2hJson = await fetchFirstJson(h2hUrls);
  const h2hData = h2hJson ? parseH2hToResults(h2hJson) : { results: [] };

  const [homeJson, awayJson] = await Promise.all([
    fetchFirstJson(teamEventsUrls(found.homeTeamId)),
    fetchFirstJson(teamEventsUrls(found.awayTeamId))
  ]);

  const homeFormData = homeJson ? formFromTeamEvents(homeJson, found.homeTeamId) : null;
  const awayFormData = awayJson ? formFromTeamEvents(awayJson, found.awayTeamId) : null;

  const hasAny = (homeFormData?.form?.length || awayFormData?.form?.length || h2hData.results.length);
  if (!hasAny) return null;

  return {
    homeFormData,
    awayFormData,
    h2hData,
    eventId: found.eventId,
    homeTeamId: found.homeTeamId,
    awayTeamId: found.awayTeamId
  };
}

/**
 * Lista matches FINALIZADOS num intervalo de dias passados.
 * Usado pra popular match_results e destravar settle de tips football.
 *
 * @param {object} opts
 * @param {number} opts.daysBack — quantos dias passados varrer (default 3)
 * @returns {Promise<Array<{eventId, home, away, homeId, awayId, homeScore, awayScore, winner, score, startIso, tournament}>>}
 */
async function getFinishedMatches({ daysBack = 3 } = {}) {
  const out = [];
  const today = new Date();
  const dates = [];
  for (let i = 0; i <= daysBack; i++) {
    const d = new Date(today.getTime() - i * 24 * 60 * 60 * 1000);
    dates.push(d.toISOString().slice(0, 10));
  }
  // Diagnóstico: conta quantos dias retornaram dados vs vazios vs eventos finished.
  // Motivo: pre-sync football retornou 0 inserções mesmo com proxy ativo (tennis
  // funcionou no mesmo proxy). Precisamos distinguir "proxy não tem rota football"
  // de "proxy retorna mas payload vazio" de "retorna payload mas nenhum finished".
  const diag = { days_tried: dates.length, days_no_data: 0, days_empty_payload: 0,
                 total_events: 0, finished_events: 0, first_url: null };
  for (const iso of dates) {
    const urls = scheduleUrlsForDate(iso);
    if (!urls.length) continue;
    if (!diag.first_url) diag.first_url = urls[0];
    const data = await fetchFirstJson(urls);
    if (!data) { diag.days_no_data++; continue; }
    const events = collectEventsFromSchedulePayload(data);
    diag.total_events += events.length;
    if (events.length === 0) diag.days_empty_payload++;
    for (const ev of events) {
      // Status "finished" pode aparecer em ev.status.type ou ev.status.code (100 = finished).
      const statusType = String(ev?.status?.type || ev?.status?.description || '').toLowerCase();
      const statusCode = Number(ev?.status?.code);
      const isFinished = statusType === 'finished' || statusCode === 100;
      if (!isFinished) continue;
      const hs = _scoreCurrent(ev, 'home');
      const as = _scoreCurrent(ev, 'away');
      if (!Number.isFinite(hs) || !Number.isFinite(as)) continue;
      const home = String(ev?.homeTeam?.name || '').trim();
      const away = String(ev?.awayTeam?.name || '').trim();
      if (!home || !away) continue;
      const winner = hs > as ? home : (as > hs ? away : 'Draw');
      const startTs = Number(ev?.startTimestamp);
      const startIso = Number.isFinite(startTs)
        ? new Date(startTs * 1000).toISOString().slice(0, 19).replace('T', ' ')
        : iso + ' 00:00:00';
      out.push({
        eventId: String(ev.id),
        home, away,
        homeId: ev?.homeTeam?.id, awayId: ev?.awayTeam?.id,
        homeScore: hs, awayScore: as,
        winner, score: `${hs}-${as}`,
        startIso,
        tournament: ev?.tournament?.name || ev?.tournament?.uniqueTournament?.name || '',
      });
      diag.finished_events++;
    }
  }
  // Log só se zerou — operação normal é silent pra não poluir log.
  // 2026-05-01: throttle WARN quando sofascore proxy está com falha sustentada.
  // Antes: cada call de pre-sync football logava WARN (URL longo, ruido) mesmo
  // quando ESPN cobria. Agora 1 WARN/h quando 100% no-data; demais ciclos silentes.
  if (out.length === 0) {
    const isProxyDead = diag.days_no_data === diag.days_tried;
    global._sofaFootballLastWarn = global._sofaFootballLastWarn || 0;
    const sinceMs = Date.now() - global._sofaFootballLastWarn;
    if (!isProxyDead || sinceMs > 60 * 60 * 1000) {
      log('WARN', 'FOOTBALL-SYNC',
        `sofascore getFinished: 0 matches de ${diag.days_tried}d (no-data=${diag.days_no_data}, empty=${diag.days_empty_payload}, events=${diag.total_events}, finished=${diag.finished_events}, url=${diag.first_url || '?'})`);
      global._sofaFootballLastWarn = Date.now();
    }
  }
  return out;
}

module.exports = { enrichMatch, normTeam, teamsMatchEvent, getFinishedMatches };

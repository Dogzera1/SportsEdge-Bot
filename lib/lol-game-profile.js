...
function _meanCs(picks, timing) {
  let sum = 0, cnt = 0;
  for (const p of (picks || [])) { const c = timing.byChampRole[_normC(p) + '|' + _normR(p)]; if (c && c.n > 0) { sum += c.csdiff15; cnt++; } }
  return cnt ? sum / cnt : 0;
}
...
function phaseEdges(draft, timing) {
  ...
  const csDiff = _meanCs(draft.blue, timing) - _meanCs(draft.red, timing);
  ...
  return {
    early: { ..._phaseFromScore(earlyScore), edge: +earlyScore.toFixed(3), measured: true,
             anchor: { golddiff15: Math.round(goldDiff), xpdiff15: Math.round(xpDiff), csdiff15: Math.round(csDiff) }, confidence: 0.8 },

...
// 2026-05-11 (audit cross-sport): defaults mais responsivos pra capturar
// edge em sports baixo-volume. Antes: minNUp=30 / step=0.05 / cap=1.20 →
// sweet spots nunca destravavam (basket/football/dota nunca atingiam n=30).
// Agora: minNUp=20 / stepUp=0.08 / cap=1.50 → permite +50% boost vs +20%.
// Trade-off: variance +5pp em sweet spots, mas growth potencial 2.5× maior.
// Todos via env pra rollback fácil.
const DEFAULT_CONFIG = {
  days: parseInt(process.env.KELLY_AUTO_TUNE_DAYS || '30', 10) || 30,
  minNUp: parseInt(process.env.KELLY_AUTO_TUNE_MIN_N_UP || '20', 10) || 20,
  minNDown: parseInt(process.env.KELLY_AUTO_TUNE_MIN_N_DOWN || '10', 10) || 10,
  roiUpThreshold: parseFloat(process.env.KELLY_AUTO_TUNE_ROI_UP || '5'),
  roiDownThreshold: parseFloat(process.env.KELLY_AUTO_TUNE_ROI_DOWN || '-5'),
  clvUpThreshold: parseFloat(process.env.KELLY_AUTO_TUNE_CLV_UP || '0'),
  clvDownThreshold: parseFloat(process.env.KELLY_AUTO_TUNE_CLV_DOWN || '-3'),
  stepUp: parseFloat(process.env.KELLY_AUTO_TUNE_STEP_UP || '0.08'),
  stepDown: parseFloat(process.env.KELLY_AUTO_TUNE_STEP_DOWN || '0.12'),
  multFloor: parseFloat(process.env.KELLY_AUTO_TUNE_FLOOR || '0.20'),
  multCeiling: parseFloat(process.env.KELLY_AUTO_TUNE_CEILING || '1.50'),
  // Cooldown: bloqueia novos adjusts dentro dessa janela após último UP/DOWN.
  // Default 3d (era 5d) — mais responsivo a mudanças de regime.
  cooldownDays: parseInt(process.env.KELLY_AUTO_TUNE_COOLDOWN_DAYS || '3', 10) || 3,
};
...
function _computeSportMetrics(db, sport, days, market = null) {
  ...
  // Frozen holdout: exclui últimos N dias (FROZEN_HOLDOUT_DAYS, override
  // FROZEN_HOLDOUT_KELLY_DAYS). Default OFF (0). Quando setado, kelly tune
  // não treina nos últimos N dias — preservados pra walk-forward eval.
  const holdoutClause = require('./frozen-holdout').getHoldoutSql('kelly', 'settled_at');

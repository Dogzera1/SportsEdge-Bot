...
function runKellyAutoTune(db, opts = {}) {
  ...
  for (const sport of sports) {
    ...
    // 2026-05-05: respeita correções do readiness-learner. Se há kelly_cut
    // ativo no readiness_corrections_log pra este sport, pula update —
    // verify/escalation/revert daquela correção é responsabilidade do learner.
    // Sem isso o kelly tune diário desfazia o cut antes do learner verificar.
    if (decision.action === 'up' || decision.action === 'down') {
      try {
        const lock = db.prepare(`
          SELECT id, applied_at, expires_at FROM readiness_corrections_log
          WHERE sport = ? AND action_type = 'kelly_cut' AND status = 'active'
          ORDER BY applied_at DESC LIMIT 1
        `).get(sport);
        if (lock) {
          results.push({ sport, action: 'readiness_lock', mult: currentMult, blocked: decision.action,
                         reason: `readiness correction #${lock.id} ativa até ${lock.expires_at}`, metrics });
          continue;
        }

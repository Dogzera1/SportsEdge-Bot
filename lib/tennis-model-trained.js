...
/**
 * Wrapper bidirectional pra mitigar assimetria p1↔p2 do modelo treinado.
 * 2026-05-28 audit confirmou: predictTrainedTennis(A,B).p1 + predictTrainedTennis(B,A).p1 ≠ 1.0
 * (Avg |Δ|=5.28pp Bo5 clay RG, Max 7.48pp). Causa: treino p1=alfabético,
 * runtime p1=feed favorito. Interactions elo×bo5 + elo×clay amplificam.
 *
 * Mitigação: chama nos dois sentidos, retorna média ((p1_fwd + (1 - p1_rev)) / 2).
 * Eliminates structural slot-1 inflation sem retreinar. Custo: 2× CPU (negligível).
 *
 * Env TENNIS_ML_SYMMETRIC=false desativa (volta ao comportamento original).
 */
function predictTrainedTennisSymmetric(ctx) {
  const disabled = /^(0|false|no)$/i.test(String(process.env.TENNIS_ML_SYMMETRIC ?? 'true'));
  if (disabled) return predictTrainedTennis(ctx);
  const fwd = predictTrainedTennis(ctx);
  if (!fwd) return null;
  // Swap p1↔p2 inputs preservando demais campos.
  const swap = (k1, k2) => { const t = ctx[k1]; ctx[k1] = ctx[k2]; ctx[k2] = t; };
  const swapKeys = [
    ['eloOverall1','eloOverall2'], ['eloSurface1','eloSurface2'],
    ['gamesSurface1','gamesSurface2'], ['rank1','rank2'],
    ['rankPoints1','rankPoints2'], ['age1','age2'], ['height1','height2'],
    ['servePct1','servePct2'], ['fatigueMin7d_1','fatigueMin7d_2'],
    ['matches14d_1','matches14d_2'], ['daysSinceLast1','daysSinceLast2'],
    ['h2hSurface1','h2hSurface2'], ['h2hAll1','h2hAll2'],
    ['player1Name','player2Name'],
  ];
  // winStreakDiff / wrLast10Diff são DIFFs (já direcionais) — negar valor ao invés de swap key.
  const ctxRev = { ...ctx };
  for (const [a,b] of swapKeys) { const t = ctxRev[a]; ctxRev[a] = ctxRev[b]; ctxRev[b] = t; }
  if (Number.isFinite(+ctxRev.winStreakDiff)) ctxRev.winStreakDiff = -(+ctxRev.winStreakDiff);
  if (Number.isFinite(+ctxRev.wrLast10Diff))  ctxRev.wrLast10Diff  = -(+ctxRev.wrLast10Diff);
  const rev = predictTrainedTennis(ctxRev);
  if (!rev) return fwd;
  const p1Sym = (fwd.p1 + (1 - rev.p1)) / 2;
  return {
    ...fwd,
    p1: +p1Sym.toFixed(4),
    p2: +(1 - p1Sym).toFixed(4),
    calibrated: +p1Sym.toFixed(4),
    method: fwd.method + '+sym',
    sym: { fwd_p1: fwd.p1, rev_p1: rev.p1, delta: +(fwd.p1 + rev.p1 - 1).toFixed(4) },
  };
}

module.exports = {
  predictTrainedTennis,
  predictTrainedTennisSymmetric,
  hasTrainedModel,

...
function buildRuntimeFeatureVector(ctx) {
  ...
  // Ordem PRECISA bater com FEATURE_NAMES do treino:
  // [NUM_FEATURES..., surface_hard, surface_clay, surface_grass,
  //  elo_x_bestof5, elo_x_clay, elo_x_grass]
  const v = [
  ...
  return { vector: v, nSignals, eloBlend, surface };
}
...
function predictTrainedTennis(ctx) {
  ...
  const { vector: v, nSignals, eloBlend, surface } = buildRuntimeFeatureVector(ctx);
  ...
  const calibrated = applyIsotonic(W.calibration?.blocks, raw);
  ...
  return {
    ...
    method: `trained(${pGbdt != null ? 'logistic+gbdt' : 'logistic'}+isotonic)`,

...
function scanBasketMarkets({
  ...
  minPmodel = 0.50,
  minOdd = 1.50, maxOdd = 3.50,
  // 2026-05-06 FIX: cap por match. Antes basket era único MT scanner sem
  // limite — slate NBA com 10 totals + 5 spreads × 2 sides = até 30 tips
  // num único jogo, gerava correlação cruzada e overstaking.
  // Default 3 (top-K por EV). 0 = unlimited.
  maxPerMatch = parseInt(process.env.BASKET_MT_MAX_PER_MATCH || '3', 10),
} = {}) {
  ...
  for (const t of totals) {
    ...
    // 2026-05-22: pModelRaw = pModel (basket sem isotonic — mig 117 audit cross-sport).
  ...
  // 2026-05-06 FIX: maxPerMatch slice top-K por EV.
...
/**
 * Per-quarter scanner: extrai handicap + total NBA period 1..4 (Q1..Q4).
 * Pinnacle expõe quarter markets com mesma estrutura (spread + total).
 *
 * Math:
 *   μ_quarter ≈ μ_full / 4 (scoring evenly distributed assumption)
 *   σ_quarter ≈ σ_full / 2 (CLT — quarter variance ≈ full × 1/4 → σ × sqrt(1/4))
 *
 * Pra Q1 especificamente, μ é tipicamente +1-2% acima da média per quarter
 * (teams start higher pace), mas first-order approximation suficiente em
 * shadow eval. Refinar pós-30d com bias detection per quarter.
 *
 * @param {object} args                    igual scanBasketMarkets +
 * @param {number} args.quarter            1, 2, 3, 4 (NBA standard)
 * @returns {Array} tips com market='total_q<N>' / 'handicap_q<N>'
 */
function scanBasketQuartersMarkets(args = {}) {
  const { quarter, trainedMarkets, ...rest } = args;
  if (!Number.isFinite(quarter) || quarter < 1 || quarter > 4) return [];
  if (!trainedMarkets) return [];
  const { totalMu, totalSigma, marginMu, marginSigma } = trainedMarkets;
  // Scale μ/σ pra escopo do quarter.
  const scaledTrained = {
    totalMu: totalMu / 4,
    totalSigma: totalSigma / 2,
    marginMu: marginMu / 4,
    marginSigma: marginSigma / 2,
  };
  // Filter line ranges typical Q1 totals 50-70 (vs full 200-260).
  // Override do scanner default em rest via line range não trivial — vamos
  // post-filter ao invés de mexer em scanBasketMarkets.
  const baseTips = scanBasketMarkets({ ...rest, trainedMarkets: scaledTrained });
  // Re-label markets pra Q-specific + filtra ranges NBA quarter.
  const out = [];
  for (const t of baseTips) {
    if (t.market === 'total') {
      // Quarter totals 35-75 typical.
      if (t.line < 35 || t.line > 75) continue;
      out.push({ ...t, market: `total_q${quarter}`, label: t.label.replace('pts', `pts Q${quarter}`) });
    } else if (t.market === 'handicap') {
      // Quarter spreads ±0.5..±10.5 typical.
      if (Math.abs(t.line) > 10.5) continue;
      out.push({ ...t, market: `handicap_q${quarter}`, label: t.label + ` Q${quarter}` });
    }
  }
  return out;
}

module.exports = { scanBasketMarkets, scanBasketQuartersMarkets, _normalCdf };

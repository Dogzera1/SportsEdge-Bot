...
// 2026-05-07 (audit P2): cap defensivo + sweep on-write ocasional. Antes _cache
// só evictava on-get expirado → key não-mais-consultada ficava forever na Map.
// Cap 500 entries (paths ~ ligas × tipos ≈ 50-100 normais; 500 é folga grande).
const _CACHE_MAX = parseInt(process.env.API_FOOTBALL_CACHE_MAX || '500', 10) || 500;
...
function _cacheSweepExpired() {
  const now = Date.now();
  for (const [k, v] of _cache.entries()) if (now > v.exp) _cache.delete(k);
  // Cap secundário (LRU por exp): drop oldest
  if (_cache.size > _CACHE_MAX) {
    const sorted = [..._cache.entries()].sort((a, b) => a[1].exp - b[1].exp);
    const drop = _cache.size - _CACHE_MAX;
    for (let i = 0; i < drop; i++) _cache.delete(sorted[i][0]);
  }
}
function _cacheSet(k, d, ttlMs) {
  _cache.set(k, { data: d, exp: Date.now() + ttlMs });
  // Sweep on-write a cada 50 sets (amortizado, baixo overhead).
  if ((++_cacheWriteCount % 50) === 0) _cacheSweepExpired();
}
    ...
    const req = https.request(url, {
      ...
      res.on('end', () => {
        ...
        try {
          ...
          // 2026-05-23 leak fix F4: não cachear empty/malformed responses
          // (provider às vezes retorna 200 com body vazio ou HTML interstitial
          // soft-block sob quota burn — cacha {} por TTL inteiro engolia
          // fixture lookup, gerando "no fixture" → zombies downstream).
          // Cacha só se json é object com pelo menos 1 key OR é array não-vazio.
          const isEmpty = (json && typeof json === 'object'
            && (Array.isArray(json) ? json.length === 0 : Object.keys(json).length === 0));
          if (ttlMs > 0 && !isEmpty) _cacheSet(cacheKey, json, ttlMs);
          resolve(json);
        } catch (e) { reject(e); }
      });
    });
...
// 2026-05-07 (audit P2): cap + sweep on-write. ~30-100 ligas ativas = baixo,
// mas sem cap key abandonada (final de season) ficava forever. 200 é folga.
const _FIXTURE_CACHE_MAX = parseInt(process.env.API_FOOTBALL_FIXTURE_CACHE_MAX || '200', 10) || 200;
let _fixtureCacheWriteCount = 0;
function _fixtureCacheSweep() {
  const now = Date.now();
  for (const [k, v] of _fixtureCache.entries()) if (now > v.exp) _fixtureCache.delete(k);
  if (_fixtureCache.size > _FIXTURE_CACHE_MAX) {
    const sorted = [..._fixtureCache.entries()].sort((a, b) => a[1].exp - b[1].exp);
    const drop = _fixtureCache.size - _FIXTURE_CACHE_MAX;
    for (let i = 0; i < drop; i++) _fixtureCache.delete(sorted[i][0]);
  }
}
...
async function getLeagueFixtures(leagueId, season) {
  ...
  if ((++_fixtureCacheWriteCount % 30) === 0) _fixtureCacheSweep();
  return fixtures;
}

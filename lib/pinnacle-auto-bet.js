...
function fireAutoBetHook(db, opts = {}) {
  ...
  (async () => {
    try {
      ...
      if (tipRow.pinnacle_bet_id) return; // idempotency: já apostou nesta tip
      ...
      if (blockList.includes(sportKey)) return;
      const allowList = String(process.env.PINNACLE_AUTO_BET_SPORTS_ALLOW || '')
        .toLowerCase().split(',').map(s => s.trim()).filter(Boolean);
      if (allowList.length > 0 && !allowList.includes(sportKey)) return;

...
async function tryAutoBet(db, opts = {}) {
  ...
  // ── Gate 6b: enforce require_confirm BEFORE Phase 2 real bet.
  // FIX 2026-05-27 (audit P0 FIN): variável requireConfirm era apenas metadado em
  // payload, nunca usada pra bloquear flow. Phase 2 Playwright LIVE em prod sem
  // segunda verificação humana — user CRIA ter safety net que não existia.
  // Effect: ZERA bets reais até Phase 2b (DM Telegram + botão aprovação que passe
  // opts.confirmed=true) ser implementado. Rollback: SET PINNACLE_AUTO_BET_REQUIRE_CONFIRM=false.
  if (requireConfirm && !opts.confirmed) {
    log('WARN', 'PINNACLE-AUTO-BET', `awaiting admin confirm: tip#${tip?.id} ${sport}/${marketId}/${side} stake=R$${stakeBrl.toFixed(2)} | EV ${evPct?.toFixed(1)}%`);
    try { require('./metrics').incr('pinnacle_auto_bet_awaiting_confirm', { sport, market: marketId }); } catch (_) {}
    return { ok: false, dry: false, reason: 'awaiting_admin_confirm', would_bet: payload };
  }
  ...
  try {
    ...
    // FIX 2026-05-27 (audit P0 FIN): local slippage post-validation. Executor pode
    // enforçar OR mentir — bot envia max_slippage_pct=2 mas nunca valida actual_odd
    // retornado. Bet já placed (não dá pra undo), apenas log+metric pra reconciliation
    // manual + visibility em alertas.
    if (Number.isFinite(body.actual_odd) && Number.isFinite(expectedOdd) && expectedOdd > 0) {
      const slipPct = ((expectedOdd - body.actual_odd) / expectedOdd) * 100;
      if (slipPct > maxSlippagePct) {
        log('ERROR', 'PINNACLE-AUTO-BET', `slippage exceeded: ${slipPct.toFixed(2)}% > ${maxSlippagePct}% | expected=${expectedOdd} actual=${body.actual_odd} | tip#${tip?.id} ${sport}/${marketId}/${side}`);
        try { require('./metrics').incr('pinnacle_slippage_exceeded', { sport, market: marketId }); } catch (_) {}
      }
    }

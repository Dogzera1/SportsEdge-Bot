...
function fireAutoBetHook(db, opts = {}) {
  ...
  (async () => {
    try {
      ...
      try {
        tipRow = db.prepare(`
          SELECT id, sport, stake_reais, is_shadow, is_live, archived,
      ...
      if (stakeBrl <= 0) { log('INFO', 'PINNACLE-AUTO-BET', `skip tip#${opts.tipId}: stake_reais=${tipRow.stake_reais} (<=0)`); return; }
      ...
      log('INFO', 'PINNACLE-AUTO-BET', `FIRE tip#${opts.tipId} sport=${sportKey} stake=R$${stakeBrl.toFixed(2)} market=${opts.marketId} side=${opts.side} → tryAutoBet`);

...
function _parseMapHeader(html) {
  ...
  // 2026-05-20: capture score per-map (X-Y format). VLR header tem <div class="score">N</div>
  // tanto pra winner (com mod-win) quanto pra loser (sem mod-win).
  // 2026-05-24: regex permissiva pra modifiers extra (mod-final, etc) — VLR mudou
  // markup em prod (G2 vs NRG VLR 660380 capturava winner mas score=null → log
  // mostrava "(?-?)"). Aceita N modifiers depois de "score".
  // 2026-05-25: fix 13cab3e parcial — VLR live tem `class="score "` (trailing
  // space) + `style="margin-right: 12px;"` attr. `\s*` antes do quote final cobre
  // o caso. Audit VLR 660380: `<div class="score mod-win" style=...>13 </div>` e
  // `<div class="score " ...>8</div>` ambos válidos agora.
  const t1Score = (t1Slice[1].match(/<div class="score(?:\s+[\w-]+)*\s*"[^>]*>\s*(\d+)\s*<\/div>/) || [])[1];
  ...
  const t2Score = (t2Slice[1].match(/<div class="score(?:\s+[\w-]+)*\s*"[^>]*>\s*(\d+)\s*<\/div>/) || [])[1];
  ...
  const score = (t1Score && t2Score) ? `${t1Score}-${t2Score}` : null;

  return { team1: t1Name.trim(), team2: t2Name.trim(), mapName, winner, score };
}
...
const { cachedHttpGet, BROWSER_UA } = require('./utils');
const VLR_BROWSER = {
...
// 2026-05-15: alias expansion antes de normName. Pinnacle/PandaScore usam
// abbreviations ("VIT", "FUT", "KC") enquanto VLR canonical é completo
// ("Team Vitality", "FUT Esports", "Karmine Corp"). Case observado prod:
// /live-snapshot duplicava match "Vitality vs FUT" + "FUT Esports vs Team
// Vitality" porque dedup falhava no naming divergent. Espelha fix Dota
// bf8dad8 commit anterior. Opt-out via VLR_ALIAS_DISABLED=true.
let _valorantExpandAlias;
try {
  _valorantExpandAlias = process.env.VLR_ALIAS_DISABLED === 'true'
    ? (s) => s
    : require('./valorant-team-aliases').expandAlias;
} catch (_) { _valorantExpandAlias = (s) => s; }

function _namesMatch(vlrName, query) {
  const a = _normName(_valorantExpandAlias(vlrName)), b = _normName(_valorantExpandAlias(query));
  if (!a || !b) return false;
...
async function _cachedGet(path, ttlMs, cacheKey) {
  ...
  const r = await cachedHttpGet(url, {
    ...
    cacheKey: cacheKey || `vlr:${path}`, retry: true,  // 2026-05-29 audit: transient VLR hiccup → retry (breaker conta resultado final)
  }).catch(() => null);
  if (!r || r.status !== 200) { _vlrNoteFail(); return null; }
  // 2026-05-29 audit #8: NÃO marca sucesso aqui. Um HTTP 200 com HTML de estrutura
  // quebrada não é sucesso — senão resetaria o breaker antes do parse-fail acumular
  // (mudança de markup VLR ficava invisível). findLiveMatch/findFinishedMatchByTeams
  // marcam _vlrNoteSuccess no parse OK (blocks>0) e _vlrNoteFail em blocks===0.
  return r.body || '';
}
...
async function findLiveMatch(team1, team2) {
  ...
  if (!html) return null;
  // 2026-04-17: VLR.gg inverteu ordem dos atributos (href agora antes de class).
  // Lookahead-based regex aceita os dois atributos em qualquer ordem.
  const blocks = [...html.matchAll(/<a(?=[^>]*class="[^"]*match-item[^"]*")(?=[^>]*href="(\/(\d+)\/[^"]+)")[^>]*>[\s\S]*?<\/a>/g)];
  if (!blocks.length) { _vlrNoteFail(); return null; }  // 2026-05-29 #8: 0 match-items num 200 = estrutura VLR mudou → trip breaker
  _vlrNoteSuccess();  // lista parseou (estrutura íntegra), mesmo que o time não esteja live (normal, não é falha)
  for (const b of blocks) {
    const [block, , id] = b;
    if (!/live-now|match-item-live|\bLIVE\b/i.test(block)) continue;
    // Bandeira de país agora aparece dentro de match-item-vs-team-name como <span class="flag">XX</span>;
    // pula essa span pra capturar o nome do time no <div class="text-of">.
    const names = [...block.matchAll(/match-item-vs-team-name[\s\S]*?<div[^>]*class="text-of"[^>]*>(?:\s*<span[^>]*>[^<]*<\/span>)?\s*([^<]+?)\s*<\/div>/g)]
      .map(m => m[1].trim());
...
/**
 * 2026-05-20 Sprint 6 — busca matchId em /matches/results scanneando teams.
 * Para settle de tips Valorant per-map (market_type='MAP{N}_WINNER').
 *
 * VLR.gg /matches/results lista finished matches recentes (50/page).
 * Parse HTML extrai matchId + team1/team2 em 1 fetch → match local por nomes.
 * Mais rápido que fetchMatchMaps N vezes.
 *
 * @param {string} team1 — nome do time 1 (PandaScore convention)
 * @param {string} team2 — nome do time 2
 * @param {number} sentAtMs — timestamp da tip (opcional, pra filtrar window)
 * @returns {Promise<{matchId, t1, t2}|null>}
 */
async function findFinishedMatchByTeams(team1, team2, sentAtMs) {
  if (!_vlrEnabled() || !team1 || !team2) return null;
  const html = await _cachedGet('/matches/results', 60_000, 'vlr:results-list');
  if (!html) return null;
  // 2026-05-20: regex captura matchId href + team names (text-of div) por match item.
  // Mesma estrutura de findLiveMatch mas sem filter LIVE.
  const blocks = [...html.matchAll(/<a(?=[^>]*class="[^"]*match-item[^"]*")(?=[^>]*href="(\/(\d+)\/[^"]+)")[^>]*>([\s\S]*?)<\/a>/g)];
  if (!blocks.length) { _vlrNoteFail(); return null; }  // 2026-05-29 #8: 0 match-items num 200 = estrutura VLR mudou → trip breaker
  _vlrNoteSuccess();
  for (const b of blocks) {
    const [, , id, body] = b;
    const names = [...body.matchAll(/match-item-vs-team-name[\s\S]*?<div[^>]*class="text-of"[^>]*>(?:\s*<span[^>]*>[^<]*<\/span>)?\s*([^<]+?)\s*<\/div>/g)]
      .map(m => m[1].trim());
    if (names.length < 2) continue;
    const [a, c] = names;
    const fwd = _namesMatch(a, team1) && _namesMatch(c, team2);
    const rev = _namesMatch(a, team2) && _namesMatch(c, team1);
    if (!fwd && !rev) continue;
    return { matchId: id, t1: a, t2: c };
  }
  return null;
}

/**
 * 2026-05-20 Sprint 6 — wrapper user-facing pra settle de tips per-map Valorant.
 * Combina findFinishedMatchByTeams + fetchMatchMaps em 1 call.
 * Retorna [{map, mapName, winner, score, played}] mirror da API HLTV (Sprint 4).
 */
async function getValorantMatchMapResults(team1, team2, sentAtMs) {
  const match = await findFinishedMatchByTeams(team1, team2, sentAtMs);
  if (!match?.matchId) return null;
  const maps = await fetchMatchMaps(match.matchId);
  if (!maps?.maps) return null;
  return {
    vlrMatchId: match.matchId,
    team1: maps.team1,
    team2: maps.team2,
    maps: maps.maps.map(m => ({
      ...
      mapName: m.name,
      winner: m.winner,
      score: m.score || null,  // 2026-05-20: agora extraído de vm-stats-game-header (X-Y)
      played: !!m.winner,
    })),
  };
}
...
module.exports = {
  ...
  fetchMatchMaps,
  findLiveMatch,
  findFinishedMatchByTeams,
  getValorantMatchMapResults,
  getMatchStats,
  summarizeLive,
  vlrBreakerStatus,
  // Pure functions exposed pra testes (sem network calls).
  _parseMapHeader,
  _parseAllMapHeaders,
};

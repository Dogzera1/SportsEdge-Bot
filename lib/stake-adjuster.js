...
function getLeagueStats(db, sport, league) {
  ...
  // 2026-05-06 FIX: shadow tips antes contaminavam league multiplier
  // (research-only não devia influenciar stake real). Mesmo bug do
  // auto_leak_guard_shadow_bug — filtrar is_shadow=0.
...
function getRecentStreak(db, sport, lookback = 15) {
  // 2026-05-06 FIX: filtrar shadow — streak damping deve refletir tips reais
  // dispatched, não research-only.
  const rows = db.prepare(`
...
function getDailyPnL(db, sport) {
  ...
  // 2026-05-06 FIX: filtrar shadow — daily stop-loss deve disparar só por
  // perda REAL, não por shadow research. Antes shadow loss inflava lossPct.

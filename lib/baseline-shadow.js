...
function settleBaselineShadow(db) {
  ...
  // 2026-05-06 FIX: cs↔cs2 normalization. baseline grava bst.sport='cs' (bucket
  // bankroll convention), HLTV/PandaScore guardam game='cs' OU 'cs2'. Antes
  // JOIN strict perdia tips PandaScore. Agora aceita 'cs' E 'cs2' quando bucket='cs'.
  const pending = db.prepare(`
    ...
    INNER JOIN match_results mr ON
      (mr.game = bst.sport
       OR (bst.sport = 'cs' AND mr.game = 'cs2')
       OR (bst.sport = 'cs2' AND mr.game = 'cs'))
      AND mr.match_id = bst.match_id

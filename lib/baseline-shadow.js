...
function settleBaselineShadow(db) {
  ...
  // Match_results table tem schema: (sport, match_id, team1_score, team2_score, ...)
  // Junta em sport + match_id; pra esports/tennis match_id é compartilhado
  // com tips/match_results.
  const pending = db.prepare(`
    SELECT bst.id, bst.sport, bst.match_id, bst.side, bst.best_odd
    FROM baseline_shadow_tips bst
    INNER JOIN match_results mr ON mr.sport = bst.sport AND mr.match_id = bst.match_id
    WHERE bst.settled_at IS NULL
      AND mr.team1_score IS NOT NULL
      AND mr.team2_score IS NOT NULL
    LIMIT 500
  `).all();
  ...
  for (const row of pending) {
    try {
      const mr = db.prepare(`
        SELECT team1_score AS s1, team2_score AS s2
        FROM match_results WHERE sport = ? AND match_id = ?
      `).get(row.sport, row.match_id);
      if (!mr) continue;
      const t1Won = mr.s1 > mr.s2;
      const t2Won = mr.s2 > mr.s1;
      const isPush = mr.s1 === mr.s2;

      let result, profit;
      if (isPush) { result = 'push'; profit = 0; }
      else if (row.side === 'team1' && t1Won) { result = 'win'; profit = +(row.best_odd - 1).toFixed(4); }
      else if (row.side === 'team2' && t2Won) { result = 'win'; profit = +(row.best_odd - 1).toFixed(4); }
      else { result = 'loss'; profit = -1; }

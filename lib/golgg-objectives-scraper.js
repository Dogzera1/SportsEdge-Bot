...
async function fetchObjectivesViaGolgg(gameId) {
  ...
  // Tenta page-summary primeiro (onde estão linhas Kills/Towers/etc team summary).
  // page-game tem player-level KDA, sem linha Kills agregada.
  const urls = [
  ...
  for (const url of urls) {
    const candidate = await httpGetHtml(url);
    if (candidate.status === 200 && candidate.body && /\bKills\b/i.test(candidate.body)) {
      r = candidate;
      urlUsed = url;
      break;
    }
  }
  if (!r) return { ok: false, reason: 'http_fail_all_urls' };

  const objectives = {};
  const debug = { url_used: urlUsed };

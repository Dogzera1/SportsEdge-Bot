...
function esportsPreFilter(match, odds, enrich, hasLiveStats, gamesContext, compScore, stmts = null, opts = {}) {
  ...
  if (f1Games < 3 && g1n >= 3 && safeNum(g1?.winRate) != null) {
    wr1 = safeNum(g1.winRate);
    f1Games = g1n;
  }
  if (f2Games < 3 && g2n >= 3 && safeNum(g2?.winRate) != null) {
    wr2 = safeNum(g2.winRate);
    f2Games = g2n;
  }
  // Small-sample guard: exige ≥3 jogos de cada lado. Com 1-2 jogos, wr=100%/0% infla scorePoints.
  if (wr1 !== null && wr2 !== null && f1Games >= 3 && f2Games >= 3) {
  ...
  const useGridH2h = gh2h && (gh2h.t1Wins + gh2h.t2Wins >= 2)
    && (!h2h || (parseInt(h2h.t1Wins, 10) + parseInt(h2h.t2Wins, 10) < 2));
  const h2hEff = useGridH2h ? gh2h : h2h;
  // Small-sample guard: exige ≥2 partidas H2H. Com 1 match isolado, WR=100%/0% não é sinal real.
  if (h2hEff && (h2hEff.t1Wins + h2hEff.t2Wins >= 2)) {
  ...
  const modelP1Raw = 1 / (1 + Math.exp(-logOddsBase));
  // Clamp opt-in (via opts.maxDivergence): limita divergência vs mercado pra evitar EV absurdo
  // a partir de modelP inflado por small-sample. Desligado por padrão para preservar callers existentes.
  const maxDivergence = parseFloat(opts?.maxDivergence);
  const modelP1 = Number.isFinite(maxDivergence) && maxDivergence > 0
    ? Math.min(impliedP1 + maxDivergence, Math.max(impliedP1 - maxDivergence, modelP1Raw))
    : modelP1Raw;
  ...
  const rawEdge = Math.abs((modelP1Raw - impliedP1) * 100);
  ...
  // Bonus de gate para partidas AO VIVO sem live stats (STATS_DISABLED pela Riot em tier-2).
  // Nesses casos so temos draft/forma/h2h — edge precisa ser maior pra compensar a cegueira.
  const isLiveNoStats = (match?.status === 'live' || match?.status === 'inprogress') && !hasLiveStats;
  const statsPenalty = isLiveNoStats ? parseFloat(process.env.LOL_LIVE_NOSTATS_BONUS ?? '2.0') : 0;

  // 2026-05-13: per-sport ML edge minimum override (shadow research expansion).
  // Quando opts.sport setado E env <SPORT>_ML_EDGE_MIN definido, substitui
  // thresholds default (3pp comp / 4pp no-comp). Permite expandir SHADOW
  // universe em sports com lift baixo (mma/dota/val) sem afetar LoL.
  // Opcional — fallback pro path comp/no-comp existente se env não setado.
  const _optsSport = opts?.sport ? String(opts.sport).toLowerCase() : null;
  if (_optsSport) {
    const _spU = _optsSport.toUpperCase().replace(/[^A-Z0-9]/g, '');
    const _sportEdgeOverride = parseFloat(process.env[`${_spU}_ML_EDGE_MIN`]);
    if (Number.isFinite(_sportEdgeOverride)) {
      const _threshold = _sportEdgeOverride + statsPenalty;
      if (maxEdge < _threshold) {
        return { pass: false, direction: null, score: maxEdge, t1Edge, t2Edge, modelP1, modelP2, modelP1Raw, rawEdge, scorePoints, impliedP1, impliedP2, factorCount, factorActive };
      }
      return { pass: true, direction, score: maxEdge, t1Edge, t2Edge, modelP1, modelP2, modelP1Raw, rawEdge, scorePoints, impliedP1, impliedP2, factorCount, factorActive };
    }
  }

  if (hasComp) {
    // Com compScore disponível, exige edge real > 3pp (5pp se live sem stats)
    const minEdgeComp = 3.0 + statsPenalty;
    if (maxEdge < minEdgeComp) return { pass: false, direction: null, score: maxEdge, t1Edge, t2Edge, modelP1, modelP2, modelP1Raw, rawEdge, scorePoints, impliedP1, impliedP2, factorCount, factorActive };
    return { pass: true, direction, score: maxEdge, t1Edge, t2Edge, modelP1, modelP2, modelP1Raw, rawEdge, scorePoints, impliedP1, impliedP2, factorCount, factorActive };
  }
  ...
  if (factorCount >= 2) {
    const MIN_EDGE_NO_COMP = (parseFloat(process.env.LOL_MIN_EDGE_NO_COMP ?? '4.0') || 4.0) + statsPenalty;

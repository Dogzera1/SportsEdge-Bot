function esportsPreFilter(match, odds, enrich, hasLiveStats, gamesContext, compScore) {
  ...
  if (factorCount < 1) return { pass: true, direction: null, score: 0, t1Edge: 0, t2Edge: 0, modelP1: impliedP1, modelP2: impliedP2, impliedP1, impliedP2, factorCount: 0 };
  ...
  // Só bloqueia quando há compScore (winrate de campeões) — sinal quantitativo real.
  // hasLiveStats sozinho NÃO bloqueia: jogo ao vivo sem dados de forma/comp não tem
  // base para calcular edge e não deve impedir a IA de avaliar.
  const hasStrongSignal = compScore !== null && !isNaN(compScore);
  if (!hasStrongSignal) return { pass: true, direction, score: maxEdge, t1Edge, t2Edge, modelP1, modelP2, impliedP1, impliedP2, factorCount };

  // Com compScore disponível, exige edge real > 3pp para valer a chamada à IA
  const MIN_EDGE = 3.0;
  if (maxEdge >= MIN_EDGE) {
    return { pass: true, direction, score: maxEdge, t1Edge, t2Edge, modelP1, modelP2, impliedP1, impliedP2, factorCount };
  }

  // compScore fraco e edge insuficiente — economiza tokens da IA
  return { pass: false, direction: null, score: maxEdge, t1Edge, t2Edge, modelP1, modelP2, impliedP1, impliedP2, factorCount };
}

...
async function httpJson(url, { ttlMs = 10 * 60 * 1000 } = {}) {
  ...
  if (!r) { try { require('./utils').logSofaNon200(url, null, 'SOFA-TT'); } catch (_) {} return null; }
  if (r.status !== 200) { try { require('./utils').logSofaNon200(url, r.status, 'SOFA-TT'); } catch (_) {} return null; }
...
/**
 * Varre Sofascore dos últimos N dias e retorna matches finished.
 * Usado pra seed histórico do DB (tabela match_results com game='tabletennis').
 *
 * @param {number} days - dias atrás para varrer (default 60)
 * @returns {Promise<Array<{date, homeTeam, awayTeam, winner, score, tournament}>>}
 */
async function fetchHistoricalMatches(days = 60) {
  const out = [];
  ...
  for (let i = 0; i < days; i++) {
    ...
    // Pequeno delay pra não martelar o proxy
    await new Promise(r => setTimeout(r, 100));
  }
  return out;
}

module.exports = { enrichMatch, fetchHistoricalMatches, namesMatch };

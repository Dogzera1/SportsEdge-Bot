...
function _evictIfFull() {
  ...
  // 2026-05-03 FIX: priorizar evicção de COUNTER_KIND (high-cardinality tag-based)
  // antes de GAUGE/TIMING (typically core metrics tipo bankroll_total/process_rss).
  // Antes FIFO simples evictava gauges importantes inseridos no boot ANTES de
  // counters tag-pesados. Agora preserva gauges/timings se possível.
  const evictBatch = Math.max(1, Math.floor(MAX_TOTALS_ENTRIES * 0.01));
  let evicted = 0;
  // Pass 1: COUNTER_KIND only (FIFO ordem de inserção)
  for (const [k, v] of _totals) {
    if (evicted >= evictBatch) break;
    if (v?.kind === COUNTER_KIND) {
      _totals.delete(k);
      _evictionCount++;
      evicted++;
    }
  }
  // Pass 2: se ainda full (poucos counters), evict outros tipos pra liberar
  if (_totals.size > MAX_TOTALS_ENTRIES) {

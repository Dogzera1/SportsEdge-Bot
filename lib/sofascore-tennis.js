...
async function httpJson(url, { ttlMs = 10 * 60 * 1000 } = {}) {
  ...
  if (!r) { try { require('./utils').logSofaNon200(url, null, 'SOFA-TENNIS'); } catch (_) {} return null; }
  if (r.status !== 200) { try { require('./utils').logSofaNon200(url, r.status, 'SOFA-TENNIS'); } catch (_) {} return null; }

...
async function ingestForMatch(db, mr) {
  ...
  try {
    ...
    log('INFO', 'CS-PERMAP-INGEST', `${mr.team1} vs ${mr.team2} (${mr.match_id}): ingested ${maps.length} maps via HLTV${totalProp ? ` (+propagated ${totalProp} cross-rows)` : ''}`);
    return { ok: true, source: 'hltv', maps_count: maps.length, propagated: totalProp };
  } catch (e) {

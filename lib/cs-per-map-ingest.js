...
const { getCsMatchMapResults, getHltvMatchId, namesMatch } = require('./hltv');
...
async function ingestForMatch(db, mr) {
  ...
  if (m) {
    ...
    // Lookup via getHltvMatchId pelo nome + data
    try {
      hltvId = await getHltvMatchId(mr.team1, mr.team2, mr.resolved_at);
    } catch (_) {}

...
function _detectDeadOptInEnvs() {
  ...
  const optInEnvs = [
    // Hour-gates envs removidos 2026-05-14: feature reverted commit f28587f,
    // envs não-consumed em código (4 callsites cleared). Vestígios Railway env.
    // Se reintroduzir hour-gate manual, re-add aqui.
    //
...
function runOverfeaturingAudit(db, opts = {}) {
  ...
  const issues = [];
  // CLAUDE.md P3 status (2026-05-11): "5 disable sources mantidas — cada
  // captura signal diferente". Tolerância expandida: só flag se MAIS de 5
  // dormentes (todas as auto_* sem nenhuma atividade), OR se manual+auto_roi_leak
  // também dormentes (significa nenhum leak guard rodou — sintoma real).
  const dormantList = findings.dormant_disable_sources.dormant || [];
  const activeList = findings.dormant_disable_sources.active || [];
  const criticalSourcesActive = activeList.some(a => a.source === 'auto_roi_leak' || a.source === 'manual');
  if (dormantList.length >= 6 || !criticalSourcesActive) {
    issues.push(`${dormantList.length} disable sources dormentes em ${days}d (critical_active=${criticalSourcesActive})`);
  }

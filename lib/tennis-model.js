...
const { predictTrainedTennis, hasTrainedModel, getTennisRecentMomentum } = require('./tennis-model-trained');

// Isotonic post-hoc calibration (fit via scripts/fit-tennis-model-isotonic.js).
// Kill-switch: TENNIS_ISOTONIC_DISABLED=false força apply (default agora é DISABLED).
// 2026-04-28: DEFAULT FLIPPED pra disabled. Análise de tips ML shadow 30d
// (50 tips, hit 18.5%, ROI -32%) confirmou overshoot persistente:
//   P=12% → yMean=46% (+34pp false)
//   P=25% → yMean=53% (+28pp false)
//   P=35% → yMean=62% (+28pp false)
//   P=52% → yMean=73% (+21pp false)
// Modelo tipa underdogs achando que tem edge alto, mas pModel real é
// muito menor que isotonic-mapped. CLV neutro (+0.3%) confirma que não é
// timing — é pick errado. Re-habilitar só com refit + audit pos-mortem.
const _ISOTONIC_DISABLED_DEFAULT = !/^(0|false|no)$/i.test(String(process.env.TENNIS_ISOTONIC_DISABLED ?? 'true'));
let _isotonicBlocks = null;
let _isotonicWarned = false;
try {
  ...
  if (fs.existsSync(p)) {
    ...
    if (Array.isArray(j.blocks) && j.blocks.length) {
      ...
      // Warn-once se shift médio >0.15 (overshoot detectável)
      const maxShift = j.blocks.reduce((m, b) => {
        const d = Math.abs(b.yMean - (b.pMin + b.pMax) / 2);
        return d > m ? d : m;
      }, 0);
      if (maxShift > 0.15) {
        try {
          const { log } = require('./utils');
          log('WARN', 'TENNIS-ISOTONIC',
            `blocks com shift máx ${maxShift.toFixed(2)}pp — modelo overshooting. ` +
            `Default DISABLED (TENNIS_ISOTONIC_DISABLED=${_ISOTONIC_DISABLED_DEFAULT}). ` +
            `Refit ou ajuste manual antes de habilitar.`);
        } catch (_) {}
      }
    }
  }
...
function _applyIsotonic(p) {
  if (_ISOTONIC_DISABLED_DEFAULT) return p;
...
/**
 * Extrai profundidade da rodada (1=R128 até 7=Final) a partir do nome da liga
 * e/ou payload Sofascore. Return 0 se não detectado.
 *
 * Mapping:
 *   1: R128 / 1st round
 *   2: R64 / 2nd round
 *   3: R32 / 3rd round
 *   4: R16 / 4th round / last 16
 *   5: QF / quarterfinal
 *   6: SF / semifinal
 *   7: F / final (incluindo Grand Final)
 *
 * @param {string} league
 * @param {object} [sofaEnrich] — opcional, pra usar roundInfo do Sofascore
 * @returns {{ depth: number, label: string|null }}
 */
function parseRoundDepth(league, sofaEnrich) {
  // Prioridade 1: Sofascore roundInfo quando disponível
  const sofaRoundName = sofaEnrich?.roundInfo?.name || sofaEnrich?.tournament;
  const src = [sofaRoundName, league].filter(Boolean).join(' ');
  const s = String(src || '').toLowerCase();
  if (!s) return { depth: 0, label: null };

  // Ordem importa: mais específico primeiro
  if (/\b(grand\s*final|grand-final)\b/i.test(s)) return { depth: 7, label: 'GF' };
  if (/\b(quarter[- ]?final|quarterfinal|\bqf\b)/i.test(s)) return { depth: 5, label: 'QF' };
  if (/\b(semi[- ]?final|semifinal|\bsf\b)/i.test(s)) return { depth: 6, label: 'SF' };
  if (/\bfinal\b/i.test(s) && !/quarter|semi|quarterfinal|semifinal/i.test(s)) return { depth: 7, label: 'F' };
  if (/\b(round of 16|\br16\b|last[- ]?16|1\/8|4th round|fourth round)\b/i.test(s)) return { depth: 4, label: 'R16' };
  if (/\b(round of 32|\br32\b|1\/16|3rd round|third round)\b/i.test(s)) return { depth: 3, label: 'R32' };
  if (/\b(round of 64|\br64\b|1\/32|2nd round|second round)\b/i.test(s)) return { depth: 2, label: 'R64' };
  if (/\b(round of 128|\br128\b|1\/64|1st round|first round)\b/i.test(s)) return { depth: 1, label: 'R128' };
  return { depth: 0, label: null };
}

/**
 * Segment gate pra tennis — decisões baseadas em backtest per-(surface × tier × bestOf).
 * Ajusta confidence e min-edge por segmento. Não skip (ITF exclusion já cuida).
 *
 * Policy (backtest 2026-04-18):
 *   Grand Slam Bo5 (todas superfícies)  → confidence ×1.15  (Brier 0.18, strong)
 *   Masters Bo3                          → confidence ×1.05  (Brier 0.21, neutral)
 *   Other/250 Bo3 (non-grass)            → normal            (Brier 0.22, fraco mas ok)
 *   Grass Other Bo3                      → edge bonus +1pp   (Brier 0.224, mais fraco)
 *
 * @returns {{ confidenceMultiplier: number, minEdgeBonus: number, reason: string|null, tier: string, bestOf: number }}
 */
function tennisSegmentGate(league, bestOf, sofaEnrich) {
  if (process.env.TENNIS_SEGMENT_GATE_OFF === 'true') {
    return { confidenceMultiplier: 1, minEdgeBonus: 0, reason: null, tier: null, bestOf: null, round: null };
  }
  const tier = tournamentTier(league);
  const surface = detectSurface(league); // 'saibro' | 'grama' | 'dura'
  const bo = Number(bestOf) || (tier === 'grandslam' ? 5 : 3);
  const round = parseRoundDepth(league, sofaEnrich);

  // Base confidence multiplier por (tier × surface × bestOf)
  let mult = 1;
  let minEdgeBonus = 0;
  const reasons = [];

  if (tier === 'grandslam' && bo === 5) {
    mult *= 1.15;
    reasons.push('Grand Slam Bo5 (backtest Brier ~0.18, strong)');
  } else if (tier === 'masters') {
    mult *= 1.05;
    reasons.push('Masters Bo3 (backtest Brier 0.21)');
  } else if (surface === 'grama' && bo === 3 && tier !== 'grandslam') {
    mult *= 0.95;
    minEdgeBonus = 1;
    reasons.push('Grass non-Slam Bo3 (Brier 0.224, fraco)');
  }

  // Round stakes bonus — QF+ = motivação máxima, jogadores no true skill.
  //   QF (5): +2%
  //   SF (6): +4%
  //   F  (7): +6%
  //   R16 e anteriores: sem bonus (1-4)
  if (round.depth >= 5) {
    const roundBonus = 0.02 * (round.depth - 4); // 0.02 @QF, 0.04 @SF, 0.06 @F
    mult *= (1 + roundBonus);
    reasons.push(`${round.label} stakes +${(roundBonus * 100).toFixed(0)}%`);
  }

  // 2026-04-28: clamp final 0.7-1.25 pra evitar overconfidence runaway se
  // futuros bonuses se acumularem. Slam Bo5 ×1.15 + F ×1.06 = 1.219 (OK).
  // Hipotético novo bonus 5% poderia levar a 1.28 sem clamp.
  const _capped = Math.max(0.70, Math.min(1.25, mult));
  return {
    confidenceMultiplier: +_capped.toFixed(3),
    minEdgeBonus,
    reason: reasons.length ? reasons.join(' | ') : null,
    tier, bestOf: bo, round: round.label, roundDepth: round.depth,
  };
}
...
function computeFatigueIndex(db, playerName, matchTimeMs, league) {
  try {
    ...
    if (daysSinceLast === null) {
      // Histórico existe mas resolved_at todos NaN — provável DB gap no import. Log pra visibility.
      log('WARN', 'TENNIS-MODEL', `fatigue: ${playerName} tem ${rows.length} rows mas resolved_at todos NaN — tratando como no_history`);
      baseFatigue = 0.3; // unknown (mantém compatível, mas agora visível no log)
...
/**
 * Aplica decay multiplicativo no Elo baseado em fatigue index.
 * Penalidade em Elo points subtraída do rating quando fatigue > 0.3 (baseline "unknown").
 * Max penalty: ~80 pts (fatigue=1.0) = ~5% swing em pMatch.
 *
 * @param {number} elo — rating original
 * @param {number} fatigueIndex — 0..1 (de computeFatigueIndex)
 * @returns {number} elo ajustado
 */
function fatigueDecayToElo(elo, fatigueIndex) {
  const excess = Math.max(0, (fatigueIndex || 0) - 0.3);
  const penalty = Math.min(80, excess * 100); // 0.3→0, 1.0→70, cap 80
  return (elo || 1500) - penalty;
}
...
function h2hSubModel(db, player1, player2, surface) {
  try {
    ...
    // Hard gate: n<2 H2H é ruído puro (coin-flip). Bleed em debut/qualifier.
    ...
    // Confidence decay abaixo de 8 matches (não só surface gate)
    const confidence = usedSurface
      ? Math.max(0.4, Math.min(1.0, sampleSize / 8))
      : Math.max(0.3, Math.min(0.7, sampleSize / 8 * 0.7));
...
function getTennisProbability(db, match, odds, enrich, surfaceOverride) {
  ...
  // 2026-04-28: track se decay foi aplicado pra evitar double-count com sub-model.
  let _fatigueDecayApplied = false;
  if (eloResult && eloResult.confidence > 0) {
    // Fatigue decay multiplicativo: jogadores cansados têm Elo efetivo menor.
    // 2026-04-28: ANTES "complementa" sub-model — efeito double-count (decay
    // mutava p1 + sub-model adicional W=0.15). Agora SUBSTITUI: quando decay
    // aplica, fatigueSubModel é skipado.
    // ENV TENNIS_FATIGUE_DECAY_OFF=true desativa decay (volta sub-model only).
    if (process.env.TENNIS_FATIGUE_DECAY_OFF !== 'true') {
      try {
        const matchTs = match.time ? new Date(match.time).getTime() : Date.now();
        const f1 = computeFatigueIndex(db, player1, matchTs, league);
        const f2 = computeFatigueIndex(db, player2, matchTs, league);
        const elo1Adj = fatigueDecayToElo(eloResult.elo1, f1.fatigue);
        const elo2Adj = fatigueDecayToElo(eloResult.elo2, f2.fatigue);
        const pAdj = 1 / (1 + Math.pow(10, (elo2Adj - elo1Adj) / 400));
        const shift = Math.abs(pAdj - eloResult.p1);
        if (shift > 0.005) {
          log('DEBUG', 'TENNIS-MODEL',
            `  └ fatigue decay: elo ${eloResult.elo1}→${Math.round(elo1Adj)} (f=${f1.fatigue.toFixed(2)}) vs ${eloResult.elo2}→${Math.round(elo2Adj)} (f=${f2.fatigue.toFixed(2)}) | pA ${(eloResult.p1*100).toFixed(1)}%→${(pAdj*100).toFixed(1)}%`);
        }
        eloResult.p1 = pAdj;
        eloResult.p2 = 1 - pAdj;
        eloResult._fatigueAdj = { elo1Adj: Math.round(elo1Adj), elo2Adj: Math.round(elo2Adj), fatigue1: +f1.fatigue.toFixed(2), fatigue2: +f2.fatigue.toFixed(2) };
        _fatigueDecayApplied = true;
      } catch (e) {
        log('DEBUG', 'TENNIS-MODEL', `fatigue decay skipped: ${e.message}`);
      }
    }
  ...
  // Nota: Markov point-by-point (priceTennisMatch) NÃO é aplicado como sub-model
  // aqui — bot.js já faz blend 40/60 Markov sobre modelP1 + TB adjustment +
  // ace pricing + live Markov + market scanner (linhas 8715-8900). Adicionar
  // sub-model aqui seria double-count. Ver `project_tennis_markov.md`.

  // ── C) Fatigue ──
  // 2026-04-28: skip se decay já foi aplicado em A) Elo (double-count).
  const matchTimeMs = match.time ? new Date(match.time).getTime() : Date.now();
  const fatigueResult = !_fatigueDecayApplied
    ? fatigueSubModel(db, player1, player2, matchTimeMs, league)
    : null;
  ...
  if (TRAINED_MODE !== 'off' && hasTrainedModel() && eloResult) {
    try {
      ...
      // Momentum via getTennisRecentMomentum (DB match_results); default 0 se sem dados
      const mom1 = getTennisRecentMomentum(db, player1);
      const mom2 = getTennisRecentMomentum(db, player2);
      const winStreakDiff = (mom1?.streak || 0) - (mom2?.streak || 0);
      const wrLast10Diff = (mom1?.wrLast10 != null && mom2?.wrLast10 != null)
        ? (mom1.wrLast10 - mom2.wrLast10) : 0;
  ...
  // 2026-05-01: CLV calibration layer (lit: arxiv 2410.21484). Puxa em direção
  // a closing line. No-op se lib/tennis-clv-calibration.json não existe.
  // Opt-out: CLV_CALIB_DISABLED=true. Override: CLV_BLEND_WEIGHT_TENNIS=0.40.
  try {
    const { applyClvCalib } = require('./clv-calibration');
    const preClv = finalP1;
    finalP1 = applyClvCalib('tennis', finalP1);
    finalP2 = 1 - finalP1;
    if (Math.abs(finalP1 - preClv) > 0.005) {
      log('DEBUG', 'TENNIS-MODEL', `  └ CLV calib: ${(preClv*100).toFixed(1)}% → ${(finalP1*100).toFixed(1)}%`);
    }
  } catch (_) { /* opcional */ }

  // Segment gate: ajusta confidence baseado em (tier × bestOf × surface) do backtest.
  // Grand Slam Bo5 ×1.15, Masters ×1.05, grass non-Slam Bo3 ×0.95.
  const bestOfForGate = /grand slam|\[g\]|wimbledon|us open|roland|australian open/i.test(league) ? 5 : 3;
  const segGate = tennisSegmentGate(league, bestOfForGate);
  if (segGate.confidenceMultiplier !== 1) {
    const preConf = confidence;
    confidence = Math.max(0, Math.min(1, confidence * segGate.confidenceMultiplier));
    log('DEBUG', 'TENNIS-MODEL', `  └ segment gate: conf ${preConf.toFixed(2)} → ${confidence.toFixed(2)} (${segGate.reason})`);
  }

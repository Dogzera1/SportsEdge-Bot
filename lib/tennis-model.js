...
function tournamentTier(league) {
  ...
  if (/challenger/i.test(low)) return 'challenger';
  if (/itf|futures/i.test(low)) return 'itf';
  return 'other';
}
...
function getTennisProbability(db, match, odds, enrich, surfaceOverride) {
  ...
  const finalP1 = finalTotal > 0 ? modelP1 / finalTotal : 0.5;
  const finalP2 = 1 - finalP1;
  ...
  log('DEBUG', 'TENNIS-MODEL', `${player1} vs ${player2} | surface=${surface} | method=${method} | P1=${(finalP1*100).toFixed(1)}% | conf=${confidence.toFixed(2)} | factors=${subModels.length}`);

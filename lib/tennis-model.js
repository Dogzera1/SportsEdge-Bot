 ...
 */
function tennisSegmentGate(league, bestOf) {
  if (process.env.TENNIS_SEGMENT_GATE_OFF === 'true') {
    return { confidenceMultiplier: 1, minEdgeBonus: 0, reason: null, tier: null, bestOf: null };
  }
  const tier = tournamentTier(league);
  const surface = detectSurface(league); // 'saibro' | 'grama' | 'dura'
  const bo = Number(bestOf) || (tier === 'grandslam' ? 5 : 3);

  // Grand Slam Bo5 em todas superfícies — segmento mais forte
  if (tier === 'grandslam' && bo === 5) {
    return {
      confidenceMultiplier: 1.15,
      minEdgeBonus: 0,
      reason: `Grand Slam Bo5 (backtest Brier ~0.18, strong)`,
      tier, bestOf: bo,
    };
  }
  // Masters 1000 — model sólido
  if (tier === 'masters') {
    return {
      confidenceMultiplier: 1.05,
      minEdgeBonus: 0,
      reason: `Masters Bo3 (backtest Brier 0.21)`,
      tier, bestOf: bo,
    };
  }
  // Grass non-Slam Bo3 — segmento mais fraco (~0.224 Brier)
  if (surface === 'grama' && bo === 3 && tier !== 'grandslam') {
    return {
      confidenceMultiplier: 0.95,
      minEdgeBonus: 1,
      reason: `Grass non-Slam Bo3 (backtest Brier 0.224, fraco — exige +1pp edge)`,
      tier, bestOf: bo,
    };
  }
  return { confidenceMultiplier: 1, minEdgeBonus: 0, reason: null, tier, bestOf: bo };
}

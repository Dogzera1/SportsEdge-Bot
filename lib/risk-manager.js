...
function adjustStakeUnits(sport, desiredUnits, snapshot, opts = {}) {
  ...
  // 2026-04-28: usa per-sport tier (lib/sport-unit.js) em vez de banca/100
  // hardcoded. Antes risk-manager calculava com modelo banca-pct-100 enquanto
  // bot.js gravava tip com tier discreto (0.5/0.6/0.8/1.0/1.2/1.5/2/3 R$).
  // Inconsistência fazia stake_reais real divergir do que o cap calculava.
  let unitValue;
  try {
    const { getSportUnitValue } = require('./sport-unit');
    const initialBancaSport = _num(sportRow?.initial_banca) ?? 100;
    unitValue = getSportUnitValue(currentBancaSport || 0, initialBancaSport);
  } catch (_) {
    unitValue = currentBancaSport > 0 ? (currentBancaSport / 100) : null;
  }
  if (!unitValue || unitValue <= 0) return { ok: true, units: units, reason: 'no_bankroll' };

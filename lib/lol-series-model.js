 ...
 *
 * Momentum calibrado:
 *   2026-04-18 (legacy): 0.03 — 662 séries test, LRT χ²=5.39 p<0.05
 *   2026-05-22 (atual):  0.10 — 878 séries test (sample 7542 total, 11× maior),
 *                        best fit 0.15 LRT χ²=96.32 p<0.001. Default 0.10 é
 *                        midpoint conservador (split entre 0.03 legacy e 0.15 best).
 *                        Regime change Spring split + EWC = mais sweeps esperados.
 *   Override per tier via env LOL_MOMENTUM_TIER1 / LOL_MOMENTUM_TIER2 /
 *   LOL_MOMENTUM_OTHER (memory: project_audit_full_2026_05_21 tier-aware).
 */

const LOL_MOMENTUM_DEFAULT = 0.10;
...
/**
 * Inverso de seriesProbFromMap. Dado P de série, devolve o pMap que a geraria.
 *
 * 2026-05-07 (causa-fix UNDER 2.5 leak):
 * Antes assumia momentum=0 (closedFormSeriesProb) na reversão MAS o downstream
 * pricing (totalMapsProb) usava momentum>0 → inconsistência matemática:
 *   - Elo treina em P_series real (captura momentum implícito naturalmente)
 *   - Reversão com momentum=0 → pMap INFLATED (cobre o "gap" de momentum)
 *   - Reconstrução de placares com momentum>0 → DUPLO inflate em P(sweep)
 *   - Resultado: bot tipa UNDER 2.5 com EV inflado, sangra ROI
 *
 * Fix: aceita opts.momentum. Quando >0, usa bisection sobre Monte Carlo
 * (`monteCarloSeries`) — mais lento mas matematicamente consistente. Cache
 * opcional via opts.iterations (default 20000).
 *
 * Backward-compat: sem opts.momentum (ou momentum=0) usa closed-form rápido.
 *
 * @param {number} pSeries
 * @param {number} bestOf
 * @param {object} [opts]
 * @param {number} [opts.momentum=0]     Boost aditivo no pMap do vencedor anterior
 * @param {number} [opts.iterations=20000] MC iters quando momentum>0
 * @returns {number} pMap correspondente.
 */
function mapProbFromSeries(pSeries, bestOf, opts = {}) {
  ...
  // Closed-form rápido quando momentum=0 (preserva comportamento legado)
  if (momentum === 0) {
  ...
  // Monte Carlo bisection — consistente com pricing downstream que usa momentum
  let lo = 0.001, hi = 0.999;
  // Menos iterações no bisection (30 em vez de 50) pra compensar custo MC; precisão
  // ~0.001 já é mais que adequada (pricing tolerance é em centésimos).
  for (let i = 0; i < 30; i++) {
    const mid = (lo + hi) / 2;
    const s = monteCarloSeries(mid, bo, momentum, iters);
    if (s < ps) lo = mid; else hi = mid;
  }
  return (lo + hi) / 2;
}

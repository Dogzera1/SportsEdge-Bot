...
function buildBookDeeplink(bookmaker, ctx = {}) {
  ...
  if (book.includes('pinnacle')) {
    // 2026-05-21: pinnacle.bet.br tem routing distinto de pinnacle.com:
    //   - `www.pinnacle.bet.br/<qualquer path>` faz 301 → `pinnacle.bet.br/`
    //     (path-strip global no www subdomain) → links iam pra homepage
    //   - `/pt/` prefix existe em pinnacle.com mas não em pinnacle.bet.br
    //     (BR já é PT por padrão); apex domain aceita path e renderiza SPA
    //   - Route SPA do Angular bundle é `/event/{id}` (segment matcher), não
    //     `/matchup/{id}` (esse é alias só do pinnacle.com)
    // Fix bug "link de tip vai pro site inicial": apex domain + `/event/`.
    // PINNACLE_REGION=int (ou =com) usa pinnacle.com internacional.
    const region = String(process.env.PINNACLE_REGION || 'br').toLowerCase();
    const useBR = !(region === 'int' || region === 'com');
    if (ctx.matchupId) {
      return useBR
        ? `https://pinnacle.bet.br/event/${encodeURIComponent(ctx.matchupId)}`
        : `https://www.pinnacle.com/pt/matchup/${encodeURIComponent(ctx.matchupId)}`;
    }
    const sportPath = {
    ...
    return useBR
      ? `https://pinnacle.bet.br/${sportPath}`
      : `https://www.pinnacle.com/pt/${sportPath}`;
  }

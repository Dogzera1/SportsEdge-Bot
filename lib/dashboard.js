...
async function runHealthSentinel(serverBase, db) {
  ...
  // 1) MUTEX STALE — runAutoAnalysis travado >5min
  ...
  // 2) POLL SILENT — cada loop esperado checa heartbeat
  const POLL_HEARTBEATS = [
    { id: 'poll_silent_lol', tag: 'AUTO', re: /\[AUTO\] LoL: \d+ partidas/, intervalLive: 300000, intervalIdle: 600000 },
    { id: 'poll_silent_dota', tag: 'AUTO-DOTA', re: /\[AUTO-DOTA\] /, intervalLive: 180000, intervalIdle: 900000 },
    { id: 'poll_silent_cs', tag: 'AUTO-CS', re: /\[AUTO-CS\] /, intervalLive: 180000, intervalIdle: 600000 },
    { id: 'poll_silent_valorant', tag: 'AUTO-VAL', re: /\[AUTO-VAL\] /, intervalLive: 180000, intervalIdle: 600000 },
    { id: 'poll_silent_tennis', tag: 'AUTO-TENNIS', re: /\[AUTO-TENNIS\] /, intervalLive: 600000, intervalIdle: 1800000 },
    { id: 'poll_silent_mma', tag: 'AUTO-MMA', re: /\[AUTO-MMA\] /, intervalLive: 600000, intervalIdle: 1800000 },
    { id: 'poll_silent_darts', tag: 'AUTO-DARTS', re: /\[AUTO-DARTS\] /, intervalLive: 240000, intervalIdle: 1200000 },
    { id: 'poll_silent_snooker', tag: 'AUTO-SNOOKER', re: /\[AUTO-SNOOKER\] /, intervalLive: 240000, intervalIdle: 1200000 },
    { id: 'poll_silent_tt', tag: 'AUTO-TT', re: /\[AUTO-TT\] /, intervalLive: 600000, intervalIdle: 1800000 },
  ];
  for (const hb of POLL_HEARTBEATS) {
    const last = lastLog(hb.re);
    if (!last) {
      out.anomalies.push({
        id: hb.id, severity: 'warning',
        detail: `nenhum log [${hb.tag}] no buffer (~3000 lines)`,
        actionable: true,
      });
      continue;
    }
    const ageMs = now - last.t;
    // Heurística: se passou >3x interval idle, certeza de problema.
    const threshold = hb.intervalIdle * 3;
    if (ageMs > threshold) {
      out.anomalies.push({
        id: hb.id, severity: ageMs > threshold * 2 ? 'critical' : 'warning',
        detail: `[${hb.tag}] sem log há ${Math.round(ageMs / 60000)}min (threshold ${Math.round(threshold / 60000)}min)`,
        at: last.t,
        actionable: true,
      });
    } else {
      out.healthy.push({ id: hb.id, ageMin: Math.round(ageMs / 60000) });
    }
  }
  ...
  // 7) SETTLEMENT PARADO — tips com result=NULL há >24h E match começou há >12h
  if (db) {
    try {
      ...
      for (const s of stale) {
        if (s.n >= 3) {
          out.anomalies.push({
            ...
            detail: `${s.sport}: ${s.n} tips pending há >24h (settlement não pegou)`,
            actionable: false,
          });
        }
      }

...
function extractTips(limit = 60) {
  ...
  for (const e of buffer) {
    ...
    if (/Tip enviada/i.test(t)) {
      sent.push({ t: e.t, bot: e.bot, isLive, kind: 'ML', text: t.replace(/^.*?Tip enviada:?\s*/i, '').trim() });
    } else if (/\[(?:LOL|DOTA|CS|VAL|TENNIS|FOOTBALL)-MARKET-TIP\].*?Admin DM/i.test(t)) {
      // MT promovido (post-847b2a4): "[X-MARKET-TIP] Admin DM: <label> @ <odd> EV <ev>% stake <Nu>"
      sent.push({ t: e.t, bot: e.bot, isLive, kind: 'MT', text: t.replace(/^.*?Admin DM:?\s*/i, '').trim() });
    } else if (/\[MT-RECORD\]/i.test(t)) {
      // recordMarketTipAsRegular sucesso: "[MT-RECORD] tennis/handicapGames/away → tip id=562 stake=2u"
      sent.push({ t: e.t, bot: e.bot, isLive, kind: 'MT-RECORD', text: stripPrefix(t) });
    } else if (/\bSem tip:/i.test(t)) {
      denied.push({ t: e.t, bot: e.bot, isLive, reason: 'sem-edge', text: t.replace(/^.*?Sem tip:\s*/i, '').trim() });
    } else if (/\[MT-MATCH-SKIP\]/i.test(t)) {
      denied.push({ t: e.t, bot: e.bot, isLive, reason: 'mt-dedup', text: stripPrefix(t) });
    } else if (/\[MT-FILTER\]/i.test(t)) {
      denied.push({ t: e.t, bot: e.bot, isLive, reason: 'mt-filter', text: stripPrefix(t) });
    } else if (/\[MT-GUARD\]/i.test(t)) {
      denied.push({ t: e.t, bot: e.bot, isLive, reason: 'mt-leak-guard', text: stripPrefix(t) });
    } else if (/\[FB-MARKET-GATE\]/i.test(t)) {
      denied.push({ t: e.t, bot: e.bot, isLive, reason: 'mt-ev-cap', text: stripPrefix(t) });

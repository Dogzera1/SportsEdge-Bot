...
function predictMapKills(t1Stats, t2Stats, opts = {}) {
  ...
  //
  // 2026-05-15 audit P1 granularidade: hierarchy Bo5 > default pra map3+.
  // Mapas 3-5 em Bo5 (placar fechado 1-2/2-2) têm dinâmica diferente (medos
  // explosivos) vs map3 Bo3 (decisive sem fadiga). Bo5-specific override
  // permite refit empírico per formato. opts.bestOf opcional (caller passa
  // 3/5/7 quando conhecido; ausente cai no FACTOR global como antes).
  if (Number.isFinite(opts.mapIndex)) {
    const idx = opts.mapIndex;
    const defaults = { 1: 1.00, 2: 1.02, 3: 0.97, 4: 1.0, 5: 1.0 };
    // Hierarchy: LOL_KILLS_MAP{N}_BO{bestOf}_FACTOR > LOL_KILLS_MAP{N}_FACTOR > default
    let envParsed = NaN;
    if (Number.isFinite(opts.bestOf) && opts.bestOf >= 3) {
      const boKey = `LOL_KILLS_MAP${idx}_BO${opts.bestOf}_FACTOR`;
      const boVal = parseFloat(process.env[boKey]);
      if (Number.isFinite(boVal)) envParsed = boVal;
    }
    if (!Number.isFinite(envParsed)) {
      const mapKey = `LOL_KILLS_MAP${idx}_FACTOR`;
      envParsed = parseFloat(process.env[mapKey]);
    }
    // 2026-05-06 FIX: parseFloat || defaults transformava 0 em default
    // (operator setando =0 pra "desligar" virava no-op). Usa Number.isFinite
    // pra distinguir "env não setada" de "env=0".
    const factor = Number.isFinite(envParsed) ? envParsed : (defaults[idx] ?? 1.0);
    if (factor !== 1.0) {
...
function predictMapKillsLive(t1Stats, t2Stats, opts = {}, liveState = {}) {
  ...
  return {
    ...
    // 2026-05-06 FIX: integer line handling. Antes, line=27 (raro mas possível
    // em Pinnacle) com cur=11 dava need=16, intNeed=16 → pUnder = CDF(16) =
    // P(X<=16) inclui X=16 → final=27 = PUSH mas tagged como under. Resultado:
    // pOver+pUnder somava 1 mas pPush adicionava → soma >1 (probabilidade
    // inválida). Fix: pra integer line, pUnder = P(final<line) = CDF(intNeed-1),
    // pOver = P(final>line) = 1-CDF(intNeed), pPush = pmf(intNeed).
    pOver(line) {
      ...
      const isIntLine = Math.floor(line) === line;
      const intNeed = Math.floor(need);
      // Integer line: pOver = P(X > intNeed) = 1 - CDF(intNeed)
      // X.5 line:    pOver = P(X >= ceil(need)) = 1 - CDF(intNeed)
      // Ambos casos: 1 - CDF(intNeed). OK.
    ...
    pUnder(line) {
      ...
      const isIntLine = Math.floor(line) === line;
      const intNeed = Math.floor(need);
      // Integer line: pUnder = P(X < intNeed) = CDF(intNeed - 1)
      // X.5 line:    pUnder = P(X <= floor(need)) = CDF(intNeed)
      const cdfArg = isIntLine ? Math.max(0, intNeed - 1) : intNeed;
      return +_poissonCdf(cdfArg, lambdaResid).toFixed(4);
    },
    pPush(line) {
      if (Math.floor(line) !== line) return 0;
      const need = line - currentKillsTotal;

 ...
 */
function _extractTotalKills(html, debug) {
  // Estratégia 0 (gol.gg page-summary precisa): linha
  // `<td>Kills</td><td style='...'>BLUE</td><td style='...'>RED</td>`
  // Pattern observado: <tr><td>Kills</td><td style='border-right-color: var(--blueside-color);'>X</td><td style='border-right-color: var(--redside-color);'>Y</td></tr>
  const killsRowMatch = html.match(/<td[^>]*>\s*Kills\s*<\/td>\s*<td[^>]*blueside[^>]*>\s*(\d+)\s*<\/td>\s*<td[^>]*redside[^>]*>\s*(\d+)\s*<\/td>/i);
  if (killsRowMatch) {
    const k1 = parseInt(killsRowMatch[1], 10);
    const k2 = parseInt(killsRowMatch[2], 10);
    if (Number.isFinite(k1 + k2) && (k1 + k2) > 0 && (k1 + k2) < 200) {
      if (debug) debug.kills_row_match = { blue: k1, red: k2 };
      return { totalKills: k1 + k2, source: 'kills_row' };
    }
    if (debug) debug.kills_row_zero = { blue: k1, red: k2 };
  }
  // Variant: classes ao invés de inline style
  const killsRowVariant = html.match(/<td[^>]*>\s*Kills\s*<\/td>\s*<td[^>]*(?:class=["'][^"']*blue[^"']*["'])?[^>]*>\s*(\d+)\s*<\/td>\s*<td[^>]*(?:class=["'][^"']*red[^"']*["'])?[^>]*>\s*(\d+)\s*<\/td>/i);
  if (killsRowVariant && !killsRowMatch) {
    const k1 = parseInt(killsRowVariant[1], 10);
    const k2 = parseInt(killsRowVariant[2], 10);
    if (Number.isFinite(k1 + k2) && (k1 + k2) > 0 && (k1 + k2) < 200) {
      return { totalKills: k1 + k2, source: 'kills_row_variant' };
    }
  }
  ...
  if (debug) debug.kda_cells_found = kdaCells.length;
  ...
  // Estratégia 2: gol.gg page-summary tem header com "score" tipo
  // "<span class="score">5</span>" — kills per team. Procura tabelas
  // ou divs com class score/result.
  // gol.gg usa elementos `<div class="game_kda">` ou `.score_box`
  const teamScoreCells = [...html.matchAll(/<div[^>]*class=["'][^"']*(?:score_box|game-result|score)[^"']*["'][^>]*>\s*(\d+)\s*<\/div>/gi)];
  if (debug) debug.team_score_cells = teamScoreCells.length;
  if (teamScoreCells.length >= 2) {
    const k1 = parseInt(teamScoreCells[0][1], 10);
    const k2 = parseInt(teamScoreCells[1][1], 10);
    if (Number.isFinite(k1 + k2) && (k1 + k2) > 0 && (k1 + k2) < 200) {
      return { totalKills: k1 + k2, source: 'team_score_div' };
    }
  }

  // Estratégia 3: <span class="score_box ..."> blue + red kills.
  // gol.gg renderiza score grande no topo: <span class="score_box win">25</span>
  const scoreSpans = [...html.matchAll(/<span[^>]*class=["'][^"']*score_box[^"']*["'][^>]*>\s*(\d+)\s*<\/span>/gi)];
  if (debug) debug.score_spans = scoreSpans.length;
  if (scoreSpans.length >= 2) {
    const k1 = parseInt(scoreSpans[0][1], 10);
    const k2 = parseInt(scoreSpans[1][1], 10);
    if (Number.isFinite(k1 + k2) && (k1 + k2) > 0 && (k1 + k2) < 200) {
      return { totalKills: k1 + k2, source: 'score_span' };
    }
  }

  // Estratégia 4: text "Total kills" / "Team Kills" header.
  const totalKillsMatch = html.match(/total\s*kills[:\s<>\/td]*?(\d+)/i);
  if (debug) debug.has_total_kills_text = !!totalKillsMatch;
  if (totalKillsMatch) {
  ...
  // Estratégia 5: scoreboard "X - Y" próximo a "Kills".
  ...
  // Debug: salva sample dos primeiros 1000 chars próximos a "kills" pra inspeção
  if (debug) {
    const killsIdx = html.toLowerCase().indexOf('kills');
    debug.html_snippet_around_kills = killsIdx >= 0 ? html.slice(Math.max(0, killsIdx - 200), killsIdx + 300) : 'no_kills_text';
    // Sample de tabelas — pega primeiras 3 tags <table
    debug.first_tables = [...html.matchAll(/<table[\s\S]{0,500}?<\/tr>/g)].slice(0, 3).map(m => m[0].slice(0, 400));
  }
...
async function _findSeriesIdFromGolggLive(team1, team2, sentAt, leagueHint, debugTrace) {
  ...
  for (const season of ['S16', 'S15']) {
    ...
    if (debugTrace) {
      // Expose ALL tournament names from this season pra debug
      _dbg(`all_${season}`, tournaments.map(t => t.trname || t.name || '').slice(0, 80));
    }
...
async function fetchKillsViaGolgg({ team1, team2, mapIndex, sentAt, db, leagueHint }) {
  ...
  // Step 4: tenta múltiplas URLs do gol.gg pra extrair kills.
  // Order: page-summary (series-level com row "Kills") → page-game (single game) →
  // page-fullstats (player tables).
  const candidateUrls = [
    `https://gol.gg/game/stats/${target.gameId}/page-summary/`,
    `https://gol.gg/game/stats/${target.gameId}/page-game/`,
    `https://gol.gg/game/stats/${target.gameId}/page-fullstats/`,
  ];

  let lastDebug = null, lastBodyLen = 0, lastUrl = '';
  for (const url of candidateUrls) {
    const r2 = await httpGetHtml(url);
    lastUrl = url;
    if (r2.status !== 200) {
      lastDebug = { http_status: r2.status };
      lastBodyLen = 0;
      continue;
    }
    const debug = {};
    const result = _extractTotalKills(r2.body, debug);
    if (result) {
      return { totalKills: result.totalKills, source: `golgg_${result.source}`, gameid: target.gameId, seriesId: series.seriesGameId, foundVia, url };
    }
    lastDebug = debug;
    lastBodyLen = r2.body.length;
  }
  return { reason: 'kills_parse_fail', gameid: target.gameId, body_len: lastBodyLen, last_url: lastUrl, debug: lastDebug };
}

...
async function _findSeriesIdFromGolggLive(team1, team2, sentAt, leagueHint, debugTrace) {
  ...
  for (const season of ['S16', 'S15']) {
    ...
    if (debugTrace) {
      // Expose all tournament names containing 'world', 'esports', or 'cup'
      const interesting = tournaments
        .map(t => t.trname || t.name || '')
        .filter(n => /world|esports|cup|ewc/i.test(n))
        .slice(0, 30);
      _dbg(`interesting_${season}`, interesting);
    }

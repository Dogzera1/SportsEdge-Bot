...
function _applyCorrection(db, cell, scope, decision, opts = {}) {
  ...
  // Negative (last resort) + Positive (kelly mult up pra winners)
    ...
    try { grs.loadFromDb(db); } catch (e) {
      // Defensive — usa cache em memória se loadFromDb falhar. Log DEBUG pra
      // visibility se issue persistir (indicaria GRS table corrompida).
      log('DEBUG', 'READINESS-LEARNER', `grs.loadFromDb falhou (kelly_cut): ${e.message}`);
    }
    ...
    const grs = require('./gates-runtime-state');
    try { grs.loadFromDb(db); } catch (e) {
      log('DEBUG', 'READINESS-LEARNER', `grs.loadFromDb falhou (kelly_up): ${e.message}`);
    }
    const cur = grs.getGateValue(cell.sport, 'kelly_mult') ?? 1.0;
...
function _verifyActiveCorrections(db, snapshot, opts = {}) {
  ...
  for (const c of dueCorrections) {
    ...
    // 2026-05-10 (P0 brain audit): preferir tips marcadas com learned_correction_id=c.id
    // (sample exato pós-aplicação). Fallback sent_at >= applied_at pra tips
    // legacy sem marker (mig 097 adicionou col mas tips antigas têm NULL).
    const isShadowVal = opts.source === 'shadow' ? 1 : 0;
    let nPost = 0;
    try {
      if (c.market) {
        nPost = db.prepare(`
          ...
          WHERE is_shadow = ${isShadowVal}
            ...
            AND result IN ('win','loss')
            AND (learned_correction_id = ? OR (learned_correction_id IS NULL AND sent_at >= ?))
        `).get(c.sport, c.market, c.id, c.applied_at)?.n || 0;
      } else if (c.league) {
        nPost = db.prepare(`
          SELECT COUNT(*) AS n FROM tips
          WHERE is_shadow = ${isShadowVal}
            ...
            AND result IN ('win','loss')
            AND (learned_correction_id = ? OR (learned_correction_id IS NULL AND sent_at >= ?))
        `).get(c.sport, c.league, c.id, c.applied_at)?.n || 0;
      }
    } catch (e) {
      // n_post=0 default mantém comportamento defer. Log DEBUG ajuda diferenciar
      // "no data" vs "query error" caso correções fiquem deferred indefinidamente.
      log('DEBUG', 'READINESS-LEARNER', `n_post query err correction#${c.id} sport=${c.sport}: ${e.message}`);
    }
 ...
 * Mantém status='active' e 'escalated' independente de idade — esses ainda
 * são acionáveis (cron verify pode reverter/escalar).
 *
 * Default: RETENTION_READINESS_CORRECTIONS_DAYS=180 (6 meses).
 *
 * @param {object} db
 * @param {object} [opts] { dryRun, retentionDays }
 * @returns {{ ok, deleted, retainedActive, dryRun, retentionDays }}
 */
function pruneReadinessCorrections(db, opts = {}) {
  const dryRun = !!opts.dryRun;
  const retentionDays = opts.retentionDays
    || parseInt(process.env.RETENTION_READINESS_CORRECTIONS_DAYS || '180', 10);
  if (!Number.isFinite(retentionDays) || retentionDays < 30) {
    return { ok: false, error: 'retentionDays deve ser >=30' };
  }
  try {
    const _countActiveEsc = db.prepare(`
      SELECT COUNT(*) AS n FROM readiness_corrections_log
      WHERE status IN ('active', 'escalated')
    `).get()?.n || 0;
    const _countToPrune = db.prepare(`
      SELECT COUNT(*) AS n FROM readiness_corrections_log
      WHERE status IN ('verified_ok', 'reverted', 'expired')
        AND applied_at < datetime('now', '-' || ? || ' days')
    `).get(retentionDays)?.n || 0;
    let deleted = 0;
    if (!dryRun && _countToPrune > 0) {
      const r = db.prepare(`
        DELETE FROM readiness_corrections_log
        WHERE status IN ('verified_ok', 'reverted', 'expired')
          AND applied_at < datetime('now', '-' || ? || ' days')
      `).run(retentionDays);
      deleted = r.changes;
    ...
    return {
      ...
      wouldDelete: _countToPrune,
      retainedActive: _countActiveEsc,
    };
  } catch (e) {

...
function tennisSxNameLike(a, b) {
  ...
  if (na.length >= 6 && nb.length >= 6) {
    if (na.startsWith(nb.slice(0, 6)) || nb.startsWith(na.slice(0, 6))) {
      // 2026-05-14: tennis first-name collision. "Federico Cina" e "Federico
      // Arnaboldi" colidiam aqui porque startsWith 6-char captura só primeiro
      // nome. Audit prod tip#3011 false-positive matchou row Tunis Quali errada.
      // Quando ambos são "First Last" (têm espaço), exigir também que sobrenome
      // match (último token). Esports team names (sem espaço pós-clean) não
      // afetados — "lng" === "lng" cai em exact match acima.
      const hasSpaceA = String(a || '').includes(' ');
      const hasSpaceB = String(b || '').includes(' ');
      if (!hasSpaceA || !hasSpaceB) return true;
      const lastA = norm(String(a).trim().split(/\s+/).pop() || '');
      const lastB = norm(String(b).trim().split(/\s+/).pop() || '');
...
function tennisSinglePlayerNameMatch(displayA, displayB) {
  ...
  // 2026-05-25: token-reversal 2-token. WTA Chinese names + algumas fontes
  // gravam "Last First" sem vírgula (ex tip "Qinwen Zheng" vs source
  // "Zheng Qinwen"). Set-equality cobre sem false positives ("John Smith"
  // vs "David Smith" não bate pq tokens diferentes).
  const tA = String(displayA || '').trim().split(/\s+/).filter(Boolean).map(t => norm(t));
  const tB = String(displayB || '').trim().split(/\s+/).filter(Boolean).map(t => norm(t));
  if (tA.length === 2 && tB.length === 2 && tA[0].length >= 2 && tA[1].length >= 2) {
    const sA = [...tA].sort();
    const sB = [...tB].sort();
  ...
  const la = tA.length ? tA[tA.length - 1] : '';
  const lb = tB.length ? tB[tB.length - 1] : '';
  return la.length >= 4 && lb.length >= 4 && la === lb;
}

function _splitDoublesPair(name) {
  return String(name || '').split('/').map(s => s.trim()).filter(Boolean);
}

function tennisPairMatchesPlayers(p1, p2, t1, t2) {
  // Singles match (mais comum)
  const singles = (tennisSinglePlayerNameMatch(p1, t1) && tennisSinglePlayerNameMatch(p2, t2))
    || (tennisSinglePlayerNameMatch(p1, t2) && tennisSinglePlayerNameMatch(p2, t1));
  if (singles) return true;

  // 2026-05-13: doubles support. Tips WTA/ATP doubles stored como
  // "P1 / P2 vs P3 / P4". Singles matcher acima trata composite string
  // como um nome só e falha. Aqui split e match partner-by-partner em
  // ambas direções (team_p1 vs team_t1 OU team_p1 vs team_t2).
  const p1Parts = _splitDoublesPair(p1);
  const p2Parts = _splitDoublesPair(p2);
  const t1Parts = _splitDoublesPair(t1);
  const t2Parts = _splitDoublesPair(t2);
  const isDoubles = p1Parts.length >= 2 || p2Parts.length >= 2
    || t1Parts.length >= 2 || t2Parts.length >= 2;
  if (!isDoubles) return false;

  const teamMatch = (pParts, tParts) => {
    if (pParts.length === 0 || pParts.length !== tParts.length) return false;
    const used = new Array(tParts.length).fill(false);
    for (const pn of pParts) {
      let found = false;
      for (let i = 0; i < tParts.length; i++) {
        if (!used[i] && tennisSinglePlayerNameMatch(pn, tParts[i])) {
          used[i] = true;
          found = true;
          break;
        }
  ...
  return (teamMatch(p1Parts, t1Parts) && teamMatch(p2Parts, t2Parts))
      || (teamMatch(p1Parts, t2Parts) && teamMatch(p2Parts, t1Parts));
}

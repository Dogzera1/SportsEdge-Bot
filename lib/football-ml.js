...
function calcPoisson(homeAttack, homeDefense, awayAttack, awayDefense) {
  ...
  // Ex: home marca 2.5/j, away concede 0.5/j → lambdaHome = sqrt(2.5×0.5) ≈ 1.12
  //     vs aritmética: (2.5+0.5)/2 = 1.50 — superestima contra defesa fechada.
  const lambdaHome = Math.max(0.3, Math.min(4, Math.sqrt(homeAttack * awayDefense)));
  const lambdaAway = Math.max(0.3, Math.min(4, Math.sqrt(awayAttack * homeDefense)));
...
function calcFootballScore(home, away, h2h, odds, meta = {}) {
  ...
  // Deslocamento máximo ±15pp em relação ao mercado, mais home advantage da liga

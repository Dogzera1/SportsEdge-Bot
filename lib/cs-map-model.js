...
function predictCsMapWinner({ liveStats, seriesScore, baselineP = 0.5, team1Name, team1IsCT }) {
  ...
  // 2026-04-28: distingue draft-only de no-data. Draft has comps mas sem live=true
  // (mid-warmup ou pre-game). Reduz confidence + log explícito pra observabilidade.
  if (!liveStats.live) {
    const hasDraft = liveStats.hasComps || (Array.isArray(liveStats.players) && liveStats.players.length);
    return {
      p: baselineP,
      confidence: hasDraft ? 0.15 : 0.10,
      factors,
      reason: hasDraft ? 'draft-only (game starting)' : 'sem live stats'
    };
  }
  ...
  // 2026-04-28: quando t1IsCT desconhecido, retornar baseline neutro em vez de
  // assumir t1 lidera via Math.max/Math.min. Antes viesava pT1>0.5 sempre,
  // produzindo edge falso quando team1 estava perdendo.
  if (t1IsCT == null) {
    return { p: 0.5, confidence: 0.2, reason: 't1IsCT_unknown', factors: [] };
  }

  const t1Rounds = t1IsCT === true ? scoreCT : scoreT;
  const t2Rounds = t1IsCT === true ? scoreT : scoreCT;
  const roundsPlayed = t1Rounds + t2Rounds;
  const scoreDiff = t1Rounds - t2Rounds;

  // 2026-05-07 (audit P2): mapa decidido (alguém atingiu threshold de win) — short-circuit.
  // Antes mpShift escalava +18pp @12, +21pp @13, +24pp @14 sem cap → over confident
  // em estado terminal. CS2 MR12 = first to 13 (OT first to 16). Detecta decisão
  // e retorna probabilidade definitiva (cap em 0.99/0.01 evita NaN/log issues downstream).
  // OT casos raros (rounds 24-28); 16 é threshold OT win. 13 é regulation.
  const t1Won = t1Rounds >= 13 && t1Rounds > t2Rounds && (t1Rounds === 13 || (t1Rounds >= 16 && t1Rounds - t2Rounds >= 2));
  const t2Won = t2Rounds >= 13 && t2Rounds > t1Rounds && (t2Rounds === 13 || (t2Rounds >= 16 && t2Rounds - t1Rounds >= 2));
  if (t1Won || t2Won) {
    factors.push({ name: 'mapDecided', delta: t1Won ? 0.49 : -0.49, value: `${t1Rounds}-${t2Rounds}` });
    return { p: t1Won ? 0.99 : 0.01, confidence: 0.99, factors, reason: 'map_decided' };
  }
  ...
  // 2026-05-07: cap em +18pp/-18pp. Antes escalava com (rounds-12)*0.03 sem teto
  // (state inconsistente: t1Rounds=14 mas mapa não decidido seria detectado acima
  // como decidido — só sobra o caso clean t1Rounds=12 < threshold em regulation).
  let mpShift = 0;
  if (t1Rounds >= 12 && t2Rounds < 12) {
    mpShift = 0.18; // cap fixo — match point sem escalar
  ...
  // 3.5) Bomb planted — round corrente. T plantou: ~62% pra fechar round
  //      (HLTV 2024 empirical). Effect SHIFT pequeno (1-2% no map level)
  //      porque é só 1 round em 24+. Útil pra micro-tips quando bomb live.
  let bombShift = 0;
  if (liveStats.bombPlanted === true) {
    // T plantou (estatística simétrica, T tem advantage post-plant)
    const bombT1 = t1IsCT === false ? 0.012 : t1IsCT === true ? -0.012 : 0;
    bombShift = bombT1;
    if (bombShift) factors.push({
      name: 'bombPlanted',
      delta: +bombShift.toFixed(3),
      value: t1IsCT === false ? 'team1 plantou' : 'team2 plantou',
    });
  }
  ...
  let pT1 = 0.5 + scoreShift + mpShift + sideShift + momentumShift + baselineShift + economyShift + bombShift;
  ...
  // Ajusta pra pickTeam — usa name match flexível (norm + substring).
  // 2026-05-03 FIX: antes strict toLowerCase divergia em "Team Liquid" vs "Liquid"
  // → flip incorreto. Usa _teamMatches igual lol/dota-map-model.
  let p = pT1;
  if (team1Name && liveStats.pickTeam) {
    const _normTeam = s => String(s || '').toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, '').replace(/[^a-z0-9]/g, '');
    const na = _normTeam(team1Name), nb = _normTeam(liveStats.pickTeam);
    const matches = na && nb && (na === nb || (na.length >= 3 && (na.includes(nb) || nb.includes(na))));
    if (!matches) p = 1 - pT1;
  }

...
function getPlayerInjuryRisk(db, playerName, opts = {}) {
  ...
  // 2026-04-28: clamp + bind parametrizado pra SQL safety + defense in depth.
  // opts vem de callers internos mas valores podem ser configurados via env;
  // template literal direto era SQL-injection latente.
  const _clampDays = (v, def, max = 3650) => {
    const n = parseInt(v, 10);
    return (Number.isFinite(n) && n > 0 && n <= max) ? n : def;
  };
  const lookbackDays = _clampDays(opts.lookbackDays, 365);
  const recentDays = _clampDays(opts.recentDays, 30);

...
function preMatchEvBonus(sport, isLive) {
  ...
  // 1. Per-sport env (manual urgente)
  const perSport = process.env[`${S}_PRE_MATCH_EV_BONUS`];
  if (perSport != null && perSport !== '') {
    const v = parseFloat(perSport);
    if (Number.isFinite(v) && v >= 0) return v;
  }
  // 2. DB auto-tune (lazy require pra evitar circular)
  try {
    const { getGateValue, GATE_KEYS } = require('./gates-runtime-state');
    const dbVal = getGateValue(S, GATE_KEYS.PRE_MATCH_EV_BONUS);
    if (dbVal != null && dbVal >= 0) return dbVal;
  } catch (_) { /* gates-runtime-state não carregado ainda */ }
  // 3. Global env

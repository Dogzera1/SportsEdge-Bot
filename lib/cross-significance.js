...
// 2026-05-22 Fase 3: tier classifiers football/basket/mma. Alinhado a memory
// project_audit_full_2026_05_21 + lib/league-tier numeric (1=top, 2=mid, 3=other).
// CSA usa strings (slam/masters/tier1/etc) pra legibilidade output.
function _classifyFootballTier(eventName, league) {
  const s = String(eventName || league || '').toLowerCase();
  // Top 5: Premier League, La Liga, Bundesliga, Serie A, Ligue 1
  if (/premier league|la liga|laliga|primera division|bundesliga|serie a|ligue 1|champions league|europa league|conference league/.test(s)) return 'top5';
  // Continental: UEFA, Copa Libertadores, Sudamericana, CONCACAF, AFCON
  if (/uefa|libertadores|sudamericana|concacaf|afcon|africa cup|copa america|world cup|euro 2/.test(s)) return 'continental';
  // BR: Série A, Série B, Série C (relevância)
  if (/serie b|serie c|brasileir|copa do brasil|brazil cup/.test(s)) return 'br';
  // Mid: Eredivisie, Primeira Liga, Liga MX, etc (4th tier UEFA)
  if (/eredivisie|primeira liga|liga mx|saudi|mls|j[1-3]|k[1-3]|a-league|kazakh|ekstraklasa/.test(s)) return 'mid';
  return 'other';
}

function _classifyBasketTier(eventName, league) {
  const s = String(eventName || league || '').toLowerCase();
  if (/\bnba\b|playoffs.*nba|finals.*nba/.test(s)) return 'nba';
  if (/euroleague|eurocup|champions league.*basket/.test(s)) return 'euroleague';
  if (/nbb|liga.*acb|liga nacional|cbb|bsl|abl|brasil|argentina/.test(s)) return 'regional';
  return 'other';
}

function _classifyMmaTier(eventName, league) {
  const s = String(eventName || league || '').toLowerCase();
  if (/\bufc\b/.test(s)) return 'ufc';
  if (/bellator|pfl/.test(s)) return 'bellator_pfl';
  if (/one championship|rizin|ksw|cage warriors/.test(s)) return 'regional';
  return 'other';
}

function classifyTier(sport, eventName, league) {
  if (sport === 'tennis') return _classifyTennisTier(eventName || league);
  if (['lol', 'cs', 'cs2', 'dota2', 'valorant'].includes(sport)) return _classifyEsportsTier(eventName, league);
  if (sport === 'football') return _classifyFootballTier(eventName, league);
  if (sport === 'basket') return _classifyBasketTier(eventName, league);
  if (sport === 'mma') return _classifyMmaTier(eventName, league);
  return null;
}

const SPORTS_WITH_TIER = new Set(['tennis', 'lol', 'cs', 'cs2', 'dota2', 'valorant', 'football', 'basket', 'mma']);
...
function realBucketStats(tips) {
  ...
  for (const t of tips) {
    ...
    if (t.clv_odds && t.odds && t.odds > 1) {
      // 2026-05-29 audit: era clv_odds/odds (sinal INVERTIDO) → avg_clv real saía
      // com sinal trocado, contradizendo shadowBucketStats + hg-neg-readiness:109 +
      // market-tips-shadow. CLV+ = bet odds > close = bateu o fechamento (bom).
 ...
 * @param {object} db
 * @param {object} [opts]
 * @param {string[]} [opts.sports=<SPORTS_WITH_TIER>] sports a analisar
...
function runCrossSignificance(db, opts = {}) {
  // 2026-05-24 (audit roi-analyst): expand default sports de ['tennis','lol']
  // pra TODOS SPORTS_WITH_TIER. minN=20 filter naturalmente exclui sports sem
  // sample. Cross-sport EDGE/LEAK visibility era cego pra cs/dota2/valorant/
  // mma/football/basket em prod CSA hardcoded restritivo.
  const cfg = {
    sports: opts.sports || Array.from(SPORTS_WITH_TIER),
    days: opts.days || 30,

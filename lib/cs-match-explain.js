...
function buildCsExplainPrompt({ pred, teams, fairOdds, edge }) {
  ...
  lines.push('Regras: cada campo 1-2 frases; NÃO invente estatísticas que não estão nos dados (sem winrate por mapa, sem rating de jogador específico); o verdict se apoia no Elo/odd justa (não invente probabilidade); NÃO sugira quanto apostar nem diga "aposte".');
  return lines.join('\n');
}

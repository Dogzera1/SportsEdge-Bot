...
function logShadowTip(db, args) {
  try {
    ...
    // Dedup por (match_key, market, side) — IGNORA line. Linhas correlacionadas
    // (Over 19.5, 20.5, 21.5) do mesmo (market, side) são tratadas como
    // mesma "aposta". Mantém a com maior p_model (linha limite).
    // Janela 24h pra cobrir multiple cycles durante dias de torneio.
    const existing = db.prepare(`
      SELECT id, odd, line, p_model, ev_pct FROM market_tips_shadow
      WHERE match_key = ? AND market = ? AND side IS ?
        AND created_at >= datetime('now', '-24 hours')
        AND result IS NULL
      ORDER BY p_model DESC
      LIMIT 1
    `).get(matchKey, tip.market, tip.side ?? null);
    if (existing) {
  ...
  for (const t of pending) {
    try {
      ...
      if (needsMapScore) {
        ...
        const parseable = candidates.find(c => parseTennisScore(c.final_score) != null);
        if (parseable) mr = parseable;
      }

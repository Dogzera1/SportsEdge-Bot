...
function settleShadowTips(db, opts = {}) {
  ...
  try {
    const cleanupTx = db.transaction(() => {
      ...
      const v3 = db.prepare(`
        ...
        SET result = 'void', settled_at = datetime('now'), profit_units = 0
        WHERE result IS NULL
          AND sport NOT IN ('tennis', 'lol', 'cs2', 'dota2', 'valorant', 'football')
  ...
  const ZOMBIE_THRESHOLDS_H = {
    ...
    cs2: parseInt(process.env.MT_SHADOW_ZOMBIE_H_CS2 || '12', 10),
  ...
  for (const t of pending) {
    try {
      ...
      const gameMap = { lol: ['lol'], dota2: ['dota2'], cs2: ['cs','cs2'], valorant: ['valorant'], tennis: ['tennis'], football: ['football'], basket: ['basket'] };
      ...
      } else if (['lol','dota2','cs2','valorant','basket'].includes(t.sport)) {

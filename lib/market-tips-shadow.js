...
function logShadowTip(db, args) {
  try {
    ...
    // Plumb incremental: caller scanner passa tip.modelPRaw quando capture raw.
    // Sem raw, fallback para p_model (helper documenta caveat).
    if (_hasModelPRawCol) {
      const pRaw = Number(tip.modelPRaw);

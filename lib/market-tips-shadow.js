...
function settleShadowTips(db) {
  ...
  try {
    const cleanupTx = db.transaction(() => {
      ...
      `).run();
      if (v1.changes > 0) log('INFO', 'MT-SHADOW', `voided ${v1.changes} ghost rows (unhandled markets >14d)`);
      ...
      if (filtered.length) {
        ...
        if (!within.length) {
          ...
          if (_isZombie(t) && hasParseable) {
            ...
            log('INFO', 'MT-SHADOW',
              `${t.sport} ${t.team1} vs ${t.team2}: all candidates resolved >${Math.round(guardMs/60000)}min before tip created_at — skip (evento anterior do par)`);
            skipped++;
            continue;
          }
        } else {

  ...
  for (const t of pending) {
    try {
      ...
      if (t.market === 'handicap' && t.sport === 'basket') {
        ...
        // 2026-05-21: handicap_rounds_mapN + total_rounds_mapN — per-map round score
        // NÃO disponível em match_results.final_score (apenas série Bo3 score).
        // TODO próxima ingestion: HLTV scoreboard tem rounds per-map. Adicionar
        // /admin/sync-cs-per-map-rounds populando match_results.result_meta_json
        // (mig 122). Por enquanto voida após 3d com reason explícito (vs 7d cleanup).
        if (new Date(t.created_at).getTime() < Date.now() - 3 * 24 * 3600 * 1000) {
          result = 'void';
          log('INFO', 'MT-SHADOW', `data_unavailable per-map rounds: ${t.market}/${t.side} ${t.team1} vs ${t.team2} (>3d, sem HLTV ingestion)`);
        } else { skipped++; continue; }
      } else if (

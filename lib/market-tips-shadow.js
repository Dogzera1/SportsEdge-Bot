...
function settleShadowTips(db, opts = {}) {
  ...
  try {
    const cleanupTx = db.transaction(() => {
      ...
      //
      // BUG FIX 2026-05-09: tennis EXCLUÍDO do cleanup #3. SQL usa strict eq
      // (`lower(team1)=lower(team1)`) sem fallback fuzzy lastname. Quando
      // Sackmann grava "Lajal M" vs Pinnacle "Mark Lajal", strict eq fail →
      // NOT EXISTS=true → cleanup voida tip ANTES do settle path (que tem
      // fallback _lastName LIKE robusto) ter chance. /admin/mt-shadow-audit
      // 30d retornou 122/500 mismatches tennis (24.4%) padrão "stored=void
      // expected=loss/win". Tennis tips órfãs reais já são cobertas pelo
      // zombie void no settle path (ZOMBIE_THRESHOLDS_H.tennis=36h via
      // _zombieVoid('no_match_results')) — cleanup #3 era duplicação que
      // matava tips legítimas que settle teria liquidado via fuzzy.
      const v3 = db.prepare(`
        UPDATE market_tips_shadow
        SET result = 'void', settled_at = datetime('now'), profit_units = 0
        WHERE result IS NULL
      ...
      `).run();
      if (v3.changes > 0) log('INFO', 'MT-SHADOW', `voided ${v3.changes} non-tennis rows (no match_result found >14d — sync gap)`);
    });
    cleanupTx();
  } catch (e) {

    ...
    try {
      ...
      if (filtered.length) {
        ...
        const within = filtered.filter(c => {
          ...
          return Number.isFinite(cTs) && (cTs >= tipTs - THIRTY_MIN_MS);
        });
        if (!within.length) {
          log('INFO', 'MT-SHADOW',
            `${t.sport} ${t.team1} vs ${t.team2}: all candidates resolved >30min before tip created_at — skip (evento anterior do par)`);
          skipped++;
          continue;
        }
        filtered = within;

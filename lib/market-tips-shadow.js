...
function settleShadowTips(db, opts = {}) {
  ...
  for (const t of pending) {
    try {
      ...
      // 2026-05-03 FIX: envelope shadow UPDATE + close_odd fallback + propagator
      // numa única transação. Antes UPDATE shadow commitava, propagator rodava FORA
      // da tx; se propagator throw, shadow stava settled mas tip real pending eternamente
      // (próximo cycle skipava por result IS NOT NULL no shadow). Tx única: ou tudo
      // commita ou tudo reverte → mantém estado consistente shadow ↔ tip real.
      try {
        const settleTx = db.transaction(() => {
          ...
          // Propaga resultado pra tips "regular" se foi promovido. Erro aqui aborta
          // a tx inteira → shadow não settla → próximo cycle re-tenta. Erros de
          // schema drift agora visíveis no log WARN do catch externo.
        ...
        log('WARN', 'MT-SHADOW', `settle tx err id=${t.id} sport=${t.sport} market=${t.market}: ${e.message}`);

...
function getShadowStats(db, opts = {}) {
  ...
  const leagueSelectClause = needLeagueInSelect && !groupByLeague ? ', league' : '';
  const leagueFilter = needLeagueInSelect ? `AND league IS NOT NULL AND TRIM(league) != ''` : '';
  const finalGroupClause = groupClause + (groupByTier && !groupByLeague ? ', league' : '');
  const rows = db.prepare(`
    SELECT sport, market${extraSelect}${leagueSelectClause},
      COUNT(*) AS n,

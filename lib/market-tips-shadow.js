...
function logShadowTip(db, args) {
  try {
    ...
    // ser redundante pelo match-level acima mas mantém safety net.
  ...
  for (const t of pending) {
    try {
      ...
      if (tipLeagueN) {
        ...
        } else if (t.sport === 'tennis' && candidates.every(c => c.league && _normLeague(c.league))) {
          // Tennis-only strict: nenhum candidate matching league + todos têm league preenchida = match errado
          skipped++;
          continue;
        }
      }

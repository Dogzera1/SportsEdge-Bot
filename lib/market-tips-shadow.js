...
function settleShadowTips(db) {
  ...
  for (const t of pending) {
    try {
      ...
      // ── GUARDRAIL UNIVERSAL: candidate.resolved_at deve ser ≥ tip.created_at - 6h ──
      // Logica: match real associado à tip resolve DEPOIS de tip ser criada
      // (pre-game) ou no máximo 6h antes (cobre tips live placed mid-match,
      // onde match começou antes do tip mas resolve depois). Qualquer candidate
      // resolvido bem antes da criação é o mesmo par jogando outro evento
      // (ATP semana anterior, esports back-to-back days, etc).
      //
      // BUG FIX 2026-04-27 (Etcheverry×Fils Madrid R16 settled R$+1.16 em 29min):
      // antes só tennis tinha 5-day check; outros sports caíam no SQL window
      // (-24h pra esports, -48h pra football) e podiam pegar partida do dia
      // anterior do mesmo par. 6h slack > 5d cobertura, evita match-mismatch
      // cross-sport. Pra tip cuja partida ainda não rolou: candidate ainda
      // não existe → skipped até match real ser ingerido.
      if (filtered.length) {
        const tipTs = new Date(t.created_at).getTime();
        const SIX_HOURS_MS = 6 * 60 * 60 * 1000;
        const within = filtered.filter(c => {
          const cTs = new Date(c.resolved_at).getTime();
          return Number.isFinite(cTs) && (cTs >= tipTs - SIX_HOURS_MS);
        });
        if (!within.length) {
          log('INFO', 'MT-SHADOW',
            `${t.sport} ${t.team1} vs ${t.team2}: all candidates resolved >6h before tip — skip (par jogou evento anterior)`);
          skipped++;
          continue;
        }
        filtered = within;

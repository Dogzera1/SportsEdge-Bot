    ...
    try {
      ...
      if (tipLeagueN) {
        ...
        if (leagueMatches.length) {
          ...
          if (_isZombie(t)) { _zombieVoid(t, 'league_no_overlap'); continue; }
          skipped++;
          continue;
        }
      }
      ...
      if (filtered.length) {
        ...
        if (!within.length) {
          // Caso típico: tip emitida pra R2/R3 mas match_results só tem R1 (R2
          // ainda não foi sincado). Tip stuck pending até R2 ser ingerido — se
          // nunca for, fica zombie. Void inline após threshold.
          if (_isZombie(t)) { _zombieVoid(t, 'all_resolved_before_tip'); continue; }
          log('INFO', 'MT-SHADOW',

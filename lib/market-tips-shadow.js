  ...
  for (const t of pending) {
    try {
      ...
      if (filtered.length) {
        ...
        // resolveram <30min antes da tip. 5min é o sweet spot empírico.
        const FIVE_MIN_MS = 5 * 60 * 1000;
        const within = filtered.filter(c => {
          const cTs = new Date(c.resolved_at).getTime();
          return Number.isFinite(cTs) && (cTs >= tipTs - FIVE_MIN_MS);
        });
        if (!within.length) {
          log('INFO', 'MT-SHADOW',
            `${t.sport} ${t.team1} vs ${t.team2}: all candidates resolved >5min before tip created_at — skip (evento anterior do par)`);
          skipped++;
          continue;
        }
        filtered = within;
      ...
      if (t.sport === 'tennis') {
        ...
        // SETTLE_TRACE_V4: dump exato dos valores feeded ao orientation check
        log('INFO', 'SETTLE_TRACE_V4',
          `tip#${t.id} mr.team1='${mr.team1}' shadow.team1='${t.team1}' mrT1Ln='${mrT1Ln}' t1Ln='${t1Ln}' eq=${mrT1Ln === t1Ln}`);
      ...
      if (t.market === 'handicapGames') {
        ...
        // Swap só se mr.team1 != shadow.team1 (orientação inversa). NÃO swap baseado
        // em quem ganhou — esses são conceitos independentes.
        ...
        // SETTLE_TRACE_V3: assinatura pra confirmar deploy. Se logs mostrarem
        // essa string, código novo está ativo. Pode remover depois.
        log('INFO', 'SETTLE_TRACE_V3',
          `tip#${t.id} ${t.team1} vs ${t.team2} side=${t.side} line=${t.line} mrT1IsShadowT1=${mrT1IsShadowT1} ` +
          `gamesT1=${gamesT1} gamesT2=${gamesT2} margin=${margin} covers=${covers} → ${result}`);
        ...
        if (t.sport === 'tennis') {
          ...
          if (!mrT1IsShadowT1) { [team1Sets, team2Sets] = [team2Sets, team1Sets]; }
        } else {

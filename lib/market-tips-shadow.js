...
function getShadowStats(db, opts = {}) {
  ...
  // 2026-05-06 FIX: realOnly=true filtra pra apenas tips com counterpart
  // dispatched real em `tips` table (is_shadow=0). Usa INNER JOIN. Importante
  // pra auto_clv_leak guard: shadow research pode estar negative mesmo quando
  // tips reais são positive. Default true via env (MT_LEAK_REAL_ONLY).
  // Caveat: se sport não tem MT promoted, INNER JOIN volta vazio → guard
  // desliga (intencional — não devia bloquear research-only).
  ...
  const fromClause = realOnly
    ? `market_tips_shadow mts INNER JOIN tips t ON t.match_id = mts.match_key

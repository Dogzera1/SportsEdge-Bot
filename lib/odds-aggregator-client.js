...
async function enrichMatches(matches) {
  ...
  let enriched1x2 = 0, enrichedOu = 0;
  for (const m of matches) {
    ...
    if (brOu.length) {
      if (!m.odds.ou25) {
        // Inicializa ou25 com primeiro book BR (h/d/a do _allOdds não ajuda aqui)
        const first = brOu[0];
  ...
  return { enriched: enriched1x2, enrichedOu, totalJogos: jogos.length };
}

...
const _cache = { ts: 0, data: [], lastErrorAt: 0, lastErrorMsg: null, errorCount: 0 };

// 2026-05-15 audit P1: exponential backoff per Supabase outage (vs fixed 60s gate).
// Antes: outage longa = 1 query/min. Agora ramps 60s → 5min → 30min cap.
function _aggBackoffMs() {
  const n = Math.max(0, _cache.errorCount || 0);
  // 60s, 2min, 5min, 10min, 20min, 30min cap
  const ramp = [60_000, 120_000, 300_000, 600_000, 1_200_000, 1_800_000];
  return ramp[Math.min(n, ramp.length - 1)];
}

// 2026-05-14: dedup slug typos antes de criar match_id agg_. Sem normalize,
// "valhecano" vs "vallecano" geravam 2 tips distintas no mesmo match (settle path
// não encontrava match_result + dashboard duplicava). Aliases hand-curados
// baseados em variantes vistas em prod (PT-BR scrapers vs nomes oficiais).
const _SLUG_NORMALIZATIONS = [
  // Rayo Vallecano: "valhecano" (PT-BR typo LH) → "vallecano" (LL oficial)
  [/\brayo-valhecano\b/g, 'rayo-vallecano'],
  [/\bvalhecano\b/g, 'vallecano'],
  // Espanhol/Espanyol
  [/\bespanhol\b/g, 'espanyol'],
  // Athletic Bilbao
  [/\bathletic-club\b/g, 'athletic-bilbao'],
  // Atletico Madrid
  [/\batletico-de-madrid\b/g, 'atletico-madrid'],
  // Manchester United
  [/\bmanchester-utd\b/g, 'manchester-united'],
  // Brighton
  [/\bbrighton-hove-united\b/g, 'brighton'],
  [/\bbrighton-hove-albion\b/g, 'brighton'],
  // PSG
  [/\bparis-saint-germain\b/g, 'parissg'],
  [/\bparis-sg\b/g, 'parissg'],
];

function _normalizeAggSlug(slug) {
  let s = String(slug || '').toLowerCase();
  for (const [re, rep] of _SLUG_NORMALIZATIONS) s = s.replace(re, rep);
  return s;
}
...
// Service-role key — necessária pra ler tabelas com RLS using(false) (ex:
// snapshots_debug que tem policy 'apenas_service'). Quando setado, endpoints
// admin que precisam de leitura privilegiada usam essa key. Anon key continua
// default pra views públicas (vw_jogos_publicos).
//
// Setup Railway: SUPABASE_SERVICE_KEY=eyJ...service_role... (do Supabase
...
function _httpGetJson(targetUrl, headers, timeoutMs = 12000) {
  return new Promise((resolve, reject) => {
    ...
    req.on('error', reject);
    // 2026-05-15 audit P0: req.setTimeout wirado de verdade. Antes `opts.timeout`
    // só cobria agent socket connect timeout (não response timeout) — hung
    // Supabase response (proxy stall, DNS hang) bloqueava promise indefinido.
...
async function fetchUpcomingJogos({ daysAhead = 14, force = false } = {}) {
  ...
  // Throttle exponencial pra erros (60s → 30min cap conforme errorCount streak).
  if (!force && _cache.lastErrorAt && (now - _cache.lastErrorAt) < _aggBackoffMs()) return null;
  ...
  try {
    ...
    if (Array.isArray(arr)) {
      ...
      _cache.errorCount = 0; // reset streak após sucesso
      return arr;
    }
    return null;
  } catch (e) {
    ...
    _cache.errorCount = (_cache.errorCount || 0) + 1; // streak counter
    return null;
  }
}
...
function _parseMl(mercados) {
  // 2026-06-10: + `winner` — chave que os scrapers de TENNIS publicam
  // (betnacional-tennis/kto-tennis gravam mercados.winner {1,2}). Sem ela,
  // enrichTennisMatches sempre retornava enriched=0 (brMl vazio) → _allOdds
  // nunca nascia → ramo tennis do soft-vs-sharp/super-odd era morto-nato.
  // Última na cadeia = zero mudança pra quem já parseava.
  const m = mercados?.ml || mercados?.moneyline || mercados?.['1x2'] || mercados?.h2h || mercados?.winner;
  if (!m) return null;
...
// =====================================================================
// 2026-05-08: Parsers MT (multi-line) — alimenta football MT scanner com odds BR.
// =====================================================================

/**
 * Parse Over/Under multi-line. Aceita:
 *   { 'over-under-multi': { '1.5': {over,under}, '2.5': {...}, ... } }   ← scraper KTO 2026-05-08+
 *   { totals: [{ linha:1.5, over, under }, { linha:2.5, ... }] }
 *   { totals: { '1.5': {over,under}, '2.5': {...} } }
 * Retorna [{ point: 1.5, over: 1.30, under: 3.70 }, ...]
 */
function _parseOuMulti(mercados) {
  if (!mercados || typeof mercados !== 'object') return [];
  const out = [];
  // Format 1: 'over-under-multi' nested (KTO 2026-05-08)
  const multi = mercados['over-under-multi'] || mercados.overUnderMulti || mercados.totals_multi;
  if (multi && typeof multi === 'object' && !Array.isArray(multi)) {
    for (const [lnK, v] of Object.entries(multi)) {
      if (!v || typeof v !== 'object') continue;
      const ln = parseFloat(String(lnK).replace(',', '.').replace('linha_', ''));
      const ov = parseFloat(v.over ?? v.acima ?? v.mais);
      const un = parseFloat(v.under ?? v.abaixo ?? v.menos);
      if (Number.isFinite(ln) && Number.isFinite(ov) && Number.isFinite(un)) {
        out.push({ point: ln, over: ov, under: un });
      }
    }
    if (out.length) return out;
  }
  // Format 2: array com {linha, over, under}
  const t = mercados.totals;
  if (Array.isArray(t)) {
    for (const it of t) {
      const ln = parseFloat(it.linha ?? it.line ?? it.point);
      const ov = parseFloat(it.over ?? it.acima);
      const un = parseFloat(it.under ?? it.abaixo);
      if (Number.isFinite(ln) && Number.isFinite(ov) && Number.isFinite(un)) {
        out.push({ point: ln, over: ov, under: un });
      }
    }
  ...
  // Format 3: object com keys numéricos (linha → {over, under})
  if (t && typeof t === 'object' && !Array.isArray(t)) {
    for (const [k, v] of Object.entries(t)) {
      const ln = parseFloat(String(k).replace(',', '.').replace('linha_', ''));
      if (!Number.isFinite(ln) || !v || typeof v !== 'object') continue;
      const ov = parseFloat(v.over ?? v.acima);
      const un = parseFloat(v.under ?? v.abaixo);
      if (Number.isFinite(ov) && Number.isFinite(un)) {
        out.push({ point: ln, over: ov, under: un });
      }
    }
  }
...
/**
 * Parse Asian/European Handicap multi-line. Aceita:
 *   { 'handicap-asian': { '-0.5': {home,away}, '+0.5': {...} } }  ← scraper KTO 2026-05-08+
 *   { handicap: { '-1': {home,away}, ... } }
 *   { handicap: [{ linha: '-0.5', home, away }] }
 * Retorna [{ line: -0.5, home: 1.85, away: 1.95 }, ...]
 */
function _parseHandicapAsian(mercados) {
  if (!mercados || typeof mercados !== 'object') return [];
  const out = [];
  const src = mercados['handicap-asian'] || mercados.handicapAsian || mercados.handicap_asian
    || (typeof mercados.handicap === 'object' && !Array.isArray(mercados.handicap) ? mercados.handicap : null);
  if (src && typeof src === 'object' && !Array.isArray(src)) {
    for (const [lnK, v] of Object.entries(src)) {
      if (!v || typeof v !== 'object') continue;
      const ln = parseFloat(String(lnK).replace(',', '.').replace('linha_', '').replace('+', ''));
      const home = parseFloat(v.home ?? v.mandante ?? v[1] ?? v['1']);
      const away = parseFloat(v.away ?? v.visitante ?? v[2] ?? v['2']);
      if (Number.isFinite(ln) && Number.isFinite(home) && Number.isFinite(away)) {
        out.push({ line: ln, home, away });
      }
    }
    if (out.length) return out;
  }
  const arr = mercados.handicap;
  if (Array.isArray(arr)) {
    for (const it of arr) {
      const ln = parseFloat(String(it.linha ?? it.line ?? '').replace(',', '.').replace('+', ''));
      const home = parseFloat(it.home ?? it.mandante);
      const away = parseFloat(it.away ?? it.visitante);
      if (Number.isFinite(ln) && Number.isFinite(home) && Number.isFinite(away)) {
        out.push({ line: ln, home, away });
      }
    }
...
/**
 * Parse Dupla Chance: 1X / 12 / X2.
 * Retorna { home_or_draw, home_or_away, draw_or_away } ou null.
 */
function _parseDoubleChance(mercados) {
  if (!mercados || typeof mercados !== 'object') return null;
  const dc = mercados['dupla-chance'] || mercados.duplaChance || mercados.double_chance || mercados.dc;
  if (!dc || typeof dc !== 'object') return null;
  const hd = parseFloat(dc['1x'] ?? dc['1X'] ?? dc.home_or_draw ?? dc.casa_ou_empate);
  const ha = parseFloat(dc['12'] ?? dc.home_or_away ?? dc.casa_ou_visitante);
  const da = parseFloat(dc['x2'] ?? dc['X2'] ?? dc.draw_or_away ?? dc.empate_ou_visitante);
  if (!Number.isFinite(hd) || !Number.isFinite(ha) || !Number.isFinite(da)) return null;
  return { home_or_draw: hd, home_or_away: ha, draw_or_away: da };
}

/**
 * Para 1 jogo Supabase, retorna agregação de markets MT por casa.
 * Output: { ouMulti: [{point, over, under, bookmaker}], ah: [{line, home, away, bookmaker}], dc: [{1x,12,x2,bookmaker}] }
 * Aplica swap quando mandante/visitante invertidos vs match do bot.
 */
function jogoToMtMarkets(jogo, swap = false) {
  const out = { ouMulti: [], ah: [], dc: [] };
  const odds = jogo?.odds || [];
  for (const o of odds) {
    const m = o.mercados || {};
    // OU multi
    for (const it of _parseOuMulti(m)) {
      out.ouMulti.push({ ...it, bookmaker: o.casa, _capturedAt: o.atualizado_em });
    }
    // AH (swap inverte home<->away)
    for (const it of _parseHandicapAsian(m)) {
      const line = swap ? -it.line : it.line;
      const home = swap ? it.away : it.home;
      const away = swap ? it.home : it.away;
      out.ah.push({ line, home, away, bookmaker: o.casa, _capturedAt: o.atualizado_em });
    }
    // DC (swap inverte hd<->da)
    const dc = _parseDoubleChance(m);
    if (dc) {
      out.dc.push({
        '1x': swap ? dc.draw_or_away : dc.home_or_draw,
        '12': dc.home_or_away,
        'x2': swap ? dc.home_or_draw : dc.draw_or_away,
        bookmaker: o.casa, _capturedAt: o.atualizado_em,
      });
    }
  }
  return out;
...
/**
 * Enrich football matches com markets MT BR.
 * Mutates: match._brOddsMt = { ouMulti, ah, dc }
 * Pode ser chamado em paralelo a enrichMatches() — não conflita.
 *
 * 2026-05-08: introduzido pra alimentar football MT scanner com odds BR
 * (KTO/Sportingbet/Bet365/Betano/Superbet via aggregator Supabase).
 */
async function enrichFootballMtMarkets(matches) {
  ...
  const jogos = cutoffMs > 0
    ? allJogos.filter(j => {
        const ts = j.atualizado_em || j.updated_at;
        if (!ts) return true;
  ...
  for (const m of matches) {
    ...
    if (!found) continue;
    const mt = jogoToMtMarkets(found.jogo, found.swap);
    if (!mt.ouMulti.length && !mt.ah.length && !mt.dc.length) continue;
    m._brOddsMt = mt;
    enriched++;
  }
...
function jogoToMatch(jogo) {
  ...
  // 2026-05-14: normalize slug spelling variants pra dedup match_id agg_.
  // Aggregator BR às vezes grava typos PT-BR (rayo-valhecano com LH) enquanto
  // Pinnacle/ESPN usam oficial (rayo-vallecano com LL). Sem fix, mesmo match
  // gerava 2 tips com match_ids distintos → settle path nem encontrava match_result.
  // Aliases vêm de lib/football-poisson-trained._TEAM_ALIASES (já mantida).
  const _slugRaw = String(jogo.slug || jogo.id || '');
  const _normalizedSlug = _normalizeAggSlug(_slugRaw);
  return {
    id: `agg_${_normalizedSlug}`,
    game: 'football',
...
async function enrichMatches(matches, opts = {}) {
  ...
  // 2026-05-12: `enriched` agora conta matches cross-referenced com aggregator
  // (estável por ciclo), não delta de books novos. Antes log alternava enriched=47/0
  // porque _appendUnique dedup retornava 0 quando matches Pinnacle compartilhavam
  // _allOdds via cache (books já presentes). Dado funcional inalterado — só o log
  // se torna estável.
  let enrichedMatches = 0, enriched1x2New = 0, enrichedOu = 0, injected = 0;
  ...
  for (const m of matches) {
    ...
    if (brOu.length) {
      ...
      _appendUnique(m.odds.ou25._allOdds, brOu);
      // 2026-05-13: count matches com ou25 BR disponível (estável por ciclo),
      // mesmo padrão de `enriched` (fixado 705d826). _appendUnique retornava 0
      // quando books já presentes em _allOdds compartilhado entre ciclos —
      // ou25 flapava 0↔33 em logs sem mudança funcional. Dado inalterado.
      enrichedOu++;
    }
    // BTTS
  ...
  return { enriched: enrichedMatches, enrichedNew: enriched1x2New, enrichedOu, injected, totalJogos: jogos.length };
}
...
/**
 * Enrich tennis matches com markets MT BR (handicapGames + totalGames).
 * Mutates: match._brOddsMtTennis = { ouMulti, ah }
 *
 * Análoga a enrichFootballMtMarkets — semantica diferente:
 *   ouMulti em tennis = over/under GAMES (não goals); item.point pode ser 21.5..27.5
 *   ah      em tennis = handicap GAMES (não asian goals); item.line pode ser ±2.5..±5.5
 *
 * 2026-05-09: introduzido pra alimentar tennis MT scanner com odds BR
 * (KTO-tennis Fase 2 e similares). Pinnacle tennis MT real ROI +7% n=134
 * d30 (sharp); BR softer = potencial line shopping. Capture overpriced lines.
 */
async function enrichTennisMtMarkets(matches) {
  if (!Array.isArray(matches) || !matches.length) return { enriched: 0, totalTennis: 0 };
  const allJogos = await fetchUpcomingJogos({ daysAhead: 14 });
  if (!allJogos?.length) return { enriched: 0, totalTennis: 0, error: _cache.lastErrorMsg };
  // Filter só tennis. Aggregator publica esporte='tenis' (Sackmann/scraper schema).
  const tnJogos = allJogos.filter(j => /tenis|tennis/i.test(String(j.esporte || '')));
  if (!tnJogos.length) return { enriched: 0, totalJogos: allJogos.length, totalTennis: 0 };
  // Stale filter — separated env from football pq scraper tennis pode rodar em
  // schedule diferente. Default 30min.
  const maxAgeMin = parseInt(process.env.TENNIS_AGG_MAX_AGE_MIN || '30', 10);
  const cutoffMs = maxAgeMin > 0 ? (Date.now() - maxAgeMin * 60_000) : 0;
  const jogos = cutoffMs > 0
    ? tnJogos.filter(j => {
        const ts = j.atualizado_em || j.updated_at;
  ...
  let enriched = 0;
  for (const m of matches) {
    if (m.game && m.game !== 'tennis') continue;
    const found = findJogoByTeams(jogos, m.team1, m.team2);
    if (!found) continue;
    // Reusa jogoToMtMarkets — parser é genérico (não-sport-aware), só extrai
    // ouMulti + ah + dc do mercados JSON. Dc tennis raramente faz sentido
    // (sem draw); ignorado pelo bot consumer.
    const mt = jogoToMtMarkets(found.jogo, found.swap);
    if (!mt.ouMulti.length && !mt.ah.length) continue;
    m._brOddsMtTennis = mt;
    enriched++;
  }
  return { enriched, totalJogos: allJogos.length, totalTennis: tnJogos.length };
}
...
/**
 * Pull snapshots de DEBUG do scraper (HTML/JSON capturados quando scraper
 * detectou regressão silenciosa). Tabela `snapshots_debug` distinta de
 * `snapshots_odds` (esta é input real). Usado pra inspecionar markup novo
 * de casas que pararam de entregar mercados secundários.
 *
 * 2026-05-09: introduzido pra debug do KTO ouMulti=0 + Fase 2 kto-tennis.
 */
async function fetchScraperDebugSnapshots({ hoursBack = 6, casa = null, limit = 20 } = {}) {
  const SU = _supabaseUrl();
  // snapshots_debug RLS using(false) — só service_role lê (sql/004 policy).
  // Fallback anon retorna [] silently. Sem service key, retorna error explícito.
  const SK = _supabaseServiceKey();
  if (!SU) return { error: 'SUPABASE_URL não configurado' };
  if (!SK) return { error: 'SUPABASE_SERVICE_KEY não configurado (snapshots_debug requer service_role; setar Railway env)' };
  const sinceIso = new Date(Date.now() - hoursBack * 3600 * 1000).toISOString();
  let target = `${SU}/rest/v1/snapshots_debug?select=id,casa_slug,tipo,contexto,criado_em&criado_em=gte.${encodeURIComponent(sinceIso)}&order=criado_em.desc&limit=${limit}`;
  if (casa) target += `&casa_slug=eq.${encodeURIComponent(casa)}`;
  const headers = { apikey: SK, Authorization: `Bearer ${SK}`, 'User-Agent': 'SportsEdgeBot/1.0' };
  try {
    return await _httpGetJson(target, headers, 12000);
  } catch (e) {
...
/**
 * Pull o conteudo (HTML/JSON) de UM snapshot debug pelo ID.
 * Requer SUPABASE_SERVICE_KEY (RLS bloqueia anon). Truncate em 200KB.
 */
async function fetchScraperDebugSnapshotContent(id) {
  const SU = _supabaseUrl();
  const SK = _supabaseServiceKey();
  if (!SU) return { error: 'SUPABASE_URL não configurado' };
  if (!SK) return { error: 'SUPABASE_SERVICE_KEY não configurado (snapshots_debug requer service_role)' };
  if (!Number.isFinite(Number(id))) return { error: 'id required' };
  const target = `${SU}/rest/v1/snapshots_debug?id=eq.${id}&select=id,casa_slug,tipo,contexto,conteudo,criado_em&limit=1`;
  const headers = { apikey: SK, Authorization: `Bearer ${SK}`, 'User-Agent': 'SportsEdgeBot/1.0' };
  try {
    const arr = await _httpGetJson(target, headers, 15000);
    if (!Array.isArray(arr) || !arr.length) return { error: 'not_found', id };
    const r = arr[0];
    if (r.conteudo && r.conteudo.length > 200_000) {
      r.conteudo = r.conteudo.slice(0, 200_000) + `\n... [truncado, total ${r.conteudo.length} chars]`;
      r._truncado = true;
    }
    return r;
  } catch (e) {
...
function _httpPostJson(targetUrl, headers, body, timeoutMs = 12000) {
  return new Promise((resolve, reject) => {
    ...
    req.on('error', reject);
    // 2026-05-15 audit P0: req.setTimeout wirado (vs opts.timeout que só cobre connect).
    req.setTimeout(timeoutMs, () => { req.destroy(); reject(new Error('timeout')); });
...
module.exports = {
  ...
  enrichTennisMatches,
  enrichFootballMtMarkets,
  enrichTennisMtMarkets,
  getStatus,
  fetchScraperHealth,
  fetchRecentSnapshots,
  fetchScraperDebugSnapshots,
  fetchScraperDebugSnapshotContent,
  fetchActiveEdgesBr,

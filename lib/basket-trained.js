...
function _computeFeatures(db, home, away, gameDate, params) {
  ...
  // 2026-05-06 FIX: season NBA atravessa anos (2025-26 começa out/2025).
  // Antes Jan/Feb retornavam ano civil (=ano final da season) mas DB grava
  // season como ano de INÍCIO (2025). Resultado: jan-jun primeiros 6 meses do
  // novo calendário, getSeasonWr(_, 2026) caía em prior 0.5. Fix: ago-dez =
  // ano civil; jan-jul = ano civil − 1.
  const _seasonFromDate = (d) => {
    const dt = d ? new Date(d) : new Date();
    const month = dt.getUTCMonth() + 1; // 1-12
    const year = dt.getUTCFullYear();
    return month >= 8 ? year : year - 1;
  };
  const season = _seasonFromDate(gameDate);

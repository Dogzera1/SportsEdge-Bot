 ...
 *   "(Grass)"              → 'grama'
 *
 * Blend: 75% surface Elo + 25% overall Elo (or 100% overall when < 5 surface games)
 */
...
function blendElo(p, surf) {
  ...
  // Confidence in surface Elo: ramp from 0→75% over first 20 surface games
  const surfWeight = Math.min(0.75, gSurf / 20 * 0.75);
...
function getTennisElo(db, player1, player2, surface, impliedP1, impliedP2) {
  ...
  // Floor em 5 jogos (abaixo disso Elo é ruído puro); full weight em 20+ jogos.
  // Antes (/10) inflava edges em ITF/Challenger com 2-5 jogos.
  ...
  // pass when BOTH players are in DB (single-found → Elo vs 1500 init is unreliable)
  const pass = (found1 && found2) && score >= 1.0;

  // Raw Elo por surface (p/ modelo treinado)
  const surfKey = surface === 'dura' ? 'dura' : surface === 'saibro' ? 'saibro' : 'grama';
  const eloOverall1 = p1Entry?.overall ?? ELO_INIT;
  const eloOverall2 = p2Entry?.overall ?? ELO_INIT;
  const eloSurface1 = p1Entry?.[surfKey] ?? ELO_INIT;
  const eloSurface2 = p2Entry?.[surfKey] ?? ELO_INIT;
  ...
  return {
    ...
    eloOverall1: Math.round(eloOverall1),
    eloOverall2: Math.round(eloOverall2),
    eloSurface1: Math.round(eloSurface1),
    eloSurface2: Math.round(eloSurface2),
    edge1, edge2,
    factorCount,
    score,
    direction,

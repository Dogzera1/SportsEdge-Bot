...
const _pollSilentCooldowns = new Map();
const POLL_SILENT_COOLDOWN_MS = 30 * 60 * 1000;

function pollSilentFix(sportKey) {
  const cdKey = `poll_silent_${sportKey}`;
  return {
    severity: 'warning',
    description: `poll ${sportKey} silent`,
    precondition: ({ ctx, anomaly }) => {
      const until = _pollSilentCooldowns.get(cdKey);
      if (until && Date.now() < until) {
        return { ok: false, reason: `cooldown ativo (fix anterior retornou 0 matches), até ${new Date(until).toISOString()}` };
      }
      if (until) _pollSilentCooldowns.delete(cdKey);
    ...
    action: async ({ ctx }) => {
      ...
      const n = isArr ? result.length : (result?.error ? `error: ${result.error}` : 'ok');
      // Sem matches reais → fix não muda heartbeat, sentinel re-detecta no próximo ciclo.
      // Cooldown 30min evita re-invocar pollFn em loop e ruído de log per-ciclo.
      if (!isArr || result.length === 0 || result?.error) {
        _pollSilentCooldowns.set(cdKey, Date.now() + POLL_SILENT_COOLDOWN_MS);
      }

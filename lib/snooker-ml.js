 ...
 *   h2hP1Wins, h2hP2Wins,          // histórico entre os dois jogadores (CueTracker)
 * }
 */
function snookerPreFilter(match, enrich) {
  ...
  // ── Fator 4: H2H histórico (CueTracker) ──
  // Só conta com ≥5 matches entre eles (amostra significativa). Peso baixo mas conservador.
  // Saturação na dominância: 80%+ H2H → ±12pp bruto × 0.12 = ±1.5pp. Não domina gate sozinho.
  ...
  if (Number.isFinite(h2hP1) && Number.isFinite(h2hP2) && (h2hP1 + h2hP2) >= 5) {
    ...
    const h2hWrP1 = h2hP1 / totalH2H; // 0..1
    const h2hBias = h2hWrP1 - 0.5;    // -0.5..+0.5
    const clamped = Math.max(-0.3, Math.min(0.3, h2hBias)); // satura em 80%+
    // Confiança adicional pelo tamanho da amostra: 5→0.5, 15+→1.0
    const h2hConf = Math.min(1.0, totalH2H / 15);
    adjustP1 += clamped * 0.12 * h2hConf;
    factorCount++;
  }

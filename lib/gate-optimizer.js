...
// 2026-05-03 FIX: vig per-sport (Pinnacle 2-way ML ~2-3%, 3-way football ~4-5%,
// soft books mais alto). Antes hardcoded 1.025 (5% vig assumido) underestima
// divergência em football (vig real maior) e overestima em esports/tennis ML.
const VIG_PER_SPORT = {
  // 2-way ML sharp: Pinnacle ~2-3%
  lol: 1.025, dota2: 1.025, cs2: 1.025, valorant: 1.025, esports: 1.025,
  tennis: 1.025, mma: 1.030,
  // 3-way (football): vig naturalmente maior, ~4-5%
  football: 1.045,
  // Outros (soft markets ou nicho): assume um pouco maior
  darts: 1.040, snooker: 1.040, tabletennis: 1.045,
  default: 1.030,
};

function _divergencePp(odds, modelP, sport) {
  const o = Number(odds);
  const p = Number(modelP);
  if (!Number.isFinite(o) || !Number.isFinite(p) || o <= 1 || p <= 0 || p >= 1) return null;
  const impliedRaw = 1 / o;
  const sp = String(sport || '').toLowerCase();
  const vigDivisor = VIG_PER_SPORT[sp] || VIG_PER_SPORT.default;
...
function runGateOptimizer(db, opts = {}) {
  ...
  // 2026-05-11 audit P2-1: baseQuery misturava tips shadow + real. gate-optimizer
  // é "recommend-only" (DM admin, não auto-apply), mas recomendação enviesa quando
  // shadow tips low-EV pesam no aggregate. Default real-only; opt-out preserva
  // legacy mixed behavior.
  const realOnly = opts.realOnly != null
    ? !!opts.realOnly
    : !/^(0|false|no)$/i.test(String(process.env.GATE_OPTIMIZER_REAL_ONLY ?? 'true'));
  ...
  for (const [sport, sportTips] of bySport.entries()) {
    ...
    for (const cap of caps) {
      const filtered = sportTips.filter(t => {
        const d = _divergencePp(t.odds, t.model_p_pick, sport);
        return d == null || d <= cap;
      });
...
module.exports = { runGateOptimizer, CURRENT_CAPS, DEFAULT_GRID, VIG_PER_SPORT, _divergencePp };

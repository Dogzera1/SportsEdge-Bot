...
function computeMarketCorrelation(tipA, tipB) {
  ...
  // ML vs handicap
  if (eq2(mA, mB, 'market_winner', 'handicapGames') || eq2(mA, mB, 'moneyline', 'handicapGames')) {
  ...
  // handicapGames vs totalGames
  // Para handicapGames, sweep ≈ line ≤ -4.5 (favorito esmagador). -1.5 não é sweep.
  if (eq2(mA, mB, 'handicapGames', 'totalGames')) {
    const handiWin = tipA.market === 'handicapGames' ? tipA : tipB;
    const totalTip = tipA.market === 'totalGames' ? tipA : tipB;
    const isSweep = handiWin.line <= -4.5;
  ...
  // handicapGames vs TB
  if (eq2(mA, mB, 'handicapGames', 'tiebreakMatch')) {
    const handiTip = tipA.market === 'handicapGames' ? tipA : tipB;
    const tbTip = tipA.market === 'tiebreakMatch' ? tipA : tipB;
    const isSweep = handiTip.line <= -4.5;
...
function adjustStakesForCorrelation(tips) {
  ...
  // Quarter-Kelly safety net pra tips no mesmo match. Mesmo c/ correlação
  // empírica <0.3, múltiplas tips no mesmo evento amplificam variance (rho
  // real >0 por construção). Floors: 1 tip = 0%, 2 = 25% (3/4 Kelly),
  // 3+ = 50% (half Kelly). Combina com pair discount via Math.max.
  const N = tips.length;
  const baselineCap = N >= 3 ? 0.50 : N === 2 ? 0.25 : 0;
  const result = [];
  for (let i = 0; i < tips.length; i++) {
    ...
    const pairDiscount = maxAbsCorr > 0.3 ? maxAbsCorr * DISCOUNT_FACTOR : 0;
    const discount = Math.max(baselineCap, pairDiscount);
    result.push({
      ...tips[i],

...
const WORKFLOWS = {
  ...
  tip_emergency: {
    ...
    steps: [
      ...
      { name: 'news_monitor', agent: 'news-monitor', critical: false },
      {
        name: 'check_alerts',
        custom: (ctx) => {
          const preAlerts = ctx.pre_match_check?.alerts || [];
          const newsAlerts = (ctx.news_monitor?.alerts || []).filter(a => a.matched_tips_count > 0);
          const total = preAlerts.length + newsAlerts.length;
          if (total === 0) {
            ctx.shortcircuit = { reason: 'no_tip_emergencies' };
            return { ok: true, skip_remaining: true };
          }
          return { ok: true, pre_match_alerts: preAlerts.length, news_alerts: newsAlerts.length };
        },
      },
      { name: 'feed_medic', agent: 'feed-medic', critical: false },
    ],
  ...
  daily_health: {
    description: 'Relatório diário: portfolio + ROI + saúde + bankroll + backtest',
    steps: [
      ...
      { name: 'backtest_validator', agent: 'backtest-validator', critical: false, args: { days: 90 } },
    ],
  },
  ...
  // Workflow: model health check — calibração + IA + bankroll + backtest.
  model_check: {
    description: 'Saúde dos modelos: backtest + calibração + IA + bankroll',
    steps: [
      { name: 'backtest_validator', agent: 'backtest-validator', critical: true, args: { days: 90 } },
      { name: 'model_calibration', agent: 'model-calibration', critical: false },
      { name: 'ia_health', agent: 'ia-health', critical: false },

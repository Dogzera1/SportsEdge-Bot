'use strict';

/**
 * epoch.js — captura git SHA + snapshot de env vars/auto-tunes ativas
 * no momento do tip insert. Permite filtrar análises por regime.
 *
 * SHA é capturado uma vez na primeira chamada (cached).
 * gateState é re-computado a cada chamada (capturando estado dinâmico).
 */

let _SHA = null;

function getCodeSha() {
  if (_SHA !== null) return _SHA;
  // 2026-05-21 audit logs+drift: Railway prod retornava code_sha=unknown porque
  // git rev-parse falhava (container deployment sem .git dir). Railway expõe
  // RAILWAY_GIT_COMMIT_SHA automático em containers — preferir env. Fallback
  // git rev-parse só em dev local (npm start sem Railway env).
  const envSha = process.env.RAILWAY_GIT_COMMIT_SHA;
  if (envSha) {
    _SHA = String(envSha).slice(0, 7);
    return _SHA;
  }
  try {
    _SHA = require('child_process')
      .execSync('git rev-parse --short HEAD', { cwd: __dirname, encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] })
      .trim() || '';
  } catch (_) { _SHA = ''; }
  return _SHA;
}

const RELEVANT_ENVS = [
  'ODDS_BUCKET_BLOCK', 'PRE_MATCH_EV_BONUS', 'MAX_STAKE_UNITS',
  'TENNIS_ISOTONIC_DISABLED', 'AI_DISABLED',
  'CS_PRE_MATCH_EV_BONUS', 'VALORANT_PRE_MATCH_EV_BONUS', 'LOL_PRE_MATCH_EV_BONUS',
  'DARTS_PRE_MATCH_EV_BONUS', 'DOTA2_PRE_MATCH_EV_BONUS',
  'TENNIS_PRE_MATCH_EV_BONUS', 'FOOTBALL_PRE_MATCH_EV_BONUS',
  'CS_MAX_STAKE_UNITS', 'LOL_MAX_STAKE_UNITS', 'TENNIS_MAX_STAKE_UNITS',
  'VALORANT_MAX_STAKE_UNITS', 'DOTA2_MAX_STAKE_UNITS',
  'CS_ODDS_BUCKET_BLOCK', 'LOL_ODDS_BUCKET_BLOCK', 'VALORANT_ODDS_BUCKET_BLOCK',
];

/**
 * Captura snapshot de gates ativos no momento do tip insert.
 *
 * Args:
 *   opts.gates_evaluated — opcional, lista per-tip {gate, passed, value, threshold, reason?}
 *     vinda de shouldSendMarketTip ou similar. Quando presente, é mergada no JSON pra
 *     audit retroativo: "que gates essa tip específica passou no momento do emit?".
 *     Sem isso, gate_state guarda só env global + auto-tunes — insuficiente pra forensics
 *     individual.
 *
 * Retorno: JSON string `{env, auto, gates_evaluated?}`. Tamanho típico <2KB.
 */
function captureGateState(opts = null) {
  const env = {};
  for (const k of RELEVANT_ENVS) {
    const v = process.env[k];
    if (v != null && v !== '') env[k] = v;
  }
  let auto = {};
  try {
    const { getAll } = require('./gates-runtime-state');
    for (const [k, v] of getAll()) auto[k] = v.value;
  } catch (_) {}

  const payload = { env, auto };
  if (opts && Array.isArray(opts.gates_evaluated) && opts.gates_evaluated.length > 0) {
    payload.gates_evaluated = opts.gates_evaluated;
  }
  return JSON.stringify(payload);
}

module.exports = { getCodeSha, captureGateState, RELEVANT_ENVS };

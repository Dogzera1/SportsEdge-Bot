...
// Fase 1 default: NBA. 2026-05-10: Euroleague opt-in via env BASKET_LEAGUES_EXTRA
// (csv de slugs ESPN: 'euroleague,wnba,mens-college-basketball'). Adicionar mais
// expande cobertura sem mudança de código, mas precisa seed histórico via
// /admin/basket-seed pra os novos slugs antes do Elo funcionar (cold-start skipa).
function _resolveLeagues() {
  const base = ['nba'];
  const extra = String(process.env.BASKET_LEAGUES_EXTRA || '')
    .split(',').map(s => s.trim().toLowerCase()).filter(Boolean);
  return [...new Set([...base, ...extra])];
}
const LEAGUES = _resolveLeagues();
...
function parseFinished(ev, leagueSlug) {
  try {
    ...
    const detail = String(comp.status?.type?.detail || comp.status?.type?.shortDetail || '');
    // 2026-05-06: forfeit detection. NBA tecnicamente reporta 0-1 ou 2-0 técnico
    // em raríssimos casos (ESPN status detail contém "Forfeit"/"FF"/"Default").
    // Tip handicap/total nesse caso → void (placar não reflete jogo real).
    const isForfeit = /\b(forfeit|forfeited|default)\b/i.test(detail);
    const completed = comp.status?.type?.completed === true
      || /STATUS_FINAL|post/i.test(String(status || ''));
    if (!completed && !isForfeit) return null;
...
async function getFinishedMatches({ daysBack = 3, leagues = LEAGUES } = {}) {
  ...
  // 2026-05-06: paralelização concurrency=8 (mesma estratégia espn-soccer).
  const tasks = [];
  for (const league of leagues) {
    for (let i = 0; i <= backs; i++) {
      const d = new Date(today.getTime() - i * 24 * 60 * 60 * 1000);
      tasks.push({ league, ymd: fmtYmd(d) });
    }
  }
...
async function getUpcomingMatches({ daysAhead = 2, leagues = LEAGUES } = {}) {
  ...
  // 2026-05-15 audit P1: paralelização concurrency=8 (espelha getFinishedMatches L139-166).
  // Antes era sequential (league × day = ~14-21 reqs × 2s/req = 28-42s wall time → cron stale).
  const tasks = [];
  for (const league of leagues) {

...
function _formSubModel(db, team1, team2, enrich, asOfDate) {
  ...
  // When asOfDate is provided the window is anchored to that date (point-in-time,
  // no future leakage); the default (asOfDate falsy) path preserves the EXACT
  // current SQL and binds so existing callers are byte-identical.
  let rows1, rows2;
  if (asOfDate) {
    try {
      rows1 = db.prepare(`
        ...
        WHERE (lower(team1) = lower(?) OR lower(team2) = lower(?))
          ...
          AND resolved_at >= datetime(?, '-60 days')
          AND resolved_at < datetime(?, '0 days')
        ORDER BY resolved_at DESC
        LIMIT 10
      `).all(team1, team1, asOfDate, asOfDate);
    } catch (_) { rows1 = []; }
    ...
    try {
      rows2 = db.prepare(`
        ...
        ORDER BY resolved_at DESC
        LIMIT 10
      `).all(team2, team2, asOfDate, asOfDate);
    } catch (_) { rows2 = []; }
  } else {
    try {
      rows1 = db.prepare(`
    ...
    try {
      ...
      const fuzzyRows = asOfDate
        ? db.prepare(`
          ...
          `).all(fuzzy, fuzzy, asOfDate, asOfDate)
        : db.prepare(`
            SELECT team1, team2, winner, final_score, league, resolved_at
            FROM match_results
            WHERE (lower(team1) LIKE lower(?) OR lower(team2) LIKE lower(?))
...
function getLolProbability(db, match, odds, enrich, compScore) {
  ...
  // 2026-05-18 (P3 full step 2): salva modelP1Raw ANTES da isotonic apply.
  // Habilita captura em market_tips_shadow.model_p_raw (mig 117) pra brier-holdout-eval
  // isolar performance do FIT cycle (sem post-calib pulls). Caller exposes via
  // result.modelP1Raw — MT scanner passa pra logShadowTip(tip.modelPRaw).
  const modelP1Raw = modelP1;
  ...
  return {
    ...
    modelP1Raw: Math.round(modelP1Raw * 10000) / 10000, // P3 step 2: pre-isotonic
...
module.exports = {
  ...
  mapProbFromSeries,
  shrinkForBestOf,
  // Exposed for testing / match-lab composition
  _eloSubModel,
  _compSubModel,
  _formSubModel,
  _oeSubModel,

 ...
 * @param {number} [opts.minSubstrLen=4] — evita matches espúrios de siglas curtas
 * @returns {{ match: boolean, score: number, method: 'exact'|'alias'|'substring'|'none' }}
 */
function nameMatches(a, b, opts = {}) {
  const { aliases, minSubstrLen = 4 } = opts;
  const na = norm(a);
  const nb = norm(b);
  ...
  if (na.length >= minSubstrLen && nb.length >= minSubstrLen) {
    if (na.includes(nb) || nb.includes(na)) {
      ...
      return { match: true, score: +(shorter / longer).toFixed(2), method: 'substring' };
    }
  }

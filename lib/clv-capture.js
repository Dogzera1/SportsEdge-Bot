...
async function captureMarketTipsClv(db, serverGet) {
  const out = { checked: 0, updated: 0, errors: 0, bySport: {}, skipped: { match: 0, odd: 0, sameOdd: 0, badOdd: 0 } };
  let rows;
  try {
    rows = db.prepare(`
  ...
  for (const tip of rows) {
    ...
    if (!Number.isFinite(openOdd) || openOdd <= 1) { out.skipped.odd++; continue; }
    if (Math.abs(currentOdd - openOdd) < 0.005) { out.skipped.sameOdd++; continue; }

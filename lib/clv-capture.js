...
async function captureMarketTipsClv(db, serverGet) {
  ...
  try {
    rows = db.prepare(`
      ...
      WHERE result IS NULL
        ...
        AND market IN ('handicap', 'handicapSets', 'handicapGames', 'total', 'totals', 'totalGames')
  ...
  for (const tip of rows) {
    out.checked++;
    const currentOdd = await _fetchCurrentMarketOdd(tip, fetchMarkets(tip.sport));

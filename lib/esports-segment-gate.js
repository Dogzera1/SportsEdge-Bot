...
// 2026-05-15 (Sprint 2 unification): migrated from esports-runtime-features.leagueTier
// (inverted 3=top convention, locked to ML feature) to canonical lib/league-tier.js
// (1=top). Inversion shim removed. ML feature path in esports-runtime-features
// keeps its own leagueTier export untouched (model trained on that convention).
const { parseBestOf } = require('./esports-runtime-features');
...
const POLICY = {
  ...
  cs2: {
    ...
    // 2026-05-11 audit P0-2: tier2/Bo3 +1pp era insuficiente. Real 30d:
    // CCT European Series 1 UNDER 2.5 maps n=5 ROI -100%, CCT South America
    // n=3 ROI -24%, ESL Challenger NA ML n=4 ROI -54.5%. Brier 0.229 é só
    // marginalmente melhor que random (0.25) → edge real precisa ser maior
    // que +1pp pra valer a pena. Bumped 1→3.
    tier2: { Bo1: { skip: false, minEdgeBonus: 3, brier: 0.242, reason: 'CS2 tier2 Bo1 Brier 0.242' },
             Bo3: { skip: false, minEdgeBonus: 3, brier: 0.229, reason: 'CS2 tier2 Bo3 audit 2026-05-11 (CCT/Challenger leak)' },
             Bo5: { skip: false, minEdgeBonus: 3, brier: 0.244, reason: 'CS2 tier2 Bo5 Brier 0.244' } },
    // tier3 = leagues completamente desconhecidas. Mantido +2-3pp original
    // (não bumped 2026-05-11 — pre-existing fix de leagueTier já realocou
    // a maioria das ligas tier1/2 corretamente; tier3 agora é genuinamente
    // unknown territory, não overlap com IEM/BLAST/CCT).
    tier3: { Bo1: { skip: false, minEdgeBonus: 3, brier: 0.25, reason: 'CS2 tier3 fallback (sinal fraco)' },
             Bo3: { skip: false, minEdgeBonus: 2, brier: 0.24, reason: 'CS2 tier3 fallback' },
             Bo5: { skip: false, minEdgeBonus: 3, brier: 0.25, reason: 'CS2 tier3 fallback' } },
  },
  dota2: {
    ...
    tier3: { Bo1: { skip: false, minEdgeBonus: 2, brier: 0.24, reason: 'Dota2 tier3 fallback' },
             Bo2: { skip: false, minEdgeBonus: 1, brier: 0.22, reason: 'Dota2 tier3 fallback' },
             Bo3: { skip: false, minEdgeBonus: 2, brier: 0.24, reason: 'Dota2 tier3 fallback' },
             Bo5: { skip: false, minEdgeBonus: 3, brier: 0.25, reason: 'Dota2 tier3 fallback' } },
  },
  lol: {
    tier1: { Bo1: { skip: false, minEdgeBonus: 0, brier: 0.22 },
             Bo2: { skip: false, minEdgeBonus: 0, brier: 0.22 },
    ...
    tier2: { Bo1: { skip: false, minEdgeBonus: 1, brier: 0.23 },
             Bo2: { skip: false, minEdgeBonus: 1, brier: 0.23 },
             Bo3: { skip: false, minEdgeBonus: 1, brier: 0.23 },
             Bo5: { skip: false, minEdgeBonus: 1, brier: 0.23 } },
    // 2026-05-03 FIX: tier3 (ligas obscuras) e Bo2 LoL faltavam — bypass total do gate
    // (skip=false, minEdgeBonus=0). Adicionar fallback conservador exige edge bonus.
    tier3: { Bo1: { skip: false, minEdgeBonus: 2, brier: 0.25, reason: 'LoL tier3 fallback (sinal fraco)' },
             Bo2: { skip: false, minEdgeBonus: 2, brier: 0.25, reason: 'LoL tier3 fallback' },
             Bo3: { skip: false, minEdgeBonus: 2, brier: 0.25, reason: 'LoL tier3 fallback' },
             Bo5: { skip: false, minEdgeBonus: 2, brier: 0.25, reason: 'LoL tier3 fallback' } },
  },
};
...
function esportsSegmentGate(game, league, bestOfOrFormat) {
  ...
  // Canonical naming (1=top, 2=mid, 3=other) — direto sem inversion shim.
  const tier = getLeagueTier(g, league);
  let bo;
  if (typeof bestOfOrFormat === 'number') bo = bestOfOrFormat;
  else bo = parseBestOf(bestOfOrFormat, null);
  const tierKey = `tier${tier}`;
  const boKey = `Bo${bo}`;
  // 2026-05-03 FIX: fallback chain pra prevenir bypass silent quando tier/bo faltam.
  // 1) tenta exact (tierKey, boKey). 2) tenta tier3 do mesmo bo. 3) tenta tierKey/Bo3.
  // 4) último: penalty bonus default 1pp em vez de 0.
  let policy = POLICY[g]?.[tierKey]?.[boKey];
  if (!policy) policy = POLICY[g]?.tier3?.[boKey];
  if (!policy) policy = POLICY[g]?.[tierKey]?.Bo3;
  if (!policy) {
    return { skip: false, minEdgeBonus: 1, reason: `${g} ${tierKey}/${boKey} ausente — fallback +1pp`, tier, bestOf: bo };
  }
  return {
    skip: !!policy.skip,

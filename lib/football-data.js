...
// Retorna partidas agendadas nos próximos daysAhead dias para uma competição
async function getUpcomingFixtures(competitionCode, { sportKey = null, daysAhead = 7 } = {}) {
  if (!competitionCode) return [];
  const today = new Date().toISOString().slice(0, 10);
  const future = new Date(Date.now() + daysAhead * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);
  const data = await httpGetJson(
    `/competitions/${competitionCode}/matches?dateFrom=${today}&dateTo=${future}&status=SCHEDULED`,
    { ttlMs: 30 * 60 * 1000 }
  ).catch(() => null);
  ...
  return data.matches.map(m => ({
    ...
    league: m.competition?.name || competitionCode,
    time: m.utcDate || null,
    odds: null,
    _source: 'football-data.org',
    _fixtureId: m.id,
    _homeId: m.homeTeam?.id || null,
    _awayId: m.awayTeam?.id || null,
    _competitionId: m.competition?.id || null,
  }));
}

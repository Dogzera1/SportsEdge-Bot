/**
 * sofascore-mma.js — Enriquecimento de MMA via proxy Sofascore.
 *
 * Env:
 * - SOFASCORE_PROXY_BASE   — URL do proxy
 * - SOFASCORE_DIRECT=true  — tenta api.sofascore.com direto
 * - SOFASCORE_ENRICH_MMA   — default: ligado se proxy existir
 *
 * Saída compatível com esportsPreFilter (form1/form2 no mesmo shape de mmaRecordToEnrich).
 */
const { safeParse, cachedHttpGet, throttleHost, BROWSER_UA, log } = require('./utils');

const BROWSER_HEADERS = {
  Accept: 'application/json,text/plain,*/*',
  'User-Agent': BROWSER_UA,
  Origin: 'https://www.sofascore.com',
  Referer: 'https://www.sofascore.com/',
  'ngrok-skip-browser-warning': 'true'
};

function normName(s) {
  return String(s || '')
    .toLowerCase()
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .replace(/[^a-z0-9]/g, '');
}

function lastName(s) {
  const t = String(s || '').trim().split(/\s+/).filter(Boolean);
  return t.length ? t[t.length - 1] : '';
}

function namesMatch(evName, query) {
  const a = normName(evName);
  const b = normName(query);
  if (!a || !b) return false;
  if (a === b || a.includes(b) || b.includes(a)) return true;
  const la = normName(lastName(evName));
  const lb = normName(lastName(query));
  return la.length >= 4 && la === lb;
}

function _directEnabled() {
  const v = String(process.env.SOFASCORE_DIRECT || '').toLowerCase();
  return v === '1' || v === 'true' || v === 'yes';
}

function buildUrl(base, path) {
  const b = String(base || '').replace(/\/+$/, '');
  const p = path.startsWith('/') ? path : `/${path}`;
  return `${b}${p}`;
}

function scheduleUrls(isoDate) {
  const d = String(isoDate || '').slice(0, 10);
  if (!/^\d{4}-\d{2}-\d{2}$/.test(d)) return [];
  const proxy = (process.env.SOFASCORE_PROXY_BASE || '').trim().replace(/\/+$/, '');
  const urls = [];
  if (proxy) urls.push(buildUrl(proxy, `/schedule/mma/${d}/`));
  if (_directEnabled()) urls.push(`https://api.sofascore.com/api/v1/sport/mma/scheduled-events/${d}`);
  return [...new Set(urls)];
}

function teamEventsUrls(id) {
  const n = parseInt(String(id), 10);
  if (!Number.isFinite(n)) return [];
  const proxy = (process.env.SOFASCORE_PROXY_BASE || '').trim().replace(/\/+$/, '');
  const urls = [];
  if (proxy) urls.push(buildUrl(proxy, `/team/${n}/events/last/0`));
  if (_directEnabled()) urls.push(`https://api.sofascore.com/api/v1/team/${n}/events/last/0`);
  return [...new Set(urls)];
}

async function httpJson(url, { ttlMs = 10 * 60 * 1000 } = {}) {
  await throttleHost('sofascore', parseInt(process.env.SOFA_MIN_GAP_MS || '200', 10));
  const r = await cachedHttpGet(url, {
    ttlMs, provider: 'sofascore', headers: BROWSER_HEADERS,
    cacheKey: `sofascore-mma:${url}`
  }).catch(() => null);
  // 2026-05-18 (EXT-1 P5): logging cross-sport via lib/utils.logSofaNon200.
  if (!r) { try { require('./utils').logSofaNon200(url, null, 'SOFA-MMA'); } catch (_) {} return null; }
  if (r.status !== 200) { try { require('./utils').logSofaNon200(url, r.status, 'SOFA-MMA'); } catch (_) {} return null; }
  return safeParse(r.body, null);
}

async function fetchFirstJson(urls) {
  for (const u of urls) {
    const j = await httpJson(u).catch(() => null);
    if (j) return j;
  }
  return null;
}

function collectEvents(data) {
  const out = [];
  const visit = node => {
    if (!node || typeof node !== 'object') return;
    if (Array.isArray(node)) { for (const x of node) visit(x); return; }
    if (node.homeTeam && node.awayTeam && node.id != null) { out.push(node); return; }
    for (const k of Object.keys(node)) visit(node[k]);
  };
  visit(data);
  return out;
}

function winRateFromFighterEvents(json, fighterId, limit = 15) {
  const events = json?.events || json?.results || [];
  let wins = 0, losses = 0;
  const recent = [];
  for (const ev of events) {
    const wc = ev?.winnerCode;
    const hid = ev?.homeTeam?.id;
    const aid = ev?.awayTeam?.id;
    if (wc !== 1 && wc !== 2) continue;
    if (hid !== fighterId && aid !== fighterId) continue;
    const won = (wc === 1 && hid === fighterId) || (wc === 2 && aid === fighterId);
    if (won) { wins++; recent.push('W'); }
    else { losses++; recent.push('L'); }
    if (recent.length >= limit) break;
  }
  const total = wins + losses;
  if (!total) return null;
  return {
    wins, losses,
    winRate: Math.round((wins / total) * 100),
    recent,
  };
}

/**
 * @param {string} f1 - nome do lutador 1
 * @param {string} f2 - nome do lutador 2
 * @param {string} commenceIso
 */
async function enrichMatch(f1, f2, commenceIso) {
  const hasProxy = !!(process.env.SOFASCORE_PROXY_BASE || '').trim();
  const env = String(process.env.SOFASCORE_ENRICH_MMA || '').toLowerCase();
  const enabled = env === 'true' || env === '1' || env === 'yes'
    || (env !== 'false' && env !== '0' && env !== 'no' && hasProxy);
  if (!enabled) return null;

  const d0 = String(commenceIso || '').slice(0, 10);
  const dates = new Set();
  if (/^\d{4}-\d{2}-\d{2}$/.test(d0)) {
    dates.add(d0);
    try {
      const t = new Date(commenceIso);
      const prev = new Date(t); prev.setUTCDate(prev.getUTCDate() - 1);
      dates.add(prev.toISOString().slice(0, 10));
      const next = new Date(t); next.setUTCDate(next.getUTCDate() + 1);
      dates.add(next.toISOString().slice(0, 10));
    } catch (_) {}
  }
  if (!dates.size) return null;

  let found = null;
  for (const d of dates) {
    const json = await fetchFirstJson(scheduleUrls(d));
    if (!json) continue;
    const events = collectEvents(json);
    for (const ev of events) {
      const hn = ev?.homeTeam?.name || ev?.homeTeam?.shortName || '';
      const an = ev?.awayTeam?.name || ev?.awayTeam?.shortName || '';
      const fwd = namesMatch(hn, f1) && namesMatch(an, f2);
      const rev = namesMatch(hn, f2) && namesMatch(an, f1);
      if (!fwd && !rev) continue;
      found = {
        eventId: ev.id,
        fighter1Id: fwd ? ev.homeTeam?.id : ev.awayTeam?.id,
        fighter2Id: fwd ? ev.awayTeam?.id : ev.homeTeam?.id,
        weightClass: ev?.tournament?.name || null,
        org: ev?.tournament?.uniqueTournament?.name || null,
        eventName: ev?.season?.name || ev?.tournament?.name || null,
      };
      break;
    }
    if (found) break;
  }
  if (!found?.eventId || !found.fighter1Id || !found.fighter2Id) return null;

  const [e1, e2] = await Promise.all([
    fetchFirstJson(teamEventsUrls(found.fighter1Id)),
    fetchFirstJson(teamEventsUrls(found.fighter2Id)),
  ]);

  const form1 = e1 ? winRateFromFighterEvents(e1, found.fighter1Id) : null;
  const form2 = e2 ? winRateFromFighterEvents(e2, found.fighter2Id) : null;

  if (!form1 && !form2) return null;

  return {
    form1, form2,
    h2h: { t1Wins: 0, t2Wins: 0, totalMatches: 0 },
    oddsMovement: null,
    eventId: found.eventId,
    fighter1Id: found.fighter1Id,
    fighter2Id: found.fighter2Id,
    weightClass: found.weightClass,
    org: found.org,
    eventName: found.eventName,
  };
}

// Lookup leve só para org/eventName (sem buscar histórico de jogadores).
// Usado para identificar UFC/PFL/Bellator nas notificações.
async function lookupOrg(f1, f2, commenceIso) {
  const d0 = String(commenceIso || '').slice(0, 10);
  const dates = new Set();
  if (/^\d{4}-\d{2}-\d{2}$/.test(d0)) {
    dates.add(d0);
    try {
      const t = new Date(commenceIso);
      const prev = new Date(t); prev.setUTCDate(prev.getUTCDate() - 1);
      dates.add(prev.toISOString().slice(0, 10));
      const next = new Date(t); next.setUTCDate(next.getUTCDate() + 1);
      dates.add(next.toISOString().slice(0, 10));
    } catch (_) {}
  }
  if (!dates.size) return null;
  for (const d of dates) {
    const json = await fetchFirstJson(scheduleUrls(d));
    if (!json) continue;
    const events = collectEvents(json);
    for (const ev of events) {
      const hn = ev?.homeTeam?.name || ev?.homeTeam?.shortName || '';
      const an = ev?.awayTeam?.name || ev?.awayTeam?.shortName || '';
      const fwd = namesMatch(hn, f1) && namesMatch(an, f2);
      const rev = namesMatch(hn, f2) && namesMatch(an, f1);
      if (!fwd && !rev) continue;
      return {
        org: ev?.tournament?.uniqueTournament?.name || null,
        eventName: ev?.season?.name || ev?.tournament?.name || null,
        weightClass: ev?.tournament?.name || null,
      };
    }
  }
  return null;
}

/**
 * Varre Sofascore upcoming MMA matches em janela ±N dias. Cobertura inclui
 * orgs regionais (LFA, Oktagon, KSW, RIZIN, etc) que NÃO têm endpoint ESPN
 * dedicado. Retorna shape compatível com getEspnMmaFights.
 *
 * @param {object} opts { futureDays=7, pastDays=2, orgFilter=null }
 * @returns {Promise<Array<{id, game, team1, team2, league, time, status, _source}>>}
 */
async function fetchUpcomingFights({ futureDays = 7, pastDays = 2, orgFilter = null } = {}) {
  const out = [];
  const today = new Date();
  const days = [];
  for (let i = -pastDays; i <= futureDays; i++) {
    const d = new Date(today);
    d.setUTCDate(d.getUTCDate() + i);
    days.push(d.toISOString().slice(0, 10));
  }
  const seen = new Set();
  let _lastJson = null;
  let _daysWithJson = 0;
  for (const iso of days) {
    const json = await fetchFirstJson(scheduleUrls(iso)).catch(() => null);
    if (!json) continue;
    _lastJson = json;
    _daysWithJson++;
    const events = collectEvents(json);
    for (const ev of events) {
      const statusType = ev?.status?.type;
      if (statusType === 'finished') continue; // só upcoming/inprogress
      const home = ev?.homeTeam?.name;
      const away = ev?.awayTeam?.name;
      if (!home || !away) continue;
      const eid = ev.id;
      if (seen.has(eid)) continue;
      seen.add(eid);
      const org = ev?.tournament?.uniqueTournament?.name || ev?.tournament?.name || null;
      if (orgFilter && org && !String(org).toLowerCase().includes(String(orgFilter).toLowerCase())) continue;
      const tsMs = (ev?.startTimestamp || 0) * 1000;
      out.push({
        id: `sofa_mma_${eid}`,
        game: 'mma',
        team1: home,
        team2: away,
        league: org,
        time: tsMs > 0 ? new Date(tsMs).toISOString() : null,
        status: statusType === 'inprogress' ? 'live' : 'upcoming',
        weightClass: ev?.tournament?.name || null,
        _source: 'sofascore',
        _eventId: eid,
      });
    }
    // 2026-05-15 audit P2: removido setTimeout(100ms) redundante. fetchFirstJson
    // já chama throttleHost('sofascore', SOFA_MIN_GAP_MS=200ms default) por URL.
    // 14-day scan economiza 14×100ms = 1.4s wall time sem perda de throttle.
  }
  // 2026-05-23 audit: visibility quando proxy retorna 200 mas collectEvents=0.
  // logSofaNon200 throttle 1h só dispara em status≠200; este caso (200-empty) era
  // silent total → MMA-DISCOVERY rawCount=0 sem err visível. Loga top-level keys
  // do payload pra surfacear schema drift (homeTeam→? rename) OR proxy stub vazio.
  // Throttle 1h por sport pra não spammar.
  if (!out.length && _daysWithJson > 0 && _lastJson) {
    try {
      global._sofaEmptyLogged = global._sofaEmptyLogged || new Map();
      const _last = global._sofaEmptyLogged.get('mma') || 0;
      if (Date.now() - _last > 60 * 60 * 1000) {
        global._sofaEmptyLogged.set('mma', Date.now());
        const _keys = (_lastJson && typeof _lastJson === 'object')
          ? Object.keys(_lastJson).slice(0, 10).join(',')
          : `(type=${typeof _lastJson})`;
        const _ev = _lastJson?.events;
        const _evField = _ev !== undefined
          ? `events=${Array.isArray(_ev) ? _ev.length : typeof _ev}`
          : 'no .events key';
        log('WARN', 'SOFA-MMA', `proxy 200 mas 0 events em ${_daysWithJson}/${days.length} dias — keys=[${_keys}] | ${_evField} — provável schema drift OR proxy stub (verificar /admin/sofascore-proxy-health)`);
      }
    } catch (_) {}
  }
  return out;
}

module.exports = { enrichMatch, lookupOrg, namesMatch, fetchUpcomingFights };

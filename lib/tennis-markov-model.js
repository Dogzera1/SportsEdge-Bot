...
function _simMatch(pA, pB, bestOf) {
  ...
  while (sA < setsToWin && sB < setsToWin) {
    ...
    // 2026-05-07 (audit P2): regra real ATP/WTA — primeiro servidor do próximo
    // set é quem NÃO sacou o último game do set anterior. Antes alternava sempre,
    // o que é wrong half the time (sets que terminam em par 6-4/7-5/6-2 = mesmo
    // servidor abre próximo set; ímpar 6-3/7-6 = alterna).
    // Derivação: A starts game 1, alterna game-a-game. Server do último game N:
    //   N par → B (último foi de B) → próximo set A starts (sem alternar)
    //   N ímpar → A (último foi de A) → próximo set B starts (alterna)
    const totalSetGames = r.gamesA + r.gamesB;
    if (totalSetGames % 2 === 1) serveAStartsSet = !serveAStartsSet;
    // even → mantém serveAStartsSet (mesmo player abre próximo set)
  }

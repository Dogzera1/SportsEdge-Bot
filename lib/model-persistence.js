...
function syncFromPersistentToLib(opts = {}) {
  ...
  for (const name of files) {
    ...
    try {
      ...
      if (fs.existsSync(dst)) {
        ...
        // Só sobrescreve se persistent é mais recente (proteção contra rollback
        // acidental quando dev pushou um seed mais novo via git).
  ...
  log('INFO', 'MODEL-PERSIST', `boot sync: ${copied} arquivo(s) overlay de ${dir} (${skipped} skipped/older)`);

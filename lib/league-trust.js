...
function getLeagueTrust(db, sport, league, market = null, opts = {}) {
  ...
  try {
    const shadowRow = db.prepare(`
      SELECT
        COUNT(*) AS n,
        SUM(CASE WHEN result='win' THEN COALESCE(stake_units,1) * (odd-1) ELSE 0 END)
        - SUM(CASE WHEN result='loss' THEN COALESCE(stake_units,1) ELSE 0 END) AS profit,
      ...
      WHERE sport = ?
        ...
        ${market ? 'AND market = ?' : ''}
    `).get(cleanSport, cleanLeague, windowDays, ...(market ? [String(market)] : []));
    if (shadowRow) {
      stats.n += shadowRow.n || 0;
      stats.profit += shadowRow.profit || 0;
      stats.staked += shadowRow.staked || 0;
    }
  } catch (_) {}

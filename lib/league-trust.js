 ...
 *
 * Dados: tips (ML real, is_shadow=0). Métrica = profit/staked em unidades.
 * Esfaque por (sport, liga, market_type). Opt-out LEAGUE_TRUST_REAL_ONLY=false
 * mistura market_tips_shadow no pool (legacy, viola P2 — não recomendado).
 *
 * Buckets:
...
function getLeagueTrust(db, sport, league, market = null, opts = {}) {
  ...
  // 2026-05-07 FIX P2 (shadow=causa, real=sintoma): default true. Trust ratio
  // multiplicava stake real (applyTrustToStake em bot.js:5511-5513) usando pool
  // que combinava tips real (com is_shadow=0 ✅) + market_tips_shadow (sem filtro).
  // Shadow ROI estruturalmente menor (low-EV bucket) derrubava trust de liga real
  // boa. Comentário linha 31-33 reconheceu o bug em parte, mas só corrigiu tips.
  // Quando true: skipa o segundo SELECT em market_tips_shadow.
  // Opt-out: LEAGUE_TRUST_REAL_ONLY=false (legacy, mistura shadow+real).
  const realOnly = !/^(0|false|no)$/i.test(String(process.env.LEAGUE_TRUST_REAL_ONLY ?? 'true'));
  ...
  // 2026-05-07 (P2 fix): bloco shadow gated por LEAGUE_TRUST_REAL_ONLY (default true).
  // Quando true, trust ratio fica 100% baseado em tips real ML — shadow não contamina
  // o multiplier que afeta stake real via applyTrustToStake.
  if (!realOnly) {
    try {

 ...
 *
 * Fluxo (cron 12h):
 *   1. Avalia (sport, market) granular nos últimos N dias (default 30).
 ...
 *   MT_AUTO_PROMOTE_WINDOW_DAYS_<SPORT>  — narrows PROMOTE window pra sport (ex:
 *     LOL=7 vs default 30). Aplica só em PROMOTE eval.
 *     REVERT/LEAGUE usam janelas próprias (revertDays/days) sem A.
 *   MT_AUTO_PROMOTE_EVAL_SINCE_<SPORT>   — ISO date (ex: '2026-05-14') que corta
 *     todas tips anteriores pra esse sport em TODAS as paths. Cross-cutting.
 *     Mais restritivo ganha vs WINDOW_DAYS.
 ...
 *   MT_AUTO_PROMOTE_MIN_SETTLED_<SPORT>  — override MIN_SETTLED pra sport
 *     (ex: LOL=50 / SNOOKER=20). Default global 150. REVERT usa Math.min(N, 20).
 *   MT_AUTO_PROMOTE_MIN_SETTLED_<SPORT>_<MARKET>  — override mais granular ainda
 *     pra par (sport, market) específico (ex: LOL_HANDICAP_GAMES=30). Hierarchy:
 *     pair > sport > global. Útil pra mercados com volume diferente do sport agregado.
...
// model state diferente não refletem comportamento atual; cortar influência
// delas em auto-promote per-sport sem mexer na janela global).
//
// applyWindowOverride=true → MT_AUTO_PROMOTE_WINDOW_DAYS_<SPORT> narrows defaultDays
// applyWindowOverride=false → só MT_AUTO_PROMOTE_EVAL_SINCE_<SPORT> aplica
// Mais restritivo (cutoff mais recente) sempre ganha. Retorna formato SQLite
// 'YYYY-MM-DD HH:MM:SS' (UTC).
function _hasSinceForSport(sport) {
  const sinceStr = process.env[`MT_AUTO_PROMOTE_EVAL_SINCE_${String(sport || '').toUpperCase()}`];
  if (!sinceStr) return false;
  return !Number.isNaN(new Date(sinceStr).getTime());
}

function _effectiveCutoffISO(sport, defaultDays, opts = {}) {
  const { applyWindowOverride = true } = opts;
  const upper = String(sport || '').toUpperCase();
  const effDays = applyWindowOverride
    ? _envInt(`MT_AUTO_PROMOTE_WINDOW_DAYS_${upper}`, defaultDays)
    : defaultDays;
  let cutoffMs = Date.now() - effDays * 86400000;
  const sinceStr = process.env[`MT_AUTO_PROMOTE_EVAL_SINCE_${upper}`];
  if (sinceStr) {
  ...
  return new Date(cutoffMs).toISOString().replace('T', ' ').replace(/\.\d{3}Z$/, '');
}
...
//
// 2026-05-17: refactored pra per-sport loop com cutoff específico (envs
// MT_AUTO_PROMOTE_WINDOW_DAYS_<SPORT> + EVAL_SINCE_<SPORT>). Overhead
// ~11 queries × <0.5ms = <6ms total (better-sqlite3 sync).
//
...
function _statsBySport(db, defaultDays, opts = {}) {
  ...
  // 2026-05-17: sports com EVAL_SINCE_<SPORT> setado bypassam holdout
  // (user explicitou walk-forward; senão interseção vazia).
...
// 2026-05-17: per-sport cutoff. REVERT usa applyWindowOverride=false default
// (callsite passa). Apenas EVAL_SINCE_<SPORT> aplica aqui.
function _statsBySportReal(db, defaultDays, opts = {}) {
...
// 2026-05-17: per-sport cutoff (B only — LEAGUE precisa janela longa pra sample).
function _statsBySportLeague(db, defaultDays, opts = {}) {
  const holdoutClause = require('./frozen-holdout').getHoldoutSql('mt_auto_promote', 'created_at');
  const BODY = `
...
// 2026-05-17: per-sport cutoff (B only — LEAGUE precisa janela longa).
function _statsBySportLeagueReal(db, defaultDays, opts = {}) {
  const holdoutClause = require('./frozen-holdout').getHoldoutSql('mt_auto_promote', 'mts.created_at');
  const BODY = `
...
// 2026-05-17 Phase 1.3 — Aggrega shadow per (sport, market) pra PROMOTE granular
// (handicap_games separado de handicap_sets etc). Shadow path — research universe.
...
// 2026-05-17 Phase 1.4 — Real path com JOIN tips (real, is_shadow=0).
// REVERT/LEAGUE decision usa esta path (sintoma — sport JÁ promovido).
...
async function runMtAutoPromoteCycle(db, opts = {}) {
  ...
  // Decide per (sport, market) ao invés de sport agregado. Permite promover
  // lol/HANDICAP_GAMES sem promover lol/HANDICAP_SETS se um tem edge e outro não.
  // State persistido em mt_market_promote_state via setMtMarketPromote.
  // Consumer (bot.js) lê via isMtMarketPromoted(sport, market).
  try {
    ...
    for (const r of sportMarketRows) {
      ...
      // 2026-05-17 Phase 1.7: hierarchy MT_AUTO_PROMOTE_MIN_SETTLED_<SPORT>_<MARKET>
      // > MT_AUTO_PROMOTE_MIN_SETTLED_<SPORT> > global default 150. Permite tunar
      // threshold por pair específico (ex: LOL_HANDICAP_GAMES=30, LOL_HANDICAP_SETS=80).
      const sportUp = sport.toUpperCase();
      const marketKey = String(market).toUpperCase().replace(/[^A-Z0-9]/g, '_');
      const minSettledForSport = _envInt(
        `MT_AUTO_PROMOTE_MIN_SETTLED_${sportUp}_${marketKey}`,
      ...
      if (!promoted && settled >= minSettledForSport && roi != null && roi >= minRoi && ciPasses) {
        ...
        log('INFO', 'MT-AUTO-PROMOTE', `${sport}/${market}: SKIP (IC inclui zero) ROI=${roi.toFixed(1)}%${ciStr} n=${settled}`);
      } else if (promoted) {
  ...
  try {
    // applyWindowOverride=false → LEAGUE precisa janela longa pra ter sample
    // por liga; A narrows demais. Só EVAL_SINCE_<SPORT> aplica.
    const leagueRows = realOnly
      ? _statsBySportLeagueReal(db, days, { applyWindowOverride: false })
      : _statsBySportLeague(db, days, { applyWindowOverride: false });
  ...
  for (const sp of SPORTS) {
    ...
    const winDaysOverride = process.env[`MT_AUTO_PROMOTE_WINDOW_DAYS_${up}`] || null;
    const sinceOverride = process.env[`MT_AUTO_PROMOTE_EVAL_SINCE_${up}`] || null;
    const minSettledOverride = process.env[`MT_AUTO_PROMOTE_MIN_SETTLED_${up}`] || null;
    if (winDaysOverride || sinceOverride || minSettledOverride) {
      const cutoffIso = _effectiveCutoffISO(sp, days, { applyWindowOverride: true });
      perSportOverrides[sp] = {
        window_days: winDaysOverride,
        eval_since: sinceOverride,
        min_settled: minSettledOverride,
        effective_cutoff_iso: cutoffIso,
      };
    }
  }
  return {
    ok: true,
    decisions,
    totals: { ...Object.fromEntries(Object.entries(decisions).map(([k, v]) => [k, v.length])) },
    config: {
      window_days: days,

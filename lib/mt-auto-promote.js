...
async function runMtAutoPromoteCycle(db, opts = {}) {
  ...
  try {
    ...
    // applyWindowOverride=true → MT_AUTO_PROMOTE_WINDOW_DAYS_<SPORT> ativo aqui.
    const sportRows = _statsBySport(db, days, { applyWindowOverride: true });
    ...
    // applyWindowOverride=false → REVERT tem janela própria (revertDays).
    // Só EVAL_SINCE_<SPORT> aplica aqui (cross-cutting).
    const revertRows = realOnly
      ? _statsBySportReal(db, revertDays, { applyWindowOverride: false })
      : _statsBySport(db, revertDays, { applyWindowOverride: false });
    ...
    for (const r of sportRows) {
      ...
      // 2026-05-17: per-sport MIN_SETTLED override. Default global 150;
      // user pode setar MT_AUTO_PROMOTE_MIN_SETTLED_<SPORT> (ex: 50 alto / 20
      // baixo volume). REVERT continua min(N, 20) pra baixar threshold revert.

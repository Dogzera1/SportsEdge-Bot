...
async function runMtAutoPromoteCycle(db, opts = {}) {
  ...
  const minSettled = _envInt('MT_AUTO_PROMOTE_MIN_SETTLED', 30);
  const minRoi = _envFloat('MT_AUTO_PROMOTE_MIN_ROI', 2);
  const revertDays = _envInt('MT_AUTO_PROMOTE_REVERT_DAYS', 14);
  const revertRoi = _envFloat('MT_AUTO_PROMOTE_REVERT_ROI', -5);
  ...
  const decisions = { promoted: [], reverted: [], league_blocked: [], league_unblocked: [] };
  ...
  try {
    ...
    for (const r of sportRows) {
      ...
      if (!promoted && settled >= minSettled && roi != null && roi >= minRoi) {
        _setMtPromoteEnv(db, sport, true);
        _logDecision(db, { sport, action: 'promote', reason: `auto: ROI ${roi.toFixed(1)}% n=${settled}`, n: settled, roi, clv });
        decisions.promoted.push({ sport, roi: +roi.toFixed(1), n: settled, clv: clv != null ? +clv.toFixed(1) : null });
        log('INFO', 'MT-AUTO-PROMOTE', `${sport}: PROMOVIDO ROI=${roi.toFixed(1)}% n=${settled}`);
      } else if (promoted) {

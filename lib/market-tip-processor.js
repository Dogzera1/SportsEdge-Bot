...
function shouldSendMarketTip(tip, ctx = {}) {
  ...
  if (Number.isFinite(tip.odd) && tip.odd > 0 && tip.odd < minOddResolved) {
    return { ok: false, reason: `odd ${tip.odd.toFixed(2)} < ${minOddResolved.toFixed(2)} (floor)` };
  }
  ...
  if (Number.isFinite(tip.pModel) && tip.pModel > _pmCap) {
    return { ok: false, reason: `pModel ${(tip.pModel * 100).toFixed(1)}% > ${(_pmCap * 100).toFixed(1)}% (ceiling — provável overconfidence em mercado não-calibrado)` };
  }

...
function buildMarketTipDM({ match, tip, stake, league, sport, isLive = false, markets = null, kellyFrac = null }) {
  ...
  return `${emoji} *MARKET TIP* — ${sportLabel}${liveTag}\n\n` +
    ...
    `💰 Stake: *${stake}u* (Kelly ${(function _kellyLabel() {
      if (Number.isFinite(kellyFrac) && kellyFrac > 0) {
        return kellyFrac.toFixed(3).replace(/0+$/, '').replace(/\.$/, '');
      }
      // Reverse-engineer effective Kelly fraction from stake (inclui sport/tier/EV throttle mults).
      // stake_units = fullKelly × effFrac × bankrollUnits(100). Resolve effFrac = (stakeU / 100) / fullKelly.
      try {
        const p = Number(tip?.pModel), o = Number(tip?.odd);
        const stakeU = parseFloat(String(stake).replace(/u$/i, '').trim());
        if (p > 0 && p < 1 && o > 1 && Number.isFinite(stakeU) && stakeU > 0) {
          const fullK = (p * (o - 1) - (1 - p)) / (o - 1);
          if (fullK > 0) {
            const eff = (stakeU / 100) / fullK;
            return eff.toFixed(3).replace(/0+$/, '').replace(/\.$/, '');
          }
        }
      } catch (_) {}
      return '0.10';

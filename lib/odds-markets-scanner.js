 ...
 */
function scanMarkets({ markets, pMap, bestOf, pricingLib, minEv = 4, momentum = 0, minOdd, maxOdd, maxPerMatch } = {}) {
  if (!markets || !pricingLib) return [];
  const swap = !!markets.swap;
  const tips = [];
  const pricingOpts = momentum > 0 ? { momentum, iters: 8000 } : {};
  // Odd floor/cap gates — caller passa explicito; sem default aqui porque
  // perfil per-sport varia (LoL sofre em >2.0, tennis em <1.5). Caller lê env
  // sport-specific. `null`/`undefined`/`0` = sem gate naquele lado.
  ...
  for (const h of (markets.handicaps || [])) {
    ...
    if (ev1 != null && ev1 >= minEv && oddOk(oddT1)) {
      tips.push({
    ...
    if (ev2 != null && ev2 >= minEv && oddOk(oddT2)) {
      tips.push({
  ...
  for (const t of (markets.totals || [])) {
    ...
    if (evO != null && evO >= minEv && oddOk(t.oddsOver)) {
      tips.push({
    ...
    if (evU != null && evU >= minEv && oddOk(t.oddsUnder)) {
      tips.push({
  ...
  // Per-match cap: top-K por EV após dedup. Markov/Poisson costuma precificar
  // múltiplos mercados na mesma direção (handicap + totals refletem mesmo
  // sinal "match longo" ou "team forte"); cap reduz correlação cruzada
  // sem perder o tip de maior edge. Caller passa explicit; default null = sem cap.
  if (Number.isFinite(maxPerMatch) && maxPerMatch > 0 && result.length > maxPerMatch) {

 ...
 */
function scanMarkets({ markets, pMap, bestOf, pricingLib, minEv = 4, momentum = 0, minOdd, maxOdd, maxPerMatch, shadowMinEv, calibLib, calibOpts, score1, score2 } = {}) {
  ...
  // 2026-05-12: live-aware pricing — propaga score atual pra pricingLib
  // (eg cs-live-pricing filtra dist por placar parcial). Sport pre-game
  // ignora silently (lol-markets dist primitives não usam score1/2).
  if (Number.isFinite(score1) && score1 >= 0) pricingOpts.score1 = score1;
  if (Number.isFinite(score2) && score2 >= 0) pricingOpts.score2 = score2;
  ...
  for (const h of (markets.handicaps || [])) {
    ...
    // 2026-05-19 audit P3 mig 117 plumb: pModelRaw é prob PRE-calib pra
    // habilitar brier-holdout-eval medir overfit do isotonic FIT cycle (não
    // combined model). logShadowTip aceita pModelRaw (commit 3692425 fix nullish
    // chain). pModel continua post-calib (back-compat com consumers stake/EV).
    _pushTip({
      market: 'handicap', line: lineT1, side: 'team1',
      pModel: +pT1Calib.toFixed(4),
      pModelRaw: +probs.team1.toFixed(4),
    ...
    _pushTip({
      ...
      pModelRaw: +probs.team2.toFixed(4),

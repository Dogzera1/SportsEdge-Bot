...
function readBotMemSnapshot() {
  try {
    ...
    // 2026-05-17: clamp em 0 — Windows mtime resolution + clock drift ocasionalmente
    // produzem mtime levemente futuro (~1-50ms à frente de Date.now()) quando o
    // write é imediatamente seguido de stat. Negative ageMs é nonsense semanticamente
    // (tip não pode ser "do futuro") e quebra test flakily em Windows.
    return {
      ...content,
      _ageMs: Math.max(0, Date.now() - stat.mtime.getTime()),
    };
  } catch (_) {
    return null;
  }

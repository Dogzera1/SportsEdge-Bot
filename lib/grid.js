...
/** Cooldown após "Requester forbidden" / sem permissão — evita spam de HTTP e WARN por partida */
let _gridForbiddenUntil = 0;
let _gridForbiddenBannerLogged = false;
const GRID_FORBIDDEN_COOLDOWN_MS = Math.max(
  10 * 60 * 1000,
  parseInt(process.env.GRID_FORBIDDEN_COOLDOWN_MS || String(6 * 60 * 60 * 1000), 10) || 6 * 60 * 60 * 1000
);

function _gridErrorIsForbidden(msg) {
  const s = String(msg || '').toLowerCase();
  return /forbidden|not authorized|unauthorized|permission denied|access denied|insufficient scope/.test(s);
}

function _tripGridForbidden(reason) {
  _gridForbiddenUntil = Date.now() + GRID_FORBIDDEN_COOLDOWN_MS;
  if (!_gridForbiddenBannerLogged) {
    _gridForbiddenBannerLogged = true;
    const h = Math.round(GRID_FORBIDDEN_COOLDOWN_MS / 3600000);
    log(
      'WARN',
      'GRID',
      `Query recusada pela API (${String(reason).slice(0, 120)}). Enrich GRID pausado ~${h}h — típico: Open Access sem LoL ou chave sem Central Data. Contate GRID ou defina LOL_GRID_ENRICH=false no bot.`
    );
  }
}
...
async function _gridCentral(apiKey, query, variables) {
  ...
  if (r.status === 401 || r.status === 403) {
    _tripGridForbidden(`HTTP ${r.status}`);
    return { ok: false, error: `HTTP ${r.status}`, forbidden: true };
  }
  if (r.status !== 200 || !r.json) {
    return { ok: false, error: `HTTP ${r.status}`, raw: r.raw };
  }
  if (r.json.errors?.length) {
    const errMsg = r.json.errors.map((e) => e.message).join('; ');
    if (_gridErrorIsForbidden(errMsg)) _tripGridForbidden(errMsg);
    return { ok: false, error: errMsg, forbidden: _gridErrorIsForbidden(errMsg) };
  }
...
async function _fetchAllSeriesPages(apiKey, gteIso, lteIso, titleId, maxPages, first) {
  ...
  for (let p = 0; p < maxPages; p++) {
    ...
    // Não repetir query sem title se já foi forbidden — evita 2× WARN e 2× quota
    if (!res.ok && titleId != null && !res.forbidden) {
      const res2 = await _gridCentral(apiKey, queryNoTitle, { gte: gteIso, lte: lteIso, first, after });
      res = { ...res2, forbidden: !!(res.forbidden || res2.forbidden) };
    }
    if (!res.ok) {
      if (!res.forbidden) log('WARN', 'GRID', `allSeries falhou: ${res.error}`);
      break;
    }
...
async function fetchGridEnrichForMatch(apiKey, team1, team2, opts = {}) {
  if (Date.now() < _gridForbiddenUntil) {
    return {
      ok: false,
      source: 'grid',
      h2h: null,
      ...
      seriesListed: 0,
      stateCalls: 0,
      error: 'forbidden_cooldown',
      reason: 'API sem permissão — cooldown ativo (sem novas chamadas)',
    };
  }
  ...
  if (!edges.length) {
    const out = {
      ...empty,
      error: Date.now() < _gridForbiddenUntil ? 'forbidden_cooldown' : 'nenhuma série no período',
    };
  ...
  _gridForbiddenBannerLogged = false;
  _gridEnrichCache.set(cacheKey, out);

...
const SPORTS = {
  ...
  cs: {
    ...
    icon: '🔫',
    // Novo esporte (abr/2026). Começa em shadow para validar antes de DM.
    enabled: process.env.CS_ENABLED === 'true' && !!process.env.TELEGRAM_TOKEN_CS,
    token: process.env.TELEGRAM_TOKEN_CS,
    games: ['cs'],
    // Default EM shadow até validar
    shadowMode: process.env.CS_SHADOW !== 'false'
  },
  valorant: {
    id: 'valorant',
    name: 'Valorant',
    icon: '🎯',
    // Novo esporte (abr/2026). Fork da estrutura CS2.
    // Compartilha o bot Telegram do CS (TELEGRAM_TOKEN_CS) por padrão — evita
    // criar outro bot. Se setar TELEGRAM_TOKEN_VALORANT, usa token dedicado.
    ...
    games: ['valorant'],
    // Default EM shadow. Graduar setando VALORANT_SHADOW=false.
    shadowMode: process.env.VALORANT_SHADOW !== 'false'
  },
  tabletennis: {
    id: 'tabletennis',
    name: 'Tênis de Mesa',

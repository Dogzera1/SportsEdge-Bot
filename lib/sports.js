...
const SPORTS = {
  ...
  darts: {
    ...
    games: ['darts'],
    // 2026-05-12: DEFAULT shadow ON (opt-out via DARTS_SHADOW=false). Memory
    // project_audit_2026_05_11 listava DARTS_SHADOW_ONLY=true como pending P0-5;
    // ROI real 30d = -100% n=3 confirma necessidade. Voltar pra real explicito
    // requer DARTS_SHADOW=false após shadow estabilizar com n≥30 + CLV≥0%.
  ...
  basket: {
    ...
    enabled: process.env.BASKET_ENABLED !== 'false' && !!(process.env.TELEGRAM_TOKEN_BASKET || process.env.TELEGRAM_TOKEN_FOOTBALL || process.env.TIPS_UNIFIED_TOKEN),

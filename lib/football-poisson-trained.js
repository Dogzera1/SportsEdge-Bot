...
function predictFootball({ teamHome, teamAway, league }) {
  ...
  const formFactor = (ppg) => 1 + (ppg - 1.5) * 0.133; // +0.2 at ppg=3, -0.2 at ppg=0
  ...
  // Home team bom form boosta seu attack e reduz vulnerabilidade defensiva
  const attackHomeAdj = th.team.attack_home * hForm;
  const defenseHomeAdj = th.team.defense_home * (2 - hForm); // bom form → menos gols sofridos
  const attackAwayAdj = ta.team.attack_away * aForm;
  const defenseAwayAdj = ta.team.defense_away * (2 - aForm);
  const lamH = lp.avg_home_goals * attackHomeAdj * defenseAwayAdj;
  const lamA = lp.avg_away_goals * attackAwayAdj * defenseHomeAdj;
  ...
  return {
    ...
    source: 'trained_poisson_v2_form',

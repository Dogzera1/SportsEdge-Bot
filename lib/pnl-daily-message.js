...
function buildPnlDailyMessage(db, opts = {}) {
  ...
  // 2026-05-21: cutoff opcional pra ignorar tips pré-reset.
  ...
  // Env: PNL_DAILY_CUTOFF_DATE=YYYY-MM-DD (vazio = sem cutoff, backward compat).
  const cutoffDateRaw = String(opts.cutoffDate || process.env.PNL_DAILY_CUTOFF_DATE || '').trim();
  const cutoffDate = /^\d{4}-\d{2}-\d{2}$/.test(cutoffDateRaw) ? cutoffDateRaw : null;

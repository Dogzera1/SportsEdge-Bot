'use strict';

// Builder do digest PnL diário (hoje + 7d + 30d + total).
// BRT-aware (DATE(settled_at, '-3 hours')) — alinha com dia BRT, não UTC.
// Usado pelo cron runPnlDaily (bot.js) e pelo endpoint admin manual.

function buildPnlDailyMessage(db, opts = {}) {
  const skipToday = !!opts.skipToday;
  const skipSummary = !!opts.skipSummary;
  const brtNow = new Date(Date.now() - 3 * 60 * 60 * 1000);
  const today = brtNow.toISOString().slice(0, 10);
  const lines = [];
  const stats = { today_n: 0, today_profit: 0, today_stake: 0, past7d_days: 0, sum30d_n: 0, sumAll_n: 0 };

  // 2026-05-21: cutoff pra ignorar tips pré-reset.
  // Pré-reset 14/05 incluía tips de períodos com config velha (1u=R$10 inflado
  // antes do rescale + leaks calibration antigos). User: "só REAIS desde reset".
  //
  // Resolução hierárquica:
  //   1. opts.cutoffDate (caller explícito)
  //   2. PNL_DAILY_CUTOFF_DATE env (YYYY-MM-DD)
  //   3. Auto-detect: MIN(settled_at) das tips REAIS não-archived
  //      = primeira tip post-reset (tips pré-reset foram archived=1).
  //   4. Fallback: null (sem cutoff, mostra tudo)
  const cutoffDateRaw = String(opts.cutoffDate || process.env.PNL_DAILY_CUTOFF_DATE || '').trim();
  let cutoffDate = /^\d{4}-\d{2}-\d{2}$/.test(cutoffDateRaw) ? cutoffDateRaw : null;
  if (!cutoffDate) {
    try {
      const auto = db.prepare(`
        SELECT MIN(DATE(settled_at, '-3 hours')) AS first_dt
          FROM tips
         WHERE COALESCE(is_shadow, 0) = 0
           AND COALESCE(archived, 0) = 0
           AND result IN ('win', 'loss', 'void')
      `).get();
      if (auto?.first_dt && /^\d{4}-\d{2}-\d{2}$/.test(auto.first_dt)) {
        cutoffDate = auto.first_dt;
      }
    } catch (_) { /* defensive — fallback null = no cutoff */ }
  }
  // Header decidido depois (precisa do past7Rows). Placeholder at idx 0.
  let headerIdx = 0;

  if (skipToday) {
    lines.push(`📊 *Resumo PnL — Tips Reais* — ${today}`, '');
  } else {
    lines.push(`🌙 *Lucro de Hoje — Tips Reais* — ${today}`, '');

    // 2026-05-28: ML profit normalizado de BRL → unidades. tips.profit_reais é
    // gravado em R$ (stake_reais × (odds-1)) com unit value tier-aware (lib/sport-unit).
    // Quando bankroll cruza tier (ratio ≥1.20 ou ≤0.80), 1u ≠ R$1 → mistura com MT
    // profit_units (sempre em unidades) e label "u" no DM viravam unit-mismatch.
    // Conversão per-tip: profit_units = profit_reais × stake_units / stake_reais.
    // Fallback (tips legacy pré-mig 026 sem stake_reais): profit_reais já é em
    // unidades pq stakeR foi computado com uv=R$1.00 (initial==initial em settle).
    const mlRows = db.prepare(`
      SELECT sport, result,
             CASE
               WHEN COALESCE(stake_reais, 0) > 0
                AND CAST(REPLACE(REPLACE(stake, 'u', ''), 'U', '') AS REAL) > 0
               THEN COALESCE(profit_reais, 0)
                  * CAST(REPLACE(REPLACE(stake, 'u', ''), 'U', '') AS REAL)
                  / stake_reais
               ELSE COALESCE(profit_reais, 0)
             END AS profit_units,
             CAST(REPLACE(REPLACE(stake, 'u', ''), 'U', '') AS REAL) AS stake_units
        FROM tips
       WHERE COALESCE(is_shadow, 0) = 0
         AND COALESCE(archived, 0) = 0
         AND result IN ('win', 'loss', 'void')
         AND DATE(settled_at, '-3 hours') = ?
    `).all(today);

    const mtRows = db.prepare(`
      SELECT sport, result,
             COALESCE(profit_units, 0) AS profit_units,
             COALESCE(stake_units, 0) AS stake_units
        FROM market_tips_shadow
       WHERE result IN ('win', 'loss', 'void')
         AND admin_dm_sent_at IS NOT NULL
         AND DATE(settled_at, '-3 hours') = ?
    `).all(today);

    const bySport = {};
    let totW = 0, totL = 0, totV = 0, totProfit = 0, totStake = 0;
    for (const r of mlRows) {
      const s = r.sport || '?';
      bySport[s] = bySport[s] || { mlN: 0, mlP: 0, mtN: 0, mtP: 0, w: 0, l: 0, v: 0 };
      bySport[s].mlN++;
      bySport[s].mlP += r.profit_units || 0;
      totProfit += r.profit_units || 0;
      totStake += r.stake_units || 0;
      if (r.result === 'win') { bySport[s].w++; totW++; }
      else if (r.result === 'loss') { bySport[s].l++; totL++; }
      else if (r.result === 'void') { bySport[s].v++; totV++; }
    }
    for (const r of mtRows) {
      const s = r.sport || '?';
      bySport[s] = bySport[s] || { mlN: 0, mlP: 0, mtN: 0, mtP: 0, w: 0, l: 0, v: 0 };
      bySport[s].mtN++;
      bySport[s].mtP += r.profit_units || 0;
      totProfit += r.profit_units || 0;
      totStake += r.stake_units || 0;
      if (r.result === 'win') { bySport[s].w++; totW++; }
      else if (r.result === 'loss') { bySport[s].l++; totL++; }
      else if (r.result === 'void') { bySport[s].v++; totV++; }
    }
    const totSettled = totW + totL + totV;
    const roi = totStake > 0 ? (totProfit / totStake * 100) : 0;
    stats.today_n = totSettled;
    stats.today_profit = totProfit;
    stats.today_stake = totStake;
    if (totSettled > 0) {
      lines.push(`📊 ${totSettled} liquidadas (${totW}W ${totL}L ${totV}V)`);
      lines.push(`💰 *${totProfit >= 0 ? '+' : ''}${totProfit.toFixed(2)}u* (${totProfit >= 0 ? '+' : ''}${roi.toFixed(1)}% ROI · ${totStake.toFixed(1)}u stake)`);
    } else {
      lines.push('_Nenhuma tip liquidada hoje._');
    }

    const sportsSorted = Object.entries(bySport).sort((a, b) => (b[1].mlP + b[1].mtP) - (a[1].mlP + a[1].mtP));
    if (sportsSorted.length > 0) {
      lines.push('', '*Por sport:*');
      for (const [s, d] of sportsSorted) {
        const c = d.mlN + d.mtN;
        const p = d.mlP + d.mtP;
        if (c === 0) continue;
        const parts = [];
        if (d.mlN > 0) parts.push(`ML ${d.mlN}`);
        if (d.mtN > 0) parts.push(`MT ${d.mtN}`);
        const emoji = p > 0 ? '🟢' : p < 0 ? '🔴' : '⚪';
        lines.push(`${emoji} ${s}: ${parts.join(' + ')} (${d.w}W${d.l}L${d.v}V) → ${p >= 0 ? '+' : ''}${p.toFixed(2)}u`);
      }
    }
  }

  // ── Últimos 7 dias (incluindo hoje), BRT-aware ──
  // 2026-05-21: user reportou que total 7d não contava tips de hoje. Antes query
  // tinha `AND date < today` (exclusivo) + `>= today-7 days` = janela 7d ANTERIOR.
  // Fix: `>= today-6 days AND <= today` = 7 dias incluindo hoje (today + 6 prévios).
  // Reflete acumulado real do período cobrindo o dia em curso.
  // Cutoff defensivo: se reset date > today-6 days, narrow automaticamente.
  const past7CutoffClause = cutoffDate ? ` AND DATE(settled_at, '-3 hours') >= ?` : '';
  const past7Params = cutoffDate
    ? [today, today, cutoffDate, today, today, cutoffDate]
    : [today, today, today, today];
  // 2026-05-28: profit_reais (BRL) convertido pra unidades inline (vide ML query
  // acima). Sem isso, soma com MT profit_units mistura escalas quando uv≠R$1.
  const past7Rows = db.prepare(`
    WITH unified AS (
      SELECT DATE(settled_at, '-3 hours') AS d,
             CASE
               WHEN COALESCE(stake_reais, 0) > 0
                AND CAST(REPLACE(REPLACE(stake, 'u', ''), 'U', '') AS REAL) > 0
               THEN COALESCE(profit_reais, 0)
                  * CAST(REPLACE(REPLACE(stake, 'u', ''), 'U', '') AS REAL)
                  / stake_reais
               ELSE COALESCE(profit_reais, 0)
             END AS p,
             CAST(REPLACE(REPLACE(stake, 'u', ''), 'U', '') AS REAL) AS s
        FROM tips
       WHERE COALESCE(is_shadow, 0) = 0
         AND COALESCE(archived, 0) = 0
         AND result IN ('win', 'loss', 'void')
         AND DATE(settled_at, '-3 hours') >= DATE(?, '-6 days')
         AND DATE(settled_at, '-3 hours') <= ?
         ${past7CutoffClause}
      UNION ALL
      SELECT DATE(settled_at, '-3 hours') AS d,
             COALESCE(profit_units, 0) AS p,
             COALESCE(stake_units, 0) AS s
        FROM market_tips_shadow
       WHERE result IN ('win', 'loss', 'void')
         AND admin_dm_sent_at IS NOT NULL
         AND DATE(settled_at, '-3 hours') >= DATE(?, '-6 days')
         AND DATE(settled_at, '-3 hours') <= ?
         ${past7CutoffClause}
    )
    SELECT d, ROUND(SUM(p), 2) AS profit,
           ROUND(SUM(s), 2) AS stake, COUNT(*) AS n
      FROM unified
     GROUP BY d
     ORDER BY d DESC
  `).all(...past7Params);

  stats.past7d_days = past7Rows.length;

  // Header override quando ambos skip flags: usa janela real de past7Rows.
  if (skipToday && skipSummary && past7Rows.length > 0) {
    const ddmm = (d) => { const p = (d || '').split('-'); return p.length === 3 ? `${p[2]}/${p[1]}` : (d || ''); };
    const last = past7Rows[0]?.d;
    const first = past7Rows[past7Rows.length - 1]?.d;
    if (last && first) lines[headerIdx] = `📊 *Lucro Tips Reais — ${ddmm(first)} a ${ddmm(last)}*`;
  }

  if (past7Rows.length > 0) {
    lines.push('', '*Últimos 7 dias:*');
    for (const r of past7Rows) {
      const parts = (r.d || '').split('-');
      const ddmm = parts.length === 3 ? `${parts[2]}/${parts[1]}` : (r.d || '');
      const p = r.profit || 0;
      const e = p > 0 ? '🟢' : p < 0 ? '🔴' : '⚪';
      const roiDay = r.stake > 0 ? ` · ${p >= 0 ? '+' : ''}${(p / r.stake * 100).toFixed(1)}%` : '';
      lines.push(`${ddmm} ${e} ${p >= 0 ? '+' : ''}${p.toFixed(2)}u (${r.n} tip${r.n === 1 ? '' : 's'}${roiDay})`);
    }
    const sum7 = past7Rows.reduce((a, r) => ({
      p: a.p + (r.profit || 0), s: a.s + (r.stake || 0), n: a.n + (r.n || 0),
    }), { p: 0, s: 0, n: 0 });
    const e7 = sum7.p > 0 ? '🟢' : sum7.p < 0 ? '🔴' : '⚪';
    const r7 = sum7.s > 0 ? ` · ROI ${sum7.p >= 0 ? '+' : ''}${(sum7.p / sum7.s * 100).toFixed(1)}%` : '';
    lines.push(`└ ${e7} *7d: ${sum7.n} tips · ${sum7.p >= 0 ? '+' : ''}${sum7.p.toFixed(2)}u${r7}*`);
  }

  // ── Resumo do sistema: 30d + Total (BRT-aware) ──
  // 2026-05-21: cutoff aplicado em ambos sum30 e sumAll quando setado. Garante
  // que counters reflitam só período post-reset (settled_at >= cutoff).
  const cutoffClause = cutoffDate ? ` AND DATE(settled_at, '-3 hours') >= ?` : '';
  // 2026-05-28: profit_reais (BRL) convertido pra unidades inline (vide ML query
  // hoje). Mantém escala consistente com MT profit_units no UNION.
  const aggregateQuery = (whereExtra, params) => db.prepare(`
    WITH unified AS (
      SELECT CASE
               WHEN COALESCE(stake_reais, 0) > 0
                AND CAST(REPLACE(REPLACE(stake, 'u', ''), 'U', '') AS REAL) > 0
               THEN COALESCE(profit_reais, 0)
                  * CAST(REPLACE(REPLACE(stake, 'u', ''), 'U', '') AS REAL)
                  / stake_reais
               ELSE COALESCE(profit_reais, 0)
             END AS p,
             CAST(REPLACE(REPLACE(stake, 'u', ''), 'U', '') AS REAL) AS s,
             result, settled_at
        FROM tips
       WHERE COALESCE(is_shadow, 0) = 0
         AND COALESCE(archived, 0) = 0
         AND result IN ('win', 'loss', 'void')
         ${whereExtra}
         ${cutoffClause}
      UNION ALL
      SELECT COALESCE(profit_units, 0) AS p,
             COALESCE(stake_units, 0) AS s,
             result, settled_at
        FROM market_tips_shadow
       WHERE result IN ('win', 'loss', 'void')
         AND admin_dm_sent_at IS NOT NULL
         ${whereExtra}
         ${cutoffClause}
    )
    SELECT ROUND(SUM(p), 2) AS profit,
           ROUND(SUM(s), 2) AS stake,
           COUNT(*) AS n,
           SUM(CASE WHEN result = 'win' THEN 1 ELSE 0 END) AS w,
           SUM(CASE WHEN result = 'loss' THEN 1 ELSE 0 END) AS l,
           MIN(DATE(settled_at, '-3 hours')) AS first_dt
      FROM unified
  `).get(...(params || []));

  // sum30: window pode ser restringido pelo cutoff (cutoff > today-29days)
  const sum30Params = cutoffDate
    ? [today, cutoffDate, today, cutoffDate]
    : [today, today];
  const sum30 = aggregateQuery(`AND DATE(settled_at, '-3 hours') >= DATE(?, '-29 days')`, sum30Params);
  const sumAllParams = cutoffDate ? [cutoffDate, cutoffDate] : [];
  const sumAll = aggregateQuery('', sumAllParams);

  stats.sum30d_n = sum30?.n || 0;
  stats.sumAll_n = sumAll?.n || 0;

  const summaryLines = [];
  const fmtSum = (label, row, sinceTag) => {
    if (!row || !row.n) return null;
    const p = row.profit || 0;
    const s = row.stake || 0;
    const e = p > 0 ? '🟢' : p < 0 ? '🔴' : '⚪';
    const roiPct = s > 0 ? ` · ROI ${p >= 0 ? '+' : ''}${(p / s * 100).toFixed(1)}%` : '';
    const settled = (row.w || 0) + (row.l || 0);
    const hit = settled > 0 ? ` · hit ${(row.w / settled * 100).toFixed(0)}%` : '';
    return `${e} ${label}: ${row.n} tips · ${p >= 0 ? '+' : ''}${p.toFixed(2)}u${roiPct}${hit}${sinceTag || ''}`;
  };
  const l30 = fmtSum('30d', sum30);
  if (l30) summaryLines.push(l30);
  const lAll = fmtSum('Total', sumAll, sumAll?.first_dt ? ` _(desde ${sumAll.first_dt})_` : '');
  if (lAll) summaryLines.push(lAll);

  if (summaryLines.length > 0 && !skipSummary) {
    lines.push('', '*Resumo do sistema:*');
    for (const ln of summaryLines) lines.push(ln);
  }

  return { msg: lines.join('\n'), stats, today };
}

module.exports = { buildPnlDailyMessage };

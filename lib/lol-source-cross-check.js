...
function hasRecentSourceDisagreement(db, team1, team2, opts = {}) {
  ...
  try {
    ...
    for (const r of rows) {
      ...
      // Match com qualquer um dos teams — sinal mais fraco mas ainda relevante
      if (t1HitB || t1HitR || t2HitB || t2HitR) {
        return {
          blocked: true,
          reason: `team ${t1HitB || t1HitR ? team1 : team2} teve game flagged ${r.gameid} (${r.date}): spread=${r.spread}`,
          evidence: r,
        };
      }
    }

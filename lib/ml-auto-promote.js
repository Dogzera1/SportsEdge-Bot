 ...
 *   ML_AUTO_PROMOTE_WINDOW_DAYS      (default 30)
...
async function runMlAutoPromoteCycle(db, opts = {}) {
  ...
  const days = _envInt('ML_AUTO_PROMOTE_WINDOW_DAYS', 30);
  ...
  try {
    ...
    const shadowBySport = _aggregate(shadowRows, r => String(r.sport || '').toLowerCase());
    const revertBySport = new Map(
      _aggregate(revertSrcRows, r => String(r.sport || '').toLowerCase())
        .map(a => [a.key, a])
    );

    for (const a of shadowBySport) {
      const sport = a.key;
      ...
      if (shadowOn && settled >= minSettled && roi != null && roi >= minRoi && ciPasses) {
      ...
      } else if (shadowOn && settled >= minSettled && roi != null && roi >= minRoi && !ciPasses) {

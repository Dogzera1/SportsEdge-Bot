 ...
 *   ML_AUTO_PROMOTE_MIN_SETTLED_<SPORT>  — override MIN_SETTLED pra sport
 *     (ex: LOL=50 / SNOOKER=20). Default global 150 calibrado pra janela ampla;
 *     sports com EVAL_SINCE recente têm sample menor — threshold proporcional.
 *     REVERT usa Math.min(N, 30) — tolera reverter com sample menor.
...
async function runMlAutoPromoteCycle(db, opts = {}) {
  ...
  try {
    ...
    for (const sport of sportsSeen) {
      ...
      // 2026-05-17: per-sport MIN_SETTLED override.
      // Default global 150 calibrado pra janela ampla; sports com EVAL_SINCE
      // recente têm sample menor — user pode setar ML_AUTO_PROMOTE_MIN_SETTLED_<SPORT>
      // (ex: 50 alto volume / 20 baixo volume). REVERT continua min(N, 30) pra
      // não-volume baixar threshold pra reverter.
      const minSettledForSport = _envInt(`ML_AUTO_PROMOTE_MIN_SETTLED_${sport.toUpperCase()}`, minSettled);
      ...
      if (shadowOn && a) {
        ...
        if (settled >= minSettledForSport && roi != null && roi >= minRoi && ciPasses && clvPasses) {
        ...
        } else if (settled >= minSettledForSport && roi != null && roi >= minRoi && !ciPasses) {
        ...
        } else if (settled >= minSettledForSport && roi != null && roi >= minRoi && ciPasses && !clvPasses) {
        ...
        if (rvSettled >= Math.min(minSettledForSport, 30) && rvRoi != null && rvRoi <= revertRoi) {
...
// Refresh in-memory cache
  ...
  for (const sp of SPORTS) {
    ...
    const minSettledOverride = process.env[`ML_AUTO_PROMOTE_MIN_SETTLED_${up}`] || null;

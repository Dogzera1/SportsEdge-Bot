 ...
 *   ML_AUTO_PROMOTE_MIN_CLV          (default -100 = bypass; set 0 ou -5 pra gate ativo)
...
async function runMlAutoPromoteCycle(db, opts = {}) {
  ...
  // ML_AUTO_PROMOTE_MIN_CLV=-5 (cap permissivo) ou =0 (estrito).
  ...
  try {
    ...
    for (const sport of sportsSeen) {
      ...
      if (shadowOn && a) {
        ...
        // com ROI = variance, não edge). clv=null bypassa (sample tiny / sem
        // captura — não penaliza). minClv=-100 default = bypass (legacy).
        const clvPasses = clv == null || clv >= minClv;
        ...
        if (settled >= minSettled && roi != null && roi >= minRoi && ciPasses && clvPasses) {
          ...
          _logDecision(db, { sport, action: 'reject_clv', reason: `CLV ${clv.toFixed(1)}% < min ${minClv}%: ROI ${roi.toFixed(1)}% mas divergence severe (variance != edge)`, n: settled, roi, clv });
          decisions.rejected_by_ci.push({ sport, roi: +roi.toFixed(1), n: settled, clv: +clv.toFixed(1), reason: 'clv_below_min' });
          log('INFO', 'ML-AUTO-PROMOTE', `${sport}: SKIP promote (CLV ${clv.toFixed(1)}% < ${minClv}% — variance != edge) ROI=${roi.toFixed(1)}% n=${settled}`);
        }
      } else if (!shadowOn) {

    ...
    try {
      ...
      const audit = (rows, suffix) => {
        ...
        for (const a of agg) {
          ...
          // Auto bucket_block: trigger só em shadow source (P2-compliant — promote
          // decision based on research universe). Real source só log.
          if (upsertBucketBlock && suffix === 'shadow' && a.settled >= bucketBlockMinN && roi <= bucketBlockRoi) {
            try {

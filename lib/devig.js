...
function devigPower(odds1, odd2) {
  ...
  // 2026-05-07 (audit P2): normaliza por overround mesmo no fast-path — antes
  // retornava `rs` cru, mas overround pode ser 0.99999998 ou 1.00000001 dentro
  // do epsilon, então probs não somavam exatamente 1. Caller que faz Σ p_i × o_i
  // pra calcular EV via sum se via off por residual numérico.
...
function devigShin(odds1, odd2) {
  ...
  // 2026-05-07 (audit P2): garante Σp = 1 exato. Se overround extremo (>1.5)
  // bisection mesmo expandindo hi=0.95 pode não convergir perfeitamente
  // (sumAt(0.95) > 1 → fallback z fora do válido). Normaliza por sum pra
  // garantir consistência downstream (EV calc, Kelly, calibration).
  const probSum = probs.reduce((a, p) => a + p, 0);

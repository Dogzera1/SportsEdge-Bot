 ...
 */
async function getMatchupHandicaps(matchupId, period) {
  const markets = await getMatchupMarkets(matchupId);
  ...
  for (const m of markets) {
    ...
    // Line: Pinnacle coloca em ph.points (home handicap). Home -1.5 → ph.points = -1.5.
    const line = Number(ph.points);
    if (!Number.isFinite(line)) continue;
    // Filter: só meio-pontos (sem push). e.g., ±1.5, ±2.5.
    if (Math.abs(line * 2) % 2 !== 1) continue;
    out.push({ line, oddsHome, oddsAway, period });
  }
  // Dedupe por linha (pode ter duplicata entre alt lines)
  const seen = new Set();
  return out.filter(r => { const k = r.line; if (seen.has(k)) return false; seen.add(k); return true; });
}
 ...
 */
async function getMatchupTotals(matchupId, period) {
  const markets = await getMatchupMarkets(matchupId);
  if (!Array.isArray(markets)) return [];
  const out = [];
  for (const m of markets) {
    ...
    out.push({ line, oddsOver, oddsUnder, period });
  }
  const seen = new Set();
  return out.filter(r => { const k = r.line; if (seen.has(k)) return false; seen.add(k); return true; });
}

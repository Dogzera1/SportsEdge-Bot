/**
 * pinnacle.js — Cliente genérico da Pinnacle Guest API (funciona do BR, sem auth real).
 *
 * Endpoint usado pelo próprio frontend pinnacle.com. X-API-Key pública.
 *
 * Sport IDs confirmados:
 *   6=Boxing, 10=Darts, 12=E-Sports, 22=MMA, 28=Snooker, 29=Soccer, 33=Tennis
 *
 * Over PINNACLE_API_KEY via env caso a key pública rotacione.
 */
'use strict';

const { safeParse, cachedHttpGet, log } = require('./utils');

const API_BASE = 'https://guest.api.arcadia.pinnacle.com';
// Pinnacle Guest API: key é PÚBLICA por design (memory
// project_risk_compliance_audit_2026_04_30). Fallback hardcoded ok — não é
// secret. Override via PINNACLE_API_KEY env se Pinnacle rotacionar.
const API_KEY = process.env.PINNACLE_API_KEY || 'CmX2KcMrXuFmNg6YFbmTxE0y9CIrOi0R';

// 2026-05-19 P3: UA centralizado em lib/utils BROWSER_UA. Headers específicos
// Pinnacle (X-API-Key, Referer) merged.
const { BROWSER_UA } = require('./utils');
const HEADERS = {
  'User-Agent': BROWSER_UA,
  'X-API-Key': API_KEY,
  'Referer': 'https://www.pinnacle.com/',
  'Accept': 'application/json',
  'Accept-Language': 'en-US,en;q=0.9'
};

function americanToDecimal(price) {
  const p = Number(price);
  if (!Number.isFinite(p) || p === 0) return null;
  if (p > 0) return +((p / 100) + 1).toFixed(3);
  return +((100 / Math.abs(p)) + 1).toFixed(3);
}

// 2026-04-28: warn-once quando key rotaciona. Antes 401/403 era opaco —
// pipeline caia silenciosamente por dias até alguém grepar logs.
let _pinnacle401WarnedAt = 0;
let _pinnacle503WarnedAt = 0;
let _pinnacleNetWarnedAt = 0;
let _pinnacleMaintenance = false; // exposed via isInMaintenance() para consumers
async function _get(path, { ttlMs = 3 * 60 * 1000 } = {}) {
  const url = `${API_BASE}${path}`;
  const r = await cachedHttpGet(url, {
    provider: 'pinnacle', ttlMs, headers: HEADERS, cacheKey: `pinnacle:${url}`,
    // 2026-06-10 (plano causal #4): soccer matchups (sport 29) estourou o MAX_BODY
    // global de 5MiB em 2026-05-30 (payload 5,25MB → football ficou sem sharp
    // anchor ≥29min). 8MiB = ~1.5x headroom; per-call pra não subir o teto global
    // (Sofascore HTML 10MB+ é o risco OOM que motivou o cap).
    maxBodyBytes: 8 * 1024 * 1024
  }).catch(e => {
    // 2026-05-29 audit: erro de rede (DNS/timeout/ECONNRESET) era engolido aqui →
    // null indistinguível de "0 matchups" na feed-health. Warn-once 1h (mirror _pinnacle401).
    const now = Date.now();
    if (now - _pinnacleNetWarnedAt > 60 * 60 * 1000) {
      _pinnacleNetWarnedAt = now;
      log('WARN', 'PINNACLE', `network error em ${path}: ${e?.message || e} (warn-once 1h)`);
    }
    return null;
  });
  if (r && (r.status === 401 || r.status === 403)) {
    const now = Date.now();
    if (now - _pinnacle401WarnedAt > 60 * 60 * 1000) {
      _pinnacle401WarnedAt = now;
      log('ERROR', 'PINNACLE', `KEY EXPIRED/INVALID — ${r.status} em ${path}. PINNACLE_API_KEY pode ter rotacionado. Pipeline odds esports/tennis/football quebrado até atualização.`);
    }
    // 2026-05-21 (audit feed-medic P0): expose gauge pra bot.js cron alertar
    // admin via DM. Antes log ERROR só dava warn-once 1h em logs Railway —
    // gap 6-24h até alguém grepar. Gauge fica em snapshot.gauges; cron 30min
    // em bot.js lê e DMs ADMIN_CHAT_IDS quando age_s < 7200 + value=1.
    try {
      require('./metrics').gauge('pinnacle_key_expired', 1, { status: String(r.status) });
    } catch (_) {}
    return null;
  }
  // 2026-05-11: detect Pinnacle global maintenance (HTTP 503 com body
  // `{"title":"MAINTENANCE"}`). Observado em prod: todos endpoints retornaram
  // 503 mas consumers chamavam markFeedSuccess(sport, 0) silenciosamente —
  // alert PINNACLE-OUTAGE só firing via count=0 cluster (cron 5min). Log WARN
  // throttled 30min + flag global pra consumers usarem markFeedFailure correto.
  if (r && r.status === 503) {
    _pinnacleMaintenance = true;
    const now = Date.now();
    if (now - _pinnacle503WarnedAt > 30 * 60 * 1000) {
      _pinnacle503WarnedAt = now;
      log('WARN', 'PINNACLE', `MAINTENANCE 503 em ${path}. Pipeline odds (esports/tennis/football/mma/darts/basket) parado até API voltar.`);
    }
    return null;
  }
  if (r && r.status === 200) {
    _pinnacleMaintenance = false; // 200 = API voltou, limpa flag
  }
  if (!r || r.status !== 200) return null;
  return safeParse(r.body, null);
}

/** Retorna true se a última resposta foi 503 maintenance.
 *  Consumers usam pra chamar markFeedFailure em vez de markFeedSuccess(0). */
function isInMaintenance() {
  return _pinnacleMaintenance;
}

async function listSportMatchups(sportId) {
  const data = await _get(`/0.1/sports/${sportId}/matchups?brandId=0`);
  return Array.isArray(data) ? data : [];
}

async function getMatchupMarkets(matchupId, opts = {}) {
  return _get(`/0.1/matchups/${matchupId}/markets/related/straight`, opts);
}

/**
 * Extrai odds moneyline de um matchup (home/away), convertendo American → Decimal.
 * @param {Object} matchup — item de listSportMatchups
 * @returns {Object|null} { id, league, group, startTime, status, team1, team2, oddsT1, oddsT2 }
 */
async function extractMoneyline(matchup) {
  try {
    const home = (matchup.participants || []).find(p => p.alignment === 'home');
    const away = (matchup.participants || []).find(p => p.alignment === 'away');
    if (!home?.name || !away?.name) return null;

    const markets = await getMatchupMarkets(matchup.id);
    if (!Array.isArray(markets)) return null;
    const ml = markets.find(x => x.type === 'moneyline' && x.period === 0 && x.status === 'open');
    if (!ml?.prices) return null;

    const ph = ml.prices.find(p => p.designation === 'home');
    const pa = ml.prices.find(p => p.designation === 'away');
    const oddsHome = americanToDecimal(ph?.price);
    const oddsAway = americanToDecimal(pa?.price);
    if (!oddsHome || !oddsAway) return null;

    return {
      id: matchup.id,
      league: matchup.league?.name || null,
      leagueId: matchup.league?.id,
      group: matchup.league?.group,
      startTime: matchup.startTime,
      status: matchup.isLive ? 'live' : 'upcoming',
      team1: home.name,
      team2: away.name,
      oddsT1: oddsHome,
      oddsT2: oddsAway,
    };
  } catch (e) {
    log('WARN', 'PINNACLE', `matchup ${matchup?.id}: ${e.message}`);
    return null;
  }
}

/**
 * Fluxo completo para qualquer esporte: lista matchups + extrai moneylines.
 * @param {number|string} sportId
 * @param {Function} [leagueFilter] — (matchup) => boolean para filtrar antes de pegar odds
 */
async function fetchSportMatchOdds(sportId, leagueFilter) {
  const matchups = await listSportMatchups(sportId);
  // 2026-05-17: throw quando maintenance ativo. Antes retornava [] silencioso
  // → callers chamavam markFeedSuccess('pinnacle', sport, 0) durante 503,
  // mascarando outage como "success com 0 odds". Audit externos 17/05 confirmou
  // isInMaintenance() dead-wire (exported sem callers). Throw deixa catch
  // existente disparar markFeedFailure correto.
  if (!matchups.length) {
    if (_pinnacleMaintenance) {
      const err = new Error('pinnacle_maintenance_503');
      err.code = 'PINNACLE_MAINTENANCE';
      throw err;
    }
    return [];
  }
  const filtered = typeof leagueFilter === 'function'
    ? matchups.filter(leagueFilter)
    : matchups;
  const out = [];
  for (const m of filtered) {
    const row = await extractMoneyline(m);
    if (row) out.push(row);
  }
  return out;
}

/**
 * Ranking implícito via de-juice das odds Pinnacle.
 * Pinnacle é o book mais afiado do mundo; suas odds de-juiced são um proxy decente
 * de ranking real quando não temos rank oficial (snooker.org etc.)
 *
 * Retorna { probA, probB } onde A=team1/home, B=team2/away (de-juice simples).
 */
function impliedProbsFromOdds(oddsA, oddsB) {
  const a = Number(oddsA), b = Number(oddsB);
  if (!a || !b || a <= 1 || b <= 1) return null;
  // 2026-05-19 audit P0-5: upper bound contra feed poisoning. Pinnacle ML real
  // raramente > 30 (longshot extremo); >50 quase sempre é parser error, alt-line
  // suspendida vazada como ML, OR MITM/feed poisoning. Sem cap: bot calcula EV
  // bizarro num underdog @50 (implied 2%), aposta no favorito @1.02 com modelo
  // overconfident → drain. Bound 50 dá margem 1.5× sobre Pinnacle hist real.
  // Disable via PINNACLE_ODDS_UPPER_BOUND=0 (não recomendado).
  const _upperBound = parseFloat(process.env.PINNACLE_ODDS_UPPER_BOUND || '50');
  if (_upperBound > 0 && (a > _upperBound || b > _upperBound)) return null;
  const ra = 1 / a, rb = 1 / b;
  const vig = ra + rb;
  return { probA: ra / vig, probB: rb / vig };
}

/**
 * Extrai moneyline de um period específico de um matchup (série).
 *
 * @param {number|string} matchupId — ID da série-raiz
 * @param {number} period — 0=série, 1..5=mapa N
 * @returns {Promise<{ oddsHome, oddsAway, status, period } | null>}
 */
async function getMatchupMoneylineByPeriod(matchupId, period) {
  const markets = await getMatchupMarkets(matchupId);
  if (!Array.isArray(markets)) return null;
  // Pode haver múltiplos moneyline entries pro mesmo period (alt lines) — pega o primeiro aberto
  const ml = markets.find(m =>
    m.type === 'moneyline' &&
    m.period === period &&
    m.status === 'open' &&
    Array.isArray(m.prices) &&
    m.prices.length >= 2
  );
  if (!ml) return null;
  const ph = ml.prices.find(p => p.designation === 'home');
  const pa = ml.prices.find(p => p.designation === 'away');
  const oddsHome = americanToDecimal(ph?.price);
  const oddsAway = americanToDecimal(pa?.price);
  if (!oddsHome || !oddsAway) return null;
  return { oddsHome, oddsAway, status: ml.status, period };
}

/**
 * Extrai mercados de handicap pra um period. Pinnacle expõe handicaps inteiros
 * e meio-pontos; filtramos meio-pontos (0.5) pra evitar push.
 *
 * @param {number|string} matchupId
 * @param {number} period — 0=série, 1..5=mapa N
 * @returns {Promise<Array<{ line, oddsHome, oddsAway, period }>>}
 */
async function getMatchupHandicaps(matchupId, period, opts = {}) {
  // opts pass-through: ttlMs=0 força cache bypass (uso: pre-DM drift instrumentation).
  const markets = await getMatchupMarkets(matchupId, opts);
  if (!Array.isArray(markets)) return opts.groupByVirtual ? { sets: [], games: [] } : [];
  // BUG FIX 2026-04-25: Pinnacle tennis cria VIRTUAL matchups separados pra
  // sets e games. Antes usávamos heurística "mais lines distintas = games"
  // mas EMPATAVA quando ambos virtuals tinham 2 lines após filtro meio-ponto
  // (ex: Lehecka vs Michelsen: sets=[-1.5,1.5], games=[-3.5,-2.5] após filter).
  // Map iteration order então decidia, frequentemente devolvendo SETS marcado
  // como GAMES → tip handicapGames ±1.5 com pricing errado (gamesMarginPdf vs
  // realidade sets handicap).
  //
  // Fix: classifica cada matchupId virtual pelo **range** de lines:
  //   max(abs(line)) ≤ 2.5  → SETS (Bo3 só ±1.5; Bo5 ±1.5/2.5)
  //   max(abs(line)) > 2.5  → GAMES (sempre tem line ≥ 3.5)
  //
  // Esports (1 virtual só): line típica ±1.5/2.5 → classificado 'sets', mas
  // caller esports ignora o kind tag (trata como map handicap genérico).
  //
  // opts.groupByVirtual=true → retorna { sets: [...], games: [...] } (tennis caller)
  // opts.includeAll=true → retorna flat de TUDO (debug)
  // default → flat com kind tag em cada entry, prefere games se ambos virtuais
  const includeAll = opts.includeAll === true;
  const groupByVirtual = opts.groupByVirtual === true;

  // Parse all candidates sem dedup
  const all = [];
  for (const m of markets) {
    if (m.type !== 'spread' && m.type !== 'handicap') continue;
    if (m.period !== period) continue;
    if (m.status !== 'open') continue;
    if (!Array.isArray(m.prices) || m.prices.length < 2) continue;
    const ph = m.prices.find(p => p.designation === 'home');
    const pa = m.prices.find(p => p.designation === 'away');
    if (!ph || !pa) continue;
    const oddsHome = americanToDecimal(ph.price);
    const oddsAway = americanToDecimal(pa.price);
    if (!oddsHome || !oddsAway) continue;
    const line = Number(ph.points);
    if (!Number.isFinite(line)) continue;
    if (Math.abs(line * 2) % 2 !== 1) continue; // meio-ponto only
    all.push({ matchupId: Number(m.matchupId), line, oddsHome, oddsAway, period });
  }

  if (!all.length) return groupByVirtual ? { sets: [], games: [] } : [];

  // Agrupa por matchupId
  const byMatch = new Map();
  for (const r of all) {
    if (!byMatch.has(r.matchupId)) byMatch.set(r.matchupId, []);
    byMatch.get(r.matchupId).push(r);
  }

  // Classifica cada grupo
  const setsList = [], gamesList = [];
  for (const [, list] of byMatch) {
    const maxAbs = Math.max(...list.map(l => Math.abs(l.line)));
    const dest = maxAbs > 2.5 ? gamesList : setsList;
    dest.push(...list);
  }

  const dedup = (list) => {
    const seen = new Set();
    return list
      .filter(r => { if (seen.has(r.line)) return false; seen.add(r.line); return true; })
      .map(({ line, oddsHome, oddsAway, period }) => ({ line, oddsHome, oddsAway, period }));
  };

  if (groupByVirtual) {
    return { sets: dedup(setsList), games: dedup(gamesList) };
  }

  if (includeAll) {
    const seen = new Set();
    return all
      .filter(r => { if (seen.has(r.line)) return false; seen.add(r.line); return true; })
      .map(({ line, oddsHome, oddsAway, period }) => ({ line, oddsHome, oddsAway, period }));
  }

  // Default: prefere games. Se só sets disponível, retorna sets MARCADO com kind
  // pra caller saber. Tennis scanner agora valida kind antes de pricar.
  if (gamesList.length) {
    return dedup(gamesList).map(r => ({ ...r, kind: 'games' }));
  }
  if (setsList.length) {
    return dedup(setsList).map(r => ({ ...r, kind: 'sets' }));
  }
  return [];
}

/**
 * Extrai totals (over/under) pra um period.
 *
 * @param {number|string} matchupId
 * @param {number} period
 * @returns {Promise<Array<{ line, oddsOver, oddsUnder, period }>>}
 */
async function getMatchupTotals(matchupId, period, opts = {}) {
  const markets = await getMatchupMarkets(matchupId);
  if (!Array.isArray(markets)) return [];
  // Mesma heurística de getMatchupHandicaps: agrupa por matchupId e escolhe o
  // virtual com MAIOR range de lines. Em tennis Bo3, (Games) tem line 22, 23,
  // 23.5, etc (5+ lines) enquanto (Sets) tem só 2.5 (1 line). Games vence.
  const includeAll = opts.includeAll === true;
  const all = [];
  for (const m of markets) {
    if (m.type !== 'total') continue;
    if (m.period !== period) continue;
    if (m.status !== 'open') continue;
    if (!Array.isArray(m.prices) || m.prices.length < 2) continue;
    const po = m.prices.find(p => p.designation === 'over');
    const pu = m.prices.find(p => p.designation === 'under');
    if (!po || !pu) continue;
    const oddsOver = americanToDecimal(po.price);
    const oddsUnder = americanToDecimal(pu.price);
    if (!oddsOver || !oddsUnder) continue;
    const line = Number(po.points);
    if (!Number.isFinite(line)) continue;
    if (Math.abs(line * 2) % 2 !== 1) continue;
    all.push({ matchupId: Number(m.matchupId), line, oddsOver, oddsUnder, period });
  }
  if (!all.length) return [];
  const byMatch = new Map();
  for (const r of all) {
    if (!byMatch.has(r.matchupId)) byMatch.set(r.matchupId, []);
    byMatch.get(r.matchupId).push(r);
  }
  // groupByMatchup: retorna [{ matchupId, lines: [...] }] separados.
  // Útil pra tennis onde Pinnacle agrupa games (matchupId X) e aces (matchupId Y)
  // em virtuais distintos. Caller usa heurística (mediana) pra identificar.
  if (opts.groupByMatchup === true) {
    const dedupGroup = (list) => {
      const seen = new Set();
      return list
        .filter(r => { if (seen.has(r.line)) return false; seen.add(r.line); return true; })
        .map(({ line, oddsOver, oddsUnder, period }) => ({ line, oddsOver, oddsUnder, period }));
    };
    return [...byMatch.entries()].map(([mid, list]) => ({
      matchupId: mid,
      lines: dedupGroup(list),
    }));
  }
  let chosen = all;
  if (byMatch.size > 1 && !includeAll) {
    let bestId = null, bestCount = 0;
    for (const [mid, list] of byMatch) {
      const uniq = new Set(list.map(l => l.line));
      if (uniq.size > bestCount) { bestCount = uniq.size; bestId = mid; }
    }
    chosen = byMatch.get(bestId) || all;
  }
  const seen = new Set();
  return chosen
    .filter(r => { if (seen.has(r.line)) return false; seen.add(r.line); return true; })
    .map(({ line, oddsOver, oddsUnder, period }) => ({ line, oddsOver, oddsUnder, period }));
}

module.exports = {
  listSportMatchups,
  getMatchupMarkets,
  extractMoneyline,
  fetchSportMatchOdds,
  americanToDecimal,
  impliedProbsFromOdds,
  getMatchupMoneylineByPeriod,
  getMatchupHandicaps,
  getMatchupTotals,
  isInMaintenance,
};

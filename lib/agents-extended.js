...
async function runPreMatchFinalCheck(serverBase, db, opts = {}) {
  ...
  for (const tip of tips) {
    try {
      ...
      if (!m) {
        // Match não encontrado pode ser:
        // (a) já jogado (endpoint só retorna upcoming/live) — settlement deveria ter pego
        // (b) cancelado/postponed
        // Pra (a) ser MENOS provável: só flagga se sent_at é recente (<6h) — match deveria
        // ainda estar listado como upcoming/live se não foi jogado.
        const sentMs = new Date((tip.sent_at || '').replace(' ', 'T') + 'Z').getTime();
        const sentAgeH = (Date.now() - sentMs) / (60 * 60 * 1000);
        if (sentAgeH > 6) {
          // Tip mais antiga: provavelmente já jogou, settlement parado. Não é cancelamento.
          continue;
        }
        out.alerts.push({
          tip_id: tip.id, sport: tip.sport, severity: 'warning',
          alert: 'match_missing',
          detail: `Tip #${tip.id} (${tip.participant1} vs ${tip.participant2}) não encontrada em ${ep} — possível cancelamento (tip de ${sentAgeH.toFixed(1)}h atrás)`,
        });
        continue;
      }

...
async function runBankrollGuardian(serverBase, db) {
  ...
  // 'esports' é bucket legado pós-split Abr/2026 — excluído de alerts/DD pra não duplicar
  // com 'lol'/'dota2'. Tips históricas com sport='esports' ainda existem no DB e surfacem
  // via union nas queries de ROI/equity, mas guardian/watcher ignoram.
  const SPORTS_LIST = ['lol', 'dota2', 'mma', 'tennis', 'football', 'cs', 'valorant', 'darts', 'snooker', 'tabletennis'];
  ...
  for (const sport of SPORTS_LIST) {
    try {
      ...
      // 2026-04-28: `current_banca || initial` mascara banca zerada (0 || N = N).
      // Se banca drenou totalmente, current=0 e DD calc retornava 0% (oculta blow-up).
      // Number.isFinite preserva 0 distinto de null/NaN.
      const current = Number.isFinite(Number(data.current_banca)) ? Number(data.current_banca) : initial;
      const peak = Number.isFinite(Number(data.peak_banca)) ? Number(data.peak_banca) : initial;
      const drawdownPct = peak > 0 ? ((peak - current) / peak) * 100 : 0;
      const growth = initial > 0 ? ((current - initial) / initial) * 100 : 0;

      const daysSettled = Number(data.days_settled) || 0;
      const sportItem = {
        sport,
        ...
        days_settled: daysSettled,
      };

      // ── Thresholds adaptativos por tamanho de banca ────────────────────────
      // Bankrolls pequenas (< R$100) têm variance relativa muito maior — 1 loss
      // de 2u em banca R$50 já gera ~36% DD. Usar thresholds frouxos evita
      // bloqueio prematuro de sports em fase early/rebuild.
      //
      // Também exige min sample (>=5 dias com settlement no período) — evita
      // alertar sports com 1-2 losses sem evidência estatística de leak.
      //
      // Overrides via env:
      //   BANKROLL_DD_BLOCK_PCT, BANKROLL_DD_SHADOW_PCT, BANKROLL_DD_REVIEW_PCT
      //   BANKROLL_ALERT_MIN_DAYS (default 5)
      const env = process.env || {};
      const smallBanca = initial > 0 && initial < 100;  // <R$100 = banca pequena
      const baseBlockPct  = parseFloat(env.BANKROLL_DD_BLOCK_PCT  || (smallBanca ? '45' : '35')) || (smallBanca ? 45 : 35);
      const baseShadowPct = parseFloat(env.BANKROLL_DD_SHADOW_PCT || (smallBanca ? '28' : '20')) || (smallBanca ? 28 : 20);
      const baseReviewPct = parseFloat(env.BANKROLL_DD_REVIEW_PCT || (smallBanca ? '18' : '12')) || (smallBanca ? 18 : 12);
      const minDaysForAlert = parseInt(env.BANKROLL_ALERT_MIN_DAYS || '5', 10) || 5;
      const hasEnoughSample = daysSettled >= minDaysForAlert;

      // Gate ROI-based adicional: sport com n≥20 tips e ROI muito negativo devia ser
      // pausado MESMO sem drawdown alto (ex: Dota ROI -34% mas DD só 9% se peak=initial).
      // Computa ROI approximation via stake units settled = N_decided × avg_stake × unit.
      // Shortcut: ROI = (current - initial) / (n * avg_stake_est). Mas sample de 20+ tips com
      // current < initial × 0.9 já indica leak persistente.
      const roiPct = parseFloat(data.roi_pct || NaN);
      const settledN = Number(data.settled_count || 0);
      const roiBlockPct  = parseFloat(env.BANKROLL_ROI_BLOCK_PCT  || '-25') || -25;
      const roiShadowPct = parseFloat(env.BANKROLL_ROI_SHADOW_PCT || '-15') || -15;
      const roiReviewPct = parseFloat(env.BANKROLL_ROI_REVIEW_PCT || '-8') || -8;
      const roiMinN = parseInt(env.BANKROLL_ROI_MIN_N || '20', 10) || 20;
      sportItem.roi_pct = Number.isFinite(roiPct) ? roiPct : null;
      sportItem.settled_n = settledN;
      const triggerROI = (severity) => {
        const thresh = severity === 'critical' ? roiBlockPct
                     : severity === 'warning'  ? roiShadowPct
                     : roiReviewPct;
        const action = severity === 'critical' ? 'BLOCK_BOT'
                     : severity === 'warning'  ? 'AUTO_SHADOW'
                     : 'REVIEW';
        return Number.isFinite(roiPct) && settledN >= roiMinN && roiPct <= thresh && severity;
      };

      if (!hasEnoughSample && drawdownPct >= baseReviewPct) {
        sportItem.severity = 'info';
        sportItem.action_recommended = 'MONITOR';
        sportItem.alert_skipped_reason = `sample_pequeno (${daysSettled}d < ${minDaysForAlert}d)`;
      } else if (drawdownPct >= baseBlockPct && hasEnoughSample) {
        sportItem.severity = 'critical';
        sportItem.action_recommended = 'BLOCK_BOT';
        out.alerts.push({ sport, severity: 'critical', drawdown_pct: drawdownPct, action: 'BLOCK_BOT', message: `${sport}: DD ${drawdownPct.toFixed(1)}% (banca R$${initial.toFixed(0)}, ${daysSettled}d settled) — bloquear emissão` });
      } else if (Number.isFinite(roiPct) && settledN >= roiMinN && roiPct <= roiBlockPct) {
        // ROI-based block: sport sangrando com sample suficiente
        sportItem.severity = 'critical';
        sportItem.action_recommended = 'BLOCK_BOT';
        out.alerts.push({ sport, severity: 'critical', roi_pct: roiPct, drawdown_pct: drawdownPct, action: 'BLOCK_BOT', message: `${sport}: ROI ${roiPct.toFixed(1)}% em ${settledN} tips — bloquear emissão (leak persistente)` });
      } else if (drawdownPct >= baseShadowPct && hasEnoughSample) {
        sportItem.severity = 'warning';
        sportItem.action_recommended = 'AUTO_SHADOW';
        out.alerts.push({ sport, severity: 'warning', drawdown_pct: drawdownPct, action: 'AUTO_SHADOW', message: `${sport}: DD ${drawdownPct.toFixed(1)}% (banca R$${initial.toFixed(0)}) — auto-shadow 1h` });
      } else if (Number.isFinite(roiPct) && settledN >= roiMinN && roiPct <= roiShadowPct) {
        sportItem.severity = 'warning';
        sportItem.action_recommended = 'AUTO_SHADOW';
        out.alerts.push({ sport, severity: 'warning', roi_pct: roiPct, action: 'AUTO_SHADOW', message: `${sport}: ROI ${roiPct.toFixed(1)}% em ${settledN} tips — auto-shadow 1h` });
      } else if (drawdownPct >= baseReviewPct && hasEnoughSample) {
        sportItem.severity = 'info';
      ...
      } else if (Number.isFinite(roiPct) && settledN >= roiMinN && roiPct <= roiReviewPct) {
        sportItem.severity = 'info';
        sportItem.action_recommended = 'REVIEW';
        out.alerts.push({ sport, severity: 'info', roi_pct: roiPct, action: 'REVIEW', message: `${sport}: ROI ${roiPct.toFixed(1)}% em ${settledN} tips — revisar modelo` });
      } else {
  ...
  // Portfolio impact per sport — quanto cada sport consumiu do portfolio global
  // (valor negativo = drenou capital dos outros). Alerta quando impact ≤ -5pp.
  if (totalInitial > 0) {
    for (const s of out.sports) {
      const delta = (s.current_banca || 0) - (s.initial_banca || 0);
      const impactPp = parseFloat((delta / totalInitial * 100).toFixed(2));
      s.portfolio_impact_pp = impactPp;
      // Novo alert: drenou ≥5% do portfolio total
      if (impactPp <= -5 && !out.alerts.some(a => a.sport === s.sport && a.action === 'BLOCK_BOT')) {
        out.alerts.push({
          sport: s.sport,
          severity: impactPp <= -10 ? 'critical' : 'warning',
          drawdown_pct: s.drawdown_pct,
          portfolio_impact_pp: impactPp,
          action: 'PORTFOLIO_DRAIN',
          message: `${s.sport}: consumiu ${Math.abs(impactPp).toFixed(1)}pp do portfolio (R$${delta.toFixed(2)})`,
        });
      }
    }
  }
...
// ────────────────────────────────────────────────────────────────────
// PandaScore cross-check helper.
// Antes de flagar match_missing, consulta PandaScore direto (fonte de verdade
// pra esports) — evita falso positivo quando /lol-matches filtra ou tem bug.
// Cache por ciclo para economizar rate limit.
// ────────────────────────────────────────────────────────────────────

const PS_GAME_PATH = {
  esports: 'lol', dota: 'dota2', cs: 'csgo', valorant: 'valorant',
};

// Cache válido por 5min (ciclo do preMatchFinalCheck é 5min)
const _psCache = new Map(); // gamePath → { ts, upcoming: [], running: [], past: [] }
const _PS_TTL_MS = 5 * 60 * 1000;

async function _loadPandaScoreSnapshot(gamePath) {
  const now = Date.now();
  const hit = _psCache.get(gamePath);
  if (hit && (now - hit.ts) < _PS_TTL_MS) return hit;
  const token = process.env.PANDASCORE_TOKEN;
  if (!token) return { ts: now, upcoming: [], running: [], past: [] };
  const headers = { 'Authorization': `Bearer ${token}` };
  const base = `https://api.pandascore.co/${gamePath}/matches`;
  const [up, run, past] = await Promise.all([
    agentHttpGet(`${base}/upcoming?per_page=50`, 8000, headers).catch(() => ({ body: '[]' })),
    agentHttpGet(`${base}/running?per_page=30`, 8000, headers).catch(() => ({ body: '[]' })),
    agentHttpGet(`${base}/past?per_page=30&sort=-end_at`, 8000, headers).catch(() => ({ body: '[]' })),
  ]);
  const safe = (r) => { try { const j = JSON.parse(r.body); return Array.isArray(j) ? j : []; } catch { return []; } };
  const snap = { ts: now, upcoming: safe(up), running: safe(run), past: safe(past) };
  _psCache.set(gamePath, snap);
  return snap;
}

function _matchTeamInMatch(m, n1, n2) {
  // PandaScore match tem opponents: [{ opponent: { name } }, ...]
  const opps = (m.opponents || []).map(o => String(o?.opponent?.name || '').toLowerCase().replace(/[^a-z0-9]/g, ''));
  if (opps.length < 2) return false;
  const has1 = opps.some(n => n.includes(n1) || n1.includes(n));
  const has2 = opps.some(n => n.includes(n2) || n2.includes(n));
  return has1 && has2;
}

/**
 * Cross-check: procura match (team1, team2) em PandaScore.
 * @returns {Promise<{ found: boolean, bucket: 'running'|'upcoming'|'past'|null, status: string|null }>}
 */
async function _pandascoreCrossCheck(sport, team1, team2) {
  const gamePath = PS_GAME_PATH[sport];
  if (!gamePath) return { found: false, bucket: null, status: null };
  const snap = await _loadPandaScoreSnapshot(gamePath);
  const norm = s => String(s || '').toLowerCase().replace(/[^a-z0-9]/g, '');
  const n1 = norm(team1), n2 = norm(team2);
  for (const bucket of ['running', 'upcoming', 'past']) {
    for (const m of snap[bucket] || []) {
      if (_matchTeamInMatch(m, n1, n2)) {
        return { found: true, bucket, status: m.status || bucket };
      }
    }
  }
  return { found: false, bucket: null, status: null };
}
...
async function runPreMatchFinalCheck(serverBase, db, opts = {}) {
  ...
  for (const tip of tips) {
    try {
      ...
      // Nota: Dota tips são salvas com sport='esports' (bot.js:8200 — umbrella DB).
      // Detectamos sub-sport via prefixo do match_id: dota2_* → Dota, senão LoL.
      const isEsportsDota = tip.sport === 'esports' && String(tip.match_id || '').startsWith('dota2_');
      const effectiveSport = isEsportsDota ? 'dota' : tip.sport;
      ...
      const ep = sportToEndpoint[effectiveSport];
      if (!ep) continue;
      ...
      if (!m) {
        ...
        // Cross-check PandaScore (fonte de verdade pra esports) antes de alertar.
        // PandaScore confirma cancelamento via status='canceled' em /past; ou
        // confirma "legit e rodando" se achar em /running ou /upcoming.
        if (PS_GAME_PATH[effectiveSport]) {
          const xc = await _pandascoreCrossCheck(effectiveSport, tip.participant1, tip.participant2);
          if (xc.found && xc.status === 'canceled') {
            // Confirmação forte: match cancelado. Upgrade pra critical.
            out.alerts.push({
              tip_id: tip.id, sport: tip.sport, severity: 'critical',
              alert: 'match_cancelled',
              detail: `Tip #${tip.id} (${tip.participant1} vs ${tip.participant2}): PandaScore confirma status=canceled (bucket=${xc.bucket}). Void recomendado.`,
            });
            continue;
          }
          if (xc.found && xc.bucket !== 'past') {
            // Match legit em upcoming/running — feed interno com bug transiente. Skip.
            continue;
          }
          if (xc.found && xc.bucket === 'past' && (xc.status === 'finished' || xc.status === null)) {
            // Match já jogou — settlement pendente, não cancelamento. Skip.
            continue;
          }
        }
        ...
        out.alerts.push({
          ...
          alert: 'match_missing',
          detail: `Tip #${tip.id} (${tip.participant1} vs ${tip.participant2}) não encontrada em ${ep} nem PandaScore — possível cancelamento (tip há ${Math.round(sentAgeMin)}min)`,
        });
        continue;
      }
...
async function runModelCalibrationWatcher(db) {
  ...
  for (const sport of SPORTS_LIST) {
    try {
      // Brier 30d (recent) — football inclui tip_context_json + market_type pra RPS.
      const isFb = sport === 'football';
      const extraCols = isFb ? ', market_type, tip_context_json' : '';
      const recent = db.prepare(`
        SELECT odds, model_p_pick, result${extraCols} FROM tips
      ...
      // 2026-05-01: RPS (Ranked Probability Score) pra football 1X2.
      // RPS = (1/(K-1)) * Σ_k (F_pred(k) - F_obs(k))² onde F são CDFs.
      // Pra 1X2: ordering Home(1) → Draw(2) → Away(3). RPS pune mais
      // "previu Home (1), deu Away (3)" do que "previu Home, deu Draw"
      // — algo que Brier (3 binários independentes) não captura.
      // Lit: Constantinou & Fenton 2012, "Solving the problem of inadequate
      // scoring rules for assessing probabilistic football forecast models".
      //
      // Outcome reconstruction:
      //   tip 1X2_H + win  → outcome=H
      //   tip 1X2_D + win  → outcome=D
      //   tip 1X2_A + win  → outcome=A
      //   tip 1X2_X + loss → ambíguo (precisa final_score). v1: skip.
      //
      // v1 limitation: só inclui tips que GANHARAM (50% da amostra). É um
      // "RPS-on-wins" — viesado pra calibração quando modelo acerta. Útil pra
      // detectar drift mas não substitui Brier. v2 (futuro): JOIN match_results
      // pra recuperar outcome em loss rows.
      const calcRPS = (rows) => {
        let sum = 0, n = 0;
        for (const r of rows) {
          if (!r.market_type || !/^1X2_/.test(r.market_type)) continue;
          if (r.result !== 'win') continue;  // v1: outcome só conhecido em wins
          let dist = null;
          try {
            const ctx = JSON.parse(r.tip_context_json || '{}');
            if (ctx?.fb_dist?.pH != null) dist = ctx.fb_dist;
          } catch (_) {}
          if (!dist) continue;
          const tipMkt = r.market_type;
          const outcomeIdx = tipMkt === '1X2_H' ? 1 : tipMkt === '1X2_D' ? 2 : 3;
          const pH = dist.pH, pD = dist.pD, pA = dist.pA;
          const F_pred = [pH, pH + pD, pH + pD + pA];
          const F_obs = [outcomeIdx <= 1 ? 1 : 0, outcomeIdx <= 2 ? 1 : 0, 1];
          // RPS = (1/(K-1)) * Σ (F_pred - F_obs)² for k=1..K-1, K=3 → div 2
          const rps = ((F_pred[0] - F_obs[0]) ** 2 + (F_pred[1] - F_obs[1]) ** 2) / 2;
          sum += rps;
          n++;
        }
        return n > 0 ? { rps: sum / n, n } : null;
      };
      ...
      const recentRps = isFb ? calcRPS(recent) : null;
      const baselineRps = isFb ? calcRPS(baseline) : null;
      ...
      if (isFb) {
        item.recent_rps = recentRps ? parseFloat(recentRps.rps.toFixed(3)) : null;
        item.recent_rps_n = recentRps ? recentRps.n : 0;
        item.baseline_rps = baselineRps ? parseFloat(baselineRps.rps.toFixed(3)) : null;
        item.baseline_rps_n = baselineRps ? baselineRps.n : 0;
        item.rps_drift = (recentRps && baselineRps)
          ? parseFloat((recentRps.rps - baselineRps.rps).toFixed(3))
          : null;
      }
...
function _applyGatesRetroactive(tip) {
  ...
  if (Number.isFinite(modelP) && Number.isFinite(odds) && odds > 1) {
    // 2026-05-11 audit P2-6: vig hardcoded 1.025 era LoL/CS/tennis-centric.
    // Football 3-way (vig real ~4-5%) subestimava divergência → tips football
    // passavam retroactive check que deveriam falhar. Reusa VIG_PER_SPORT de
    // lib/gate-optimizer.js pra single source of truth.
    const { VIG_PER_SPORT } = require('./gate-optimizer');
    const impliedRaw = 1 / odds;
    const vigDivisor = VIG_PER_SPORT[String(tip.sport || '').toLowerCase()] || VIG_PER_SPORT.default;
    const impliedDejuiced = impliedRaw / vigDivisor;
...
// ── Calibração granular: reliability diagram + ECE + Brier decomposition ──
// Recebe array de { p, outcome } (p ∈ [0,1], outcome 0 ou 1)
// Retorna: buckets [0..9], ECE, sharpness, decomposição Brier
function _computeCalibration(samples) {
  ...
  let totalP = 0, totalWins = 0;
  for (const s of samples) {
    const p = Math.max(0, Math.min(0.9999, s.p));
    const idx = Math.min(NUM_BUCKETS - 1, Math.floor(p * NUM_BUCKETS));
    buckets[idx].n++;
    buckets[idx].sumP += p;
    if (s.outcome === 1) buckets[idx].wins++;
    totalP += p;
    if (s.outcome === 1) totalWins++;
  }
  const N = samples.length;
  const baseRate = totalWins / N;
  const avgP = totalP / N;
  // Sharpness: variance das predições
  const sharpness = samples.reduce((a, s) => a + (s.p - avgP) ** 2, 0) / N;
  // Por bucket
  let reliability = 0, resolution = 0, ece = 0;
  const bucketSummary = [];
  for (const b of buckets) {
    if (b.n === 0) {
      bucketSummary.push({ range: b.range, n: 0 });
      continue;
    }
    const predAvg = b.sumP / b.n;
    const actualFreq = b.wins / b.n;
    const gap = actualFreq - predAvg;
    const weight = b.n / N;
    reliability += weight * (predAvg - actualFreq) ** 2;
    resolution += weight * (actualFreq - baseRate) ** 2;
    ece += weight * Math.abs(gap);
    let direction = 'ok';
    if (Math.abs(gap) > 0.05) direction = gap > 0 ? 'underconfident' : 'OVERCONFIDENT';
    bucketSummary.push({
      range: b.range,
      n: b.n,
      predicted_avg: parseFloat((predAvg * 100).toFixed(1)),
      actual_freq: parseFloat((actualFreq * 100).toFixed(1)),
      gap_pp: parseFloat((gap * 100).toFixed(1)),
      direction,
    });
  }
  const uncertainty = baseRate * (1 - baseRate);
  const brierFromDecomp = reliability - resolution + uncertainty;
  return {
    n: N,
    base_rate: parseFloat((baseRate * 100).toFixed(1)),
    avg_predicted: parseFloat((avgP * 100).toFixed(1)),
    sharpness: parseFloat(sharpness.toFixed(3)),
    ece: parseFloat(ece.toFixed(3)),
    decomposition: {
      reliability: parseFloat(reliability.toFixed(3)),
      resolution: parseFloat(resolution.toFixed(3)),
      uncertainty: parseFloat(uncertainty.toFixed(3)),
      brier_check: parseFloat(brierFromDecomp.toFixed(3)),
    },
    buckets: bucketSummary,
    interpretation: _interpretCalibration(ece, reliability, resolution, sharpness, bucketSummary),
  };
}

function _interpretCalibration(ece, reliability, resolution, sharpness, buckets) {
  const issues = [];
  // ECE thresholds (rule of thumb pra binário): <0.05 ok, 0.05-0.10 medio, >0.10 ruim
  if (ece < 0.05) issues.push(`✅ Calibração boa (ECE ${ece.toFixed(3)})`);
  else if (ece < 0.10) issues.push(`🟡 Calibração média (ECE ${ece.toFixed(3)})`);
  else issues.push(`🔴 Calibração ruim (ECE ${ece.toFixed(3)})`);
  // Reliability: pequeno = boa calibração
  if (reliability > 0.02) issues.push(`reliability ${reliability.toFixed(3)} — predições mal-alinhadas com freq real`);
  // Resolution: grande = bom discriminador
  if (resolution < 0.01) issues.push(`resolution ${resolution.toFixed(3)} — modelo discrimina pouco (predições próximas da base rate)`);
  else issues.push(`resolution ${resolution.toFixed(3)} — discriminação OK`);
  // Sharpness
  if (sharpness < 0.02) issues.push(`sharpness ${sharpness.toFixed(3)} — modelo cauteloso (predições próximas de 50%)`);
  else if (sharpness > 0.10) issues.push(`sharpness ${sharpness.toFixed(3)} — modelo muito confiante`);
  // Buckets problemáticos
  const overconfidentBuckets = buckets.filter(b => b.n >= 3 && b.direction === 'OVERCONFIDENT');
  const underconfidentBuckets = buckets.filter(b => b.n >= 3 && b.direction === 'underconfident');
  if (overconfidentBuckets.length) {
    issues.push(`⚠️ Overconfident em: ${overconfidentBuckets.map(b => b.range).join(', ')} — recalibrar com shrinkage`);
  }
  if (underconfidentBuckets.length) {
    issues.push(`💡 Underconfident em: ${underconfidentBuckets.map(b => b.range).join(', ')} — modelo poderia ser mais agressivo`);
  }
  return issues;
}
...
async function runBacktestValidator(db, opts = {}) {
  ...
  const calibrationSamples = new Map(); // sport → [{ p, outcome }]
  const allCalibSamples = []; // overall
  ...
  for (const tip of tips) {
    ...
    // Coleta amostras pra calibração granular (só win/loss, exclui push)
    if (isWin || isLoss) {
      const sample = { p: pUsed, outcome: isWin ? 1 : 0 };
      allCalibSamples.push(sample);
      if (!calibrationSamples.has(sport)) calibrationSamples.set(sport, []);
      calibrationSamples.get(sport).push(sample);
    }
  ...
  // Veredito por sport — diferencia ROI vs Brier vs gates impact
  const verdictSport = (sb) => {
    ...
    const gatesNet = h.savedLoss - h.lostProfit;

    if (r.n < 30) return { code: 'insufficient', label: '⚪ Sample insuficiente', detail: `n=${r.n} (precisa ≥30)` };

    // 4 quadrantes (Brier vs ROI):
    // (1) Brier OK + ROI positivo = edge real
    // (2) Brier OK + ROI baixo = calibração boa, sample limitado / bookie tight
    // (3) Brier ruim + ROI positivo = sorte/variância, NÃO sustentável
    // (4) Brier ruim + ROI negativo = bleed confirmado

    if (brierEdge != null && brierEdge > 0.005 && roi > 3) return { code: 'edge_real', label: '🟢 Edge real (calibração + ROI)', detail: `Brier ${brier.toFixed(3)} < baseline ${baselineBrier.toFixed(3)} | ROI +${roi.toFixed(1)}%` };
    ...
    if (brierEdge != null && brierEdge < -0.005 && roi > 3) {
      // Brier ruim + ROI positivo = sorte. Se gates estão custando, alerta.
      const gatesNote = gatesNet < -1 ? ` | ⚠️ gates custaram R$${Math.abs(gatesNet).toFixed(2)} (revisar cap)` : '';
      return { code: 'lucky_uncalib', label: '🟡 ROI+ mas modelo descalibrado (sorte?)', detail: `Brier ${brier.toFixed(3)} > baseline ${baselineBrier.toFixed(3)} mas ROI +${roi.toFixed(1)}%${gatesNote}` };
    }
    if (brierEdge != null && brierEdge < -0.005 && roi < -3) return { code: 'bleed_uncalib', label: '🔴 BLEED + descalibrado', detail: `Brier ${brier.toFixed(3)} > baseline ${baselineBrier.toFixed(3)} | ROI ${roi.toFixed(1)}% — bleed real` };
    if (brierEdge != null && brierEdge < -0.005) return { code: 'noise', label: '🔴 Modelo descalibrado', detail: `Brier ${brier.toFixed(3)} > baseline ${baselineBrier.toFixed(3)} | ROI ${roi.toFixed(1)}% — pior que random` };
    if (roi > 5) return { code: 'roi_no_calib', label: '🟢 ROI positivo, calibração neutra', detail: `ROI +${roi.toFixed(1)}% | Brier ≈ baseline` };
  ...
  for (const sb of bySport.values()) {
    ...
    // Calibração granular (só se n>=10 pra ser estatisticamente útil)
    const samples = calibrationSamples.get(sb.sport) || [];
    const calibration = samples.length >= 10 ? _computeCalibration(samples) : null;
    sports.push({
      sport: sb.sport,
      n: r.n,
  ...
  // Calibração overall
  const overallCalibration = allCalibSamples.length >= 30 ? _computeCalibration(allCalibSamples) : null;
  ...
  return {
    ...
    overall: {
      ...
      calibration: overallCalibration, // null se n<30
    },
    sports,
    milestones,
    summary: {
...
async function runNewsMonitor(serverBase, db, opts = {}) {
  ...
  for (const src of results) {
    ...
    for (const item of (src.items || [])) {
      ...
      // 2026-05-12: affected_teams pra news-impact gate em /record-tip.
      // Extrai todos team_keys que matched no title (mesmo regex usado acima).
      // Mais útil que matched_tip_ids: tip pode AINDA não existir mas vamos
      // emitir em segundos → gate consulta esse cache.
      const affectedTeams = [];
      for (const [teamKey, tips] of teamToTips.entries()) {
        if (teamKey.length < 3) continue;
        const teamWordRe = new RegExp(`\\b${teamKey.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'i');
        if (teamWordRe.test(titleLow)) affectedTeams.push(teamKey);
      }
      out.alerts.push({

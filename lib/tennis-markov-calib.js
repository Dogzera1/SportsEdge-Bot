 ...
 *
 * Routing:
 *   - opts.isLive=true: tenta markets.live[market], fallback pra markets[market]
 *   - opts.isLive=false (default): markets[market] (calib pre-jogo)
 *
 * Live calib separada está ready em infra mas atualmente vazia (sample
 * insuficiente — 21 live tips total / 0 settled). Quando >=30 settled live
 * acumular, refitar com `node scripts/fit-tennis-markov-calibration.js
 * --filter=live` que injeta em markets.live.{handicap,total}.
 *
...
function applyMarkovCalib(pRaw, market, opts = {}) {
  ...
  // Compute pCalib (isotonic) — se calib desabilitada/ausente, usa pRaw.
  // Shrink é aplicado DEPOIS, independente da calib, pra defender contra
  // overconfidence residual que sobreviveu à isotonic.
  const pCalib = (() => {

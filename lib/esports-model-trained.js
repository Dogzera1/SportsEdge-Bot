...
function buildVector(ctx, expectedDim) {
  ...
  //   18 = base + cat               (sem extras LoL)
  //   29 = base + lolExtras + cat   (LoL pré-OE)
  //   34 = base + lolExtras + oeExtras + cat   (LoL com OE)
  if (expectedDim === base.length + cat.length) return [...base, ...cat];
  if (expectedDim === base.length + lolExtras.length + cat.length) return [...base, ...lolExtras, ...cat];
...
function predictTrainedEsports(game, ctx) {
  ...
  const calibrated = useCal ? applyIsotonic(W.calibration.blocks, raw) : raw;

...
function buildVector(ctx, expectedDim) {
  ...
  // LoL patch features (2) — populated via lib/lol-patches em buildTrainedContext.
  // Order DEVE match train-esports-model.js LOL_PATCH_FEATURES = ['days_since_patch', 'post_patch_window'].
  // Default daysSincePatch=60 (cap, neutro pós-standardize), postPatchWindow=0.
  const lolPatch = [
    Number.isFinite(+ctx.daysSincePatch) ? +ctx.daysSincePatch : 60,
    Number.isFinite(+ctx.postPatchWindow) ? +ctx.postPatchWindow : 0,
  ];
  ...
  //   43 = base + momentum + lolExtras + oeExtras + playerExtras + lolPatch + cat (LoL patch, c47ecbe 2026-05-13)
  ...
  if (expectedDim === base.length + momentum.length + lolExtras.length + oeExtras.length + playerExtras.length + lolPatch.length + cat.length)
    return [...base, ...momentum, ...lolExtras, ...oeExtras, ...playerExtras, ...lolPatch, ...cat];
  return [...base, ...momentum, ...oneVone, ...lolExtras, ...oeExtras, ...playerExtras, ...lolPatch, ...cat];
}

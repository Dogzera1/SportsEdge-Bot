 ...
 *
 * 2026-05-27 (ATTACK 4): tier-aware shrink via classifyTier (mt-tier-classifier).
 * Shadow stats memory cross_sport_audit mostraram leak severo em Challengers
 * (Spain -100%, EMEA -34%, Pacific -38%). Elo flat sem tier-awareness ficava
 * cego a esta heterogeneidade. Shrink applied AFTER elo blend, shrinks modelP
 * toward 0.5 by tier-specific α. Symmetric — sum P1+P2 preservado em 1.
 */

const { classifyTier } = require('./mt-tier-classifier');

const VALORANT_TIER_SHRINK = {
  tier1_intl: 0.00,        // VCT Masters/Champions — top tier, no shrink
  tier2_franchised: 0.10,  // VCT Americas/EMEA/Pacific — light shrink
  tier3_challengers: 0.50, // Challengers/VCL/Ascension — heavy (Spain -100%, EMEA -34%)
...
function getValorantElo(db, team1, team2, impliedP1, impliedP2) {
  ...
  const minGamesFloor = parseInt(process.env.VAL_MIN_ELO_GAMES || '3', 10);
...
function getValorantModel(db, team1, team2, impliedP1, impliedP2, ctx = {}) {
  ...
  // Tier-aware shrink (ATTACK 4 2026-05-27). Symmetric shrink toward 0.5 — sum preserved.
  // ctx.league required (passed from bot.js caller match.league). No-op if missing.
  let tierShrink = null;
  if (ctx.league && !process.env.VALORANT_TIER_SHRINK_DISABLED) {
    const tier = classifyTier('valorant', ctx.league);
    const alpha = VALORANT_TIER_SHRINK[tier] ?? 0.15;
    if (alpha > 0) {
      elo.modelP1 = elo.modelP1 * (1 - alpha) + 0.5 * alpha;
      elo.modelP2 = elo.modelP2 * (1 - alpha) + 0.5 * alpha;
      // Re-compute edges post-shrink
      elo.edge1 = (elo.modelP1 - (impliedP1 || 0.5)) * 100;
      elo.edge2 = (elo.modelP2 - (impliedP2 || 0.5)) * 100;
      elo.score = Math.max(elo.edge1, elo.edge2);
      // Re-evaluate direction (may flip toward 'none' if edge collapsed)
      elo.direction = elo.edge1 > elo.edge2 && elo.edge1 > 0.5 ? 'p1'
                    : elo.edge2 > elo.edge1 && elo.edge2 > 0.5 ? 'p2' : 'none';
      elo.pass = (elo.found1 && elo.found2) && elo.score >= 1.0;
    }
    tierShrink = { tier, alpha };
  }
  elo.tierShrink = tierShrink;

  // Form: diferencial de win rate recente vs Elo-implied.
  // VALORANT_FORM_WINDOW opt-in: widen p/ liberar form em teams regionais
  // (Spain/Challengers) que ficam null com default 10. Validar shadow 14d antes de subir.
  const formWindow = parseInt(process.env.VALORANT_FORM_WINDOW || '10', 10);
  const form1 = getRecentForm(db, team1, formWindow);
  const form2 = getRecentForm(db, team2, formWindow);

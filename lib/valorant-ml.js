...
/**
 * Modelo completo Valorant: Elo + form recente + in-series adjustment.
 *
...
function getValorantModel(db, team1, team2, impliedP1, impliedP2, ctx = {}) {
  ...
  if (!elo.found1 || !elo.found2) {
    return { ...elo, form1: null, form2: null, inSeriesAdjusted: false };
  }

  // Form: diferencial de win rate recente vs Elo-implied
  const form1 = getRecentForm(db, team1);
  const form2 = getRecentForm(db, team2);
  const eloOnlyP1 = elo.modelP1; // já é blend Elo+implied com eloWeight
  let formAdjust = 0;
  let formFactor = false;
  if (form1 && form2) {
    const wrDiff = form1.winRate - form2.winRate; // -1..+1
    // Peso conservador: 0.10 × diff, cap ±4pp
  ...
  // BoX variance: Bo1 reduz peso Elo (mais ruído)
  const bo = Number(ctx.bo) || 3;
  const boWeight = bo === 1 ? 0.80 : 1.0;

  let mapP1 = eloOnlyP1 * boWeight + impliedP1 * (1 - boWeight) + formAdjust;
  ...
  const direction = edge1 > edge2 && edge1 > 0.5 ? 'p1'
                  : edge2 > edge1 && edge2 > 0.5 ? 'p2' : 'none';
  const factorCount = (elo.found1 ? 1 : 0) + (elo.found2 ? 1 : 0) + (formFactor ? 1 : 0);
...
module.exports = { getValorantElo, getValorantModel, seriesWinProb, getRecentForm, invalidateEloCache, computeEloFromDB };

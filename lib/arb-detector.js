...
function shouldDm(sport, matchKey, marketType, cooldownMs = 60 * 60 * 1000, db = null) {
  const k = `${sport}|${matchKey}|${marketType}`;
  const last = _lastDmAt.get(k) || 0;
  if (Date.now() - last < cooldownMs) return false;
  // Backstop persistente: se DB tem evento mesmo (sport, match_label, market_type)
  // dentro do cooldown, considera já notificado (fonte de verdade cross-restart).
  if (db) {
    try {
      ...
      `).get(sport, matchKey, marketType);
      if (row) {
        _lastDmAt.set(k, Date.now()); // sincroniza memória pra evitar query toda vez
        return false;
      }
    } catch (_) {}
  }

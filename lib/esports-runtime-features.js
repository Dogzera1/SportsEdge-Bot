...
// 2026-05-11 audit P0-2: gaps detectados via test cases reais.
//   tier1 (return 3): originalmente LoL-only (LCK/LPL/etc). Adicionados CS
//     S-tier (IEM/BLAST premier/Cologne/Katowice/ESL Pro League/EWC) que
//     antes caíam como tier "other" → segment_gate tier3 +2pp injustificado
//     (Brier histórico 0.216 ótimo, modelo bom lá).
//   tier2 (return 2): adicionados \bcct\b + challenger league/series.
//     Antes CCT European/South America + ESL Challenger NA caíam como
//     tier3 → +2pp. Real: UNDER 2.5 CCT ROI -100% n=5, ML "regain" ESL
//     Challenger NA ROI -54.5% n=4 — tier2 com +3pp (bumped) é apropriado.
//
// 🔒 INTERNAL / ML-FEATURE-ONLY (2026-05-15 Sprint 2 unification):
//   Esta função usa convenção INVERTIDA (3=top, 2=mid, 1=other) porque
//   modelos ML em prod foram TREINADOS com essa numeração. Mudar quebra
//   feature drift entre train/inference.
//   - PARA paths operacionais (gates, leak guards, reports) → use
//     `lib/league-tier.js getLeagueTier(sport, league)` (canonical 1=top).
//   - PARA paths ML (buildTrainedContext interno) → continua usando
//     este leagueTier diretamente; preserva feature parity com modelo.
//   Pendente sprint futuro: sincronizar regex entre esta função e
//   scripts/extract-esports-features.leagueTier (training pipeline) +
//   considerar retrain do modelo com regex unificado pra eliminar drift.
function leagueTier(league) {
  const l = String(league || '').toLowerCase();
  if (/lck|lpl|lec|lcs|lcp|worlds|msi|\biem\b|blast premier|blast world|blast fall|blast spring|cologne|katowice|esl pro league|esports world cup|\bewc\b/.test(l)) return 3;
  if (/lla|cblol|vcs|pcs|ljl/.test(l)) return 2;
  if (/masters|academy|emea|lpl2|pro league|one|major|dpc|ti|riyadh|epl|esl pro|\bcct\b|challenger league|challenger series/i.test(l)) return 2;
  return 1;
}
...
function buildTrainedContext(db, game, match) {
  ...
  // LoL patch features (2026-05-13 commit c47ecbe wired no training mas runtime
  // ctx não populava — buildVector falhava em weights v43 [+2 patch features]
  // por mismatch dim. days_since_patch via lib/lol-patches; cap em 60d.
  let daysSincePatch = 60, postPatchWindow = 0;
  if (game === 'lol') {
    try {
      const { getDaysSincePatch, isPostPatchWindow } = require('./lol-patches');
      daysSincePatch = getDaysSincePatch(now);
      postPatchWindow = isPostPatchWindow(now);
    } catch (_) { /* lol-patches opcional */ }
  }
  ...
  return {
    ...
    // Patch features (LoL only) — populado via lib/lol-patches.
    daysSincePatch, postPatchWindow,
  };
}

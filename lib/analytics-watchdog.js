...
async function runWatchdog(db, opts = {}) {
  ...
  for (const rule of RULES) {
    ...
    for (const match of matches) {
      // Dedup: suppress se existe alert open OU resolved dentro do cooldown window.
      // Bug fix 2026-05-14: antes só checava idade vs cooldown 24h — após 25h
      // reabria alert idêntico todo dia, gerando 76 open em ~15 sintomas únicos.
      // Bug fix 2026-05-16: antes só checava status='open'. Manual resolve via
      // /admin/alerts-resolve bypassava dedup — próxima run re-fire mesmo alert
      // imediatamente. Agora dedup respeita resolves recentes (cooldown_h).
      // Audit trail preservada via UPDATE metric_value (não cria nova entry).
      const existing = db.prepare(`
        SELECT id, fired_at, metric_value, status, resolved_at FROM analytics_alerts
         WHERE rule_id = ? AND sport = ?
           AND (
             status = 'open'
             OR (status = 'resolved' AND COALESCE(resolved_at, fired_at) > datetime('now', '-' || ? || ' hours'))
           )
         ORDER BY fired_at DESC LIMIT 1
      `).get(rule.id, match.sport, cooldownH);
      if (existing) {
        // Refresh metric_value SÓ se ainda open (resolved não deve mudar). Pro
        // endpoint /admin/alerts mostrar valor atual da condição persistente.
        if (existing.status === 'open') {
          try {
            db.prepare(`UPDATE analytics_alerts SET metric_value = ? WHERE id = ?`)
              .run(match.value, existing.id);
          } catch (_) {}
        }
        // Cooldown ainda relevante pra DM admin (não spammar) — mantido na lib chamadora.
        out.suppressed++;
        continue;
      }
      const alert = {
  ...
  // Cleanup: auto-resolve alerts open >N dias (default 7). Evita acúmulo
  // crônico de alerts antigos sem ação humana. User pode reabrir via novo
  // dispatch quando sintoma reaparecer.
  // 2026-05-16: baixado de 30→7. Snapshot alerts (sharpe/drawdown/calib) fire
  // em point-in-time; quando condição clear, watchdog não re-fire mas alert
  // antigo fica órfão. 30d era too long — alerts de 3 dias ainda open hoje.
  // 7d = balance: dá 1 semana pra humano agir, mas auto-cleanup se ignorado.
  const cleanupDays = parseInt(process.env.WATCHDOG_AUTO_RESOLVE_DAYS || '7', 10);
  if (cleanupDays > 0) {
    try {
      const cleanup = db.prepare(`
        UPDATE analytics_alerts
           SET status='resolved',
               ...
               context_json=json_patch(COALESCE(context_json,'{}'), json_object('resolved_reason','auto_age_expired'))
         WHERE status='open' AND fired_at <= datetime('now', '-' || ? || ' days')
      `).run(cleanupDays);
      out.auto_resolved = cleanup.changes;
    } catch (_) { out.auto_resolved = 0; }
  }
  return out;
}
...
async function runPnlReport(db, opts = {}) {
  ...
  // 2026-05-20: UNION ALL com market_tips_shadow.
  // Bug reportado pelo user — tennis MT tips ausentes do PnL.
  // - sd.tips: ML real (is_shadow=0, archived=0). profit_reais em R$ → per-tip ratio.
  // - sd.market_tips_shadow: MT dispatched (admin_dm_sent_at NOT NULL → user recebeu).
  //   profit_units já em units. Shadow puro (sem admin_dm_sent_at) NÃO entra
  //   pra não inflar ROI com tips research-only.
  // 2026-05-28: ML profit_reais (BRL) convertido pra units via stake_reais/stake_units
  // per-tip (uv tier-aware embedded). unitValue global não captura per-sport tier
  // crossing (lib/sport-unit). Fallback unitValue global quando stake_reais NULL
  // (tips legacy pre-mig 026).
  const unifiedSettledCte = `
    WITH unified AS (
      SELECT settled_at, sport, result,
             CASE
               WHEN COALESCE(stake_reais, 0) > 0
                AND CAST(REPLACE(REPLACE(stake, 'u', ''), 'U', '') AS DOUBLE) > 0
               THEN COALESCE(profit_reais, 0)
                  * CAST(REPLACE(REPLACE(stake, 'u', ''), 'U', '') AS DOUBLE)
                  / stake_reais
               ELSE COALESCE(profit_reais, 0) / ${unitValue}
             END AS profit_units,
             CASE
               WHEN COALESCE(stake_reais, 0) > 0
                AND CAST(REPLACE(REPLACE(stake, 'u', ''), 'U', '') AS DOUBLE) > 0
               THEN CAST(REPLACE(REPLACE(stake, 'u', ''), 'U', '') AS DOUBLE)
               ELSE COALESCE(stake_reais, 0) / ${unitValue}
             END AS stake_units
        FROM sd.tips
  ...
  const monthly = await query(`
    ...
    SELECT strftime(CAST(settled_at AS TIMESTAMP), '%Y-%m') AS month,
      ...
      FROM unified
     WHERE CAST(settled_at AS TIMESTAMP) >= (now() - INTERVAL '180' DAY)
       ${sportFilter}
  ...
  // Per-sport monthly (current month only, top 8)
  ...
  // Pending exposure (units abertos — ML real + MT dispatched ainda sem result)
  // 2026-05-28: ML stake_reais (BRL) convertido pra units per-tip (parse stake text).
  const pending = await query(`
    WITH pending_unified AS (
      SELECT CASE
               WHEN COALESCE(stake_reais, 0) > 0
                AND CAST(REPLACE(REPLACE(stake, 'u', ''), 'U', '') AS DOUBLE) > 0
               THEN CAST(REPLACE(REPLACE(stake, 'u', ''), 'U', '') AS DOUBLE)
               ELSE COALESCE(stake_reais, 0) / ${unitValue}
             END AS stake_units, sport
        FROM sd.tips
       WHERE result IS NULL
         AND COALESCE(is_shadow, 0) = 0
         AND COALESCE(archived, 0) = 0
      UNION ALL
      SELECT stake_units, sport
        FROM sd.market_tips_shadow
       WHERE result IS NULL
         AND admin_dm_sent_at IS NOT NULL
    )
    SELECT ROUND(SUM(stake_units), 2) AS pending_units,
           COUNT(*) AS n
      FROM pending_unified

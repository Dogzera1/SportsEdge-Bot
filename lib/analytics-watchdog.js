...
async function runWatchdog(db, opts = {}) {
  ...
  for (const rule of RULES) {
    ...
    for (const match of matches) {
      // Dedup: se já existe open pra (rule, sport), SEMPRE suppress (não reabrir).
      ...
      // Audit trail preservada via UPDATE metric_value + fired_at (last_seen).
      const existing = db.prepare(`
        SELECT id, fired_at, metric_value FROM analytics_alerts
         WHERE rule_id = ? AND sport = ? AND status = 'open'
         ORDER BY fired_at DESC LIMIT 1
      `).get(rule.id, match.sport);
      if (existing) {
        // Refresh metric_value pro endpoint /admin/alerts mostrar valor atual.
        // Não cria entry nova — só atualiza in-place.
        try {
        ...
        const ageMs = Date.now() - new Date(existing.fired_at).getTime();
        if (ageMs < cooldownMs) { out.suppressed++; continue; }
        // Cooldown expirou, mas alert já open: suppress mesmo assim.
        // Resolver via /admin/alerts-resolve quando sintoma corrigido.
        out.suppressed++;
        continue;
      }
      const alert = {

...
async function runPnlReport(db, opts = {}) {
  ...
  // - sd.tips: ML real (is_shadow=0, archived=0). profit_reais em R$ → /unitValue.
  ...
  const unifiedSettledCte = `
    WITH unified AS (
      SELECT settled_at, sport, result,
             profit_reais / ${unitValue} AS profit_units,
             stake_reais / ${unitValue} AS stake_units
        FROM sd.tips
  ...
  const pending = await query(`
    WITH pending_unified AS (
      SELECT stake_reais / ${unitValue} AS stake_units, sport
        FROM sd.tips

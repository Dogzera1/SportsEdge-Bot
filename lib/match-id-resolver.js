...
const DATE_WINDOW_DAYS = (() => {
  const v = parseInt(process.env.MATCH_ID_ALIAS_DATE_WINDOW || '2', 10);
  return Number.isFinite(v) && v >= 0 && v <= 7 ? v : 2;
})();
...
function resolveAlias(db, matchId, game) {
  ...
  try {
    const cached = db.prepare(
      `SELECT canonical_match_id FROM match_id_aliases WHERE alias = ? AND game = ?`
    ).get(matchId, game);
    if (cached?.canonical_match_id) return cached.canonical_match_id;
  } catch (e) {

 ...
 */
function mapScoreDistribution(pMap, bestOf) {
 ...
 */
function handicapProb(pMap, bestOf, line) {
  const dist = mapScoreDistribution(pMap, bestOf);
  const bo = Number(bestOf) || 1;
  const N = bo <= 1 ? 1 : Math.ceil(bo / 2);
  const L = Number(line);
 ...
 */
function totalMapsProb(pMap, bestOf, line) {
  const dist = mapScoreDistribution(pMap, bestOf);
  const L = Number(line);

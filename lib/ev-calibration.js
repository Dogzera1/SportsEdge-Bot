...
function refreshEvCalibration(db) {
  if (/^(0|false|no)$/i.test(String(process.env.EV_CALIB ?? ''))) {
    _cache = { mtime: Date.now(), bySportBucket: new Map(), byBucketGlobal: new Map(), disabled: true };
    return _cache;
  }
  ...
  const aggSport = new Map();
  const aggGlobal = new Map();
  for (const r of rows) {
    ...
    if (sp) {
      const k = `${sp}|${idx}`;
      const cur = aggSport.get(k) || { n: 0, stake: 0, profit: 0 };
      cur.n++; cur.stake += r.stake; cur.profit += r.profit;
      aggSport.set(k, cur);
    }
  ...
  _cache = { mtime: Date.now(), bySportBucket: bySport, byBucketGlobal: byGlobal, disabled: false };
  return _cache;
}

// Returns mult [minMult, 1] or null se não tem amostra suficiente.
// Caller deve fallback pra throttle/1.0 quando null.
function getEvCalibrationMult(sport, evPct) {

 ...
 * Reutiliza match_results WHERE game IN ('cs','cs2') — settle/sync gravam 'cs2'
 * (28k rows); filtro antigo game='cs' casava 0 rows → Elo sempre found=false
 * e tips CS reais caíam em prob genérica (bug C4, audit causal 2026-06-09).
 *
...
function computeEloFromDB(db) {
  const rows = db.prepare(`
    ...
    FROM match_results
    WHERE game IN ('cs', 'cs2') AND winner IS NOT NULL AND winner != ''

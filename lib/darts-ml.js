...
function dartsPreFilter(match, enrich) {
  ...
  // 2026-05-09: pass=true quando enrich vazio (Pinnacle-only via getPinnacleDartsMatches —
  // sem playerId Sofascore pra hidratar avg/wr/H2H). Sem isso, factorCount=0 → pass=false →
  // pollDarts pula antes do trained model + IA, mesmo Pinnacle tendo odds sharp.
  // Modelo trained em bot.js:20585 + EV gate (DARTS_MIN_EV) + IA second-opinion filtram
  // downstream. ModelP=impliedP (de-juice puro) é ponto de partida válido.
  const _enrichEmpty = factorCount === 0
    && !Number.isFinite(avgP1) && !Number.isFinite(avgP2)
    && (gP1 || 0) === 0 && (gP2 || 0) === 0;
  const pass = _enrichEmpty
    ? true  // Pinnacle-only — deixa trained+EV gate filtrar
    : (score >= minEdge && factorCount >= 1);

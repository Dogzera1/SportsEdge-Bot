...
function dartsPreFilter(match, enrich) {
  ...
  // Requer ≥3 matches entre eles. Peso conservador (0.10) × confiança amostra.
  // Saturação em ±0.3 bias → ±3pp bruto máximo.
  const h2hP1 = Number(enrich?.h2hP1Wins);
  const h2hP2 = Number(enrich?.h2hP2Wins);
  if (Number.isFinite(h2hP1) && Number.isFinite(h2hP2) && (h2hP1 + h2hP2) >= 3) {

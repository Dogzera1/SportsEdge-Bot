...
// Reverse: code → nome amigável p/ match_results.league. Antes salvava 'E0'
// (code) e o trained Poisson filtra por substring "Premier League" — não bateu.
const CODE_TO_NAME = {
  E0: 'Premier League', E1: 'England Championship', E2: 'England League One',
  SP1: 'La Liga', SP2: 'Spain Segunda',
  D1: 'Bundesliga', D2: 'Germany 2.Bundesliga',
  I1: 'Serie A', I2: 'Italy Serie B',
  F1: 'Ligue 1', F2: 'France Ligue 2',
  N1: 'Eredivisie', B1: 'Belgium Pro League',
  P1: 'Portugal Primeira Liga', T1: 'Turkey Super Lig',
  G1: 'Greece Super League', SC0: 'Scotland Championship',
};
...
async function syncLeagueSeason(db, league, season, opts = {}) {
  ...
  // Mig 109 FASE 1 dual-source: log source 'football_data_csv' pra reconcile
  try {
    mrSourceInsert = db.prepare(`
      INSERT INTO match_result_sources (match_id, game, source, team1, team2, winner, final_score)
      VALUES (?, 'football', 'football_data_csv', ?, ?, ?, ?)
    `);
  } catch (_) {}
      ...
      try {
        ...
        if (mrUpsert) {
          ...
          if (Number.isFinite(fthg) && Number.isFinite(ftag) && /^[HDA]$/.test(ftr)) {
            ...
            if (resolvedAt) {
              const friendlyLeague = CODE_TO_NAME[r.league] || r.league;
              mrUpsert.run(matchId, home, away, winner, score, friendlyLeague, resolvedAt);
              if (mrSourceInsert) { try { mrSourceInsert.run(matchId, home, away, winner, score); } catch (_) {} }
              mrInserted++;
            }
          }
        }

...
function scanFootballMarkets({ pinMarkets, trainedMarkets, minEv = 5, maxEv = 25, minPmodel = 0.45, minOdd = 1.50, maxOdd = 4.00, shadowMinEv } = {}) {
  ...
  if (Number.isFinite(shadowMinEv) && shadowMinEv < minEv) {
    const promotable = scanFootballMarkets({
      pinMarkets, trainedMarkets, minEv, maxEv, minPmodel, minOdd, maxOdd,
    });
    const shadow = scanFootballMarkets({
      pinMarkets, trainedMarkets,
      minEv: shadowMinEv,
      maxEv: 999,
      minPmodel: 0.10, // captura tips low-prob
      minOdd: 1.01,
      maxOdd: 99,
    });
  ...
  for (const t of totals) {
    ...
    // 2026-05-22: pModelRaw = pModel (football Poisson trained sem isotonic post-fit;
    // calib factor já aplicado ao trained — mig 117 audit cross-sport).
    if (Number.isFinite(overEv) && overEv >= minEv && overEv <= maxEv && probs.over >= minPmodel
      ...
      tips.push({
        ...
        pModelRaw: +probs.over.toFixed(4),
    ...
    if (Number.isFinite(underEv) && underEv >= minEv && underEv <= maxEv && probs.under >= minPmodel
      ...
      tips.push({
        ...
        pModelRaw: +probs.under.toFixed(4),
    ...
    for (const v of variants) {
      ...
      if (Number.isFinite(evN) && evN >= minEv && evN <= maxEv && v.trainedP >= minPmodel
        ...
        tips.push({
          ...
          pModelRaw: +v.trainedP.toFixed(4),
  ...
  if (pinMarkets.btts && trainedMarkets.btts) {
    ...
    if (Number.isFinite(yesEv) && yesEv >= minEv && yesEv <= maxEv && trainedMarkets.btts.yes >= minPmodel
      ...
      tips.push({
        ...
        pModelRaw: +trainedMarkets.btts.yes.toFixed(4),
    ...
    if (Number.isFinite(noEv) && noEv >= minEv && noEv <= maxEv && trainedMarkets.btts.no >= minPmodel
      ...
      tips.push({
        ...
        pModelRaw: +trainedMarkets.btts.no.toFixed(4),
  ...
  for (const h of handicaps) {
    ...
    if (Number.isFinite(homeEv) && homeEv >= minEv && homeEv <= maxEv && probs.home >= minPmodel
      ...
      tips.push({
        ...
        pModelRaw: +probs.home.toFixed(4),
    ...
    if (Number.isFinite(awayEv) && awayEv >= minEv && awayEv <= maxEv && probs.away >= minPmodel
      ...
      tips.push({
        ...
        pModelRaw: +probs.away.toFixed(4),

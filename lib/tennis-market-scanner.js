...
function scanTennisMarkets({ markov, aces, doubleFaults, markets, minEv = 4, bestOf = 3, maxEv = 40, maxEvPerMarket = null, minOdd, maxOdd, maxPerMatch, isLive = false, gamesScale = 1.0 } = {}) {
  ...
  for (const t of (markets.totals || [])) {
    ...
    // gamesScale<1.0 → effectiveLine maior → pOver menor (modelo expects fewer games).
    const _effLine = _gamesScale === 1.0 ? t.line : t.line / _gamesScale;
    const pOverRaw = _pOverFromPdf(markov.totalGamesPdf, _effLine);
    if (pOverRaw == null) continue;
    const pOver = applyMarkovCalib(pOverRaw, 'totalGames', calibOpts);
    ...
    if (evSafe(evO, pOver, t.oddsOver, 'totalGames') && evO >= minEv) {
      tips.push({
        ...
        stakeAdjust: +_stakeAdjust(pOver, pOverRaw).toFixed(3),

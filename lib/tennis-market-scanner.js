...
function scanTennisMarkets({ markov, aces, doubleFaults, markets, minEv = 4, bestOf = 3, maxEv = 40, maxEvPerMarket = null, minOdd, maxOdd, maxPerMatch, isLive = false, gamesScale = 1.0, shadowMinEv, tier } = {}) {
  ...
  const evSafe = (ev, p, odd, mkt) =>
    ev != null && ev <= _maxEvFor(mkt) && p != null && p < maxPModel
  ...
  for (const t of (markets.totals || [])) {
    ...
    if (evSafe(evO, pOver, t.oddsOver, 'totalGames') && evO >= minEv) {
      tips.push({
    ...
    if (evSafe(evU, pUnder, t.oddsUnder, 'totalGames') && evU >= minEv) {
      tips.push({
  ...
  if (enableGamesHandicap && markov.gamesMarginPdf) {
    for (const h of gamesHandicapsSource) {
      ...
      if (evSafe(evH, pT1, h.oddsHome, 'handicapGames') && evH >= minEv) {
        tips.push({
      ...
      if (evSafe(evA, pT2, h.oddsAway, 'handicapGames') && evA >= minEv) {
        tips.push({

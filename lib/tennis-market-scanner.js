 ...
 */
function scanTennisMarkets({ markov, aces, markets, minEv = 4, bestOf = 3, maxEv = 40, minOdd, maxOdd, maxPerMatch } = {}) {
  ...
  const evSafe = (ev, p, odd) =>
    ev != null && ev <= maxEv && p != null && p < 0.985
  ...
  //
  // BUG FOUND 2026-04-23: Pinnacle period=0 "handicap" em tennis é QUASE SEMPRE
  // GAMES handicap, não sets. Lines tipo ±1.5/±2.5 em Pinnacle tennis são games
  // handicap. Scanner precificava via setDist → p_model 0.95+ (home não bageled)
  // contra odd real de games ~1.77 → EV aparente massive. Settlement contava
  // sets → match com condição diferente da odd → 97% hit rate fake, CLV -2.8%.
  //
  // Fix temporário: desativa pricing de handicapSets até scanner diferenciar
  // games vs sets handicap via Pinnacle market metadata. Override pra re-ativar
  // (após distinguir): TENNIS_HANDICAP_SETS_ENABLED=true.
  const handicapSetsEnabled = /^(1|true|yes)$/i.test(String(process.env.TENNIS_HANDICAP_SETS_ENABLED || 'false'));
  const handicapIterable = handicapSetsEnabled ? (markets.handicaps || []) : [];
  for (const h of handicapIterable) {

...
// 2026-05-06 FIX: usar devigEnsemble (auto power/Shin) em vez de
// devigMultiplicative. ML em bot.js já usa ensemble; manter MT em
// multiplicativo gerava EV inflado em underdog assimétrico (1.30 vs 4.50)
// pelo bias do método proporcional.
...
// 2026-05-29: devig method configurable. Default 'auto' (no-op) = ensemble picks
// Shin for asymmetric 2-way (|o1-o2|>=1.5) else power. Force via TENNIS_MT_DEVIG_METHOD
// ('shin'/'power'/'multiplicative'). Read at module load (restart to change).
const _DEVIG_METHOD = String(process.env.TENNIS_MT_DEVIG_METHOD || 'auto').toLowerCase();
function _dej(a, b) {
 ...
 */
function scanTennisMarkets({ markov, aces, doubleFaults, markets, minEv = 4, bestOf = 3, maxEv = 40, maxEvPerMarket = null, minOdd, maxOdd, maxPerMatch, isLive = false, gamesScale = 1.0, shadowMinEv, tier } = {}) {
  if (!markov || !markets) return [];
  // 2026-05-04: shadow puro mode — quando shadowMinEv passado < minEv, retorna
  // OBJ { promotable, shadow } via dupla chamada recursiva:
  //   - promotable: opts originais (todos gates ativos)
  //   - shadow: gates relaxados (minEv=shadowMinEv, maxEv=999, oddFloor 1.01,
  //             sem oddCap, sem maxPerMatch) — captura quase tudo positivo
  // Pra back-compat (caller não passa shadowMinEv), retorna array como antes.
  if (Number.isFinite(shadowMinEv) && shadowMinEv < minEv) {
    const promotable = scanTennisMarkets({
      markov, aces, doubleFaults, markets, minEv, bestOf, maxEv, maxEvPerMarket,
      minOdd, maxOdd, maxPerMatch, isLive, gamesScale, tier,
      // shadowMinEv omitted → recursive call returns array (não retorna split)
    });
    const shadow = scanTennisMarkets({
      markov, aces, doubleFaults, markets,
      minEv: shadowMinEv,
      bestOf, maxEv: 999, maxEvPerMarket: null,
      minOdd: 1.01, maxOdd: undefined, maxPerMatch: undefined,
      isLive, gamesScale, tier,
    });
    return { promotable, shadow };
  }
  const tips = [];
  // 2026-05-07: tier propagado pra applyMarkovCalib usa bins per-tier quando
  // calib v2 disponível (schema markets.X.tiers[tier]). Causa-fix Challenger leak.
  // 2026-05-25: format ('bo3'/'bo5') derivado de bestOf. Calib v3 separa
  // ATP main Bo5 Slam (R1 leak -30.8pp) de ATP main Bo3 (Masters/500/250).
  // Fallback transparente: se v3 bins ausentes, cai pra tier-only v2.1.
  const calibOpts = {
    isLive: !!isLive,
    tier: tier || undefined,
    format: bestOf >= 5 ? 'bo5' : 'bo3',
  };
  // Per-market EV cap override — quando shadow data mostra que market X tolera
  // EV alto (handicapGames ROI +10% em EV>30% n=118 prod 30d) enquanto market Y
  // vaza (totalGames over -22.9% n=56), permite ceiling diferente.
  // Fallback no maxEv genérico se entry não setada.
  const _maxEvFor = (mkt) => {
    if (maxEvPerMarket && Number.isFinite(maxEvPerMarket[mkt])) return maxEvPerMarket[mkt];
    return maxEv;
  };
  // 2026-04-29 Sprint 2.1: gender/tour-aware shrink. Audit 7d: ATP men's
  // non-Slam Bo3 totalGames OVER -23.7% ROI (n=73), WTA mesma janela +5%.
  // Modelo subestima holds em ATP → over-prediz games. Shrink line × 1/scale
  // (scale=0.95 → effectiveLine=line*1.053 → pOver desce ~6pp).
  const _gamesScale = Number.isFinite(gamesScale) && gamesScale > 0 && gamesScale <= 1.2 ? gamesScale : 1.0;
  // stakeAdjust: haircut em [0.4, 1.0] proporcional a (p_calib/p_raw). Quando
  // calib encolhe pModel significativamente (ex: 0.85→0.61 = ratio 0.72),
  // a precisão pontual do model é menor → Kelly sobre-aposta. Caller usa
  // stake_final = kelly_stake * stakeAdjust pra comprimir exposição em tips
  // onde calib teve correção forte. Tips sem calib (ratio=1) ficam intactas.
  // Floor 0.4 evita matar Kelly mesmo em correções extremas.
  const _stakeAdjust = (pCalib, pRaw) => {
    if (!Number.isFinite(pCalib) || !Number.isFinite(pRaw) || pRaw <= 0) return 1;
    const ratio = pCalib / pRaw;
  ...
  // 4º arg `mkt` opcional — habilita per-market cap. Fallback: maxEv global.
  // 2026-05-19 extreme-line guard (5º arg `line`): audit logs 2026-05-19T10:08-10:28Z
  // mostrou 12 MT-EV-CAP WARN/20min em handicapGames/totalGames com line/total
  // extremo (EV 32-483%). Markov calib v2 não tem dim line-bucket — sample sparse
  // (412/90d total vs ~6/bucket+tier+side) inviabiliza refit v3 estatístico.
  // Heurística: |line|>5.5 (handicapGames) OU |line-22.5|>6.5 (totalGames)
  // = "extreme" → EV cap 30% (vs maxEv global 40%, shadow 999). Bloqueia tip
  // emit no scanner em vez de deixar passar pra MT-EV-CAP downstream gerar WARN.
  // Disable via TENNIS_EXTREME_LINE_GUARD_DISABLED=true.
  const _extremeLineGuardEnabled = !/^(1|true|yes)$/i.test(String(process.env.TENNIS_EXTREME_LINE_GUARD_DISABLED || ''));
  const _extremeLineEvCap = parseFloat(process.env.TENNIS_EXTREME_LINE_EV_CAP_PCT || '30');
  const _isExtremeLine = (mkt, line) => {
    if (!_extremeLineGuardEnabled || !Number.isFinite(line)) return false;
    if (mkt === 'handicapGames' && Math.abs(line) > 5.5) return true;
    if (mkt === 'totalGames' && Math.abs(line - 22.5) > 6.5) return true;
    return false;
  };
  const evSafe = (ev, p, odd, mkt, line) => {
    const effMaxEv = _isExtremeLine(mkt, line)
      ? Math.min(_maxEvFor(mkt), _extremeLineEvCap)
      : _maxEvFor(mkt);
    return ev != null && ev <= effMaxEv && p != null && p < maxPModel
  ...
  for (const t of (markets.totals || [])) {
    ...
    // 2026-05-03 FIX: gamesScale aplicado simétrico em over+under inflava pUnder
    // ao corrigir leak só do over. Agora over usa scaled line; under usa line raw
    // pra evitar gerar tip side oposta artificialmente. ATP non-Slam Bo3 model
    // over-predizia games — correção só puxa pOver pra baixo, deixa pUnder honesto.
    const pOverRaw = _pOverFromPdf(markov.totalGamesPdf, t.line);
    if (pOverRaw == null) continue;
    const _effLineOver = _gamesScale === 1.0 ? t.line : t.line / _gamesScale;
    const pOverRawScaled = _gamesScale === 1.0 ? pOverRaw : _pOverFromPdf(markov.totalGamesPdf, _effLineOver);
    const pOver = applyMarkovCalib(pOverRawScaled ?? pOverRaw, 'totalGames', { ...calibOpts, side: 'over' });
    const dj = _dej(t.oddsOver, t.oddsUnder);
    const evO = _ev(pOver, t.oddsOver);
    if (evSafe(evO, pOver, t.oddsOver, 'totalGames', t.line) && evO >= minEv) {
      tips.push({
        market: 'totalGames', line: t.line, side: 'over',
        pModel: +pOver.toFixed(4),
        pModelRaw: +(pOverRawScaled ?? pOverRaw).toFixed(4),
        pImplied: dj ? +dj.pA.toFixed(4) : null,
        odd: t.oddsOver, ev: +evO.toFixed(2),
        stakeAdjust: +_stakeAdjust(pOver, pOverRawScaled ?? pOverRaw).toFixed(3),
        label: `Over ${t.line} games`,
      });
    }
    // Under usa pOverRaw (line original sem scale) → pUnder honesto.
    const pUnderRaw = 1 - pOverRaw;
    const pUnder = applyMarkovCalib(pUnderRaw, 'totalGames', { ...calibOpts, side: 'under' });
    const evU = _ev(pUnder, t.oddsUnder);
    if (evSafe(evU, pUnder, t.oddsUnder, 'totalGames', t.line) && evU >= minEv) {
      tips.push({
        ...
        stakeAdjust: +_stakeAdjust(pUnder, pUnderRaw).toFixed(3),
  ...
  if (enableGamesHandicap && markov.gamesMarginPdf) {
    for (const h of gamesHandicapsSource) {
      ...
      // 2026-05-11: schema v2.1 side-aware. HOME/AWAY tem dinâmicas diferentes
      // (serve adv, court familiarity); pT1=home, pT2=away. Mesmo padrão p/ totalGames.
      const pT1 = applyMarkovCalib(pT1Raw, 'handicapGames', { ...calibOpts, side: 'home' });
      const dj = _dej(h.oddsHome, h.oddsAway);
      const pT2Raw = 1 - pT1Raw;
      const pT2 = applyMarkovCalib(pT2Raw, 'handicapGames', { ...calibOpts, side: 'away' });
      const evH = _ev(pT1, h.oddsHome);
      if (evSafe(evH, pT1, h.oddsHome, 'handicapGames', h.line) && evH >= minEv) {
        tips.push({
          ...
          stakeAdjust: +_stakeAdjust(pT1, pT1Raw).toFixed(3),
          label: `Handicap ${h.line >= 0 ? '+' : ''}${h.line} games team1`,
        });
      }
      const evA = _ev(pT2, h.oddsAway);
      if (evSafe(evA, pT2, h.oddsAway, 'handicapGames', h.line) && evA >= minEv) {
        tips.push({
          ...
          stakeAdjust: +_stakeAdjust(pT2, pT2Raw).toFixed(3),
  ...
  // 2026-05-29 EDGE-SHRINK pass: anchor each tip's model prob toward its devigged
  // fair line (pImplied) to counter adverse selection (EV gate selecting the model's
  // overconfident errors), then re-gate on minEv. Per-market shrink via
  // TENNIS_<TAG>_EDGE_SHRINK > TENNIS_MT_EDGE_SHRINK > 1.0 (default no-op).
  // DRY single pass over all markets; `tips` is const → mutate in place.
  {
    const kept = [];
    for (const t of tips) {
      const s = resolveShrink(t.market);
      if (s < 1 && t.pImplied != null) {
        const pUsed = applyEdgeShrink(t.pModel, t.pImplied, s);
        const ev = (pUsed * t.odd - 1) * 100;
        if (!(ev >= minEv)) continue; // re-gate: drop tips that no longer clear the bar
        t.pModel = +pUsed.toFixed(4);
        t.ev = +ev.toFixed(2);
      }
      kept.push(t);
    }

...
module.exports = function initDatabase(dbPath = 'sportsedge.db') {
  ...
  const stmts = {
    ...
    `),
    getUnsettledTips: db.prepare(`SELECT * FROM tips WHERE sport = ? AND result IS NULL AND sent_at > datetime('now', ?) AND (archived IS NULL OR archived = 0)`),
    ...
    getCalibration: db.prepare(`
      ...
      FROM tips WHERE sport = ? AND result IS NOT NULL AND result != 'void' GROUP BY confidence
    `),

...
module.exports = function initDatabase(dbPath = 'sportsedge.db') {
  ...
  const stmts = {
    ...
    getTeamForm: db.prepare(`SELECT * FROM match_results WHERE (team1 = ? OR team2 = ?) AND game = ? ORDER BY resolved_at DESC LIMIT 10`),
    getH2H: db.prepare(`SELECT * FROM match_results WHERE ((team1 = ? AND team2 = ?) OR (team1 = ? AND team2 = ?)) AND game = ? ORDER BY resolved_at DESC LIMIT 10`),

...
module.exports = function initDatabase(dbPath = 'sportsedge.db') {
  ...
  // 'esports' é legado (tips LoL+Dota combinadas). 'lol' e 'dota2' são os novos
  // buckets separados — seeded em R$100 independentemente. Use env
  // LOL_INITIAL_BANCA / DOTA2_INITIAL_BANCA pra valor diferente no 1º boot.
  const lolInit = parseFloat(process.env.LOL_INITIAL_BANCA || '100') || 100;
  const dotaInit = parseFloat(process.env.DOTA2_INITIAL_BANCA || '100') || 100;
  ...
  db.prepare('INSERT OR IGNORE INTO bankroll (sport, initial_banca, current_banca) VALUES (?, ?, ?)').run('lol', lolInit, lolInit);
  db.prepare('INSERT OR IGNORE INTO bankroll (sport, initial_banca, current_banca) VALUES (?, ?, ?)').run('dota2', dotaInit, dotaInit);

...
module.exports = function initDatabase(dbPath = 'sportsedge.db') {
  ...
  // 2026-05-10: foreign_keys=ON pra integrity referencial (P0 audit). Default
  // SQLite é OFF — risk orphans em tip_factor_log→tips, market_tips_shadow→
  // matches. Opt-out via DB_FOREIGN_KEYS=off pra debug emergência (não ativar
  // em prod). Mig 096 backfilou is_shadow + UNIQUE constraint pré-flag.
  if (!/^(0|false|off|no)$/i.test(String(process.env.DB_FOREIGN_KEYS || ''))) {
    try { db.pragma('foreign_keys = ON'); } catch (e) { console.log(`[db] foreign_keys on falhou: ${e.message}`); }
  }
  ...
  // 2026-05-07 (OOM fix — boot-diag confirmou SIGKILL com null exit files):
  // SQLite cache_size/mmap_size sem cap = SQLite cresce livremente em queries
  // pesadas (settle batch, propagator scan, calib refit). Em Railway 512MB hobby
  // tier, com DB 151MB on disk + Node baseline ~50MB, qualquer burst >300MB
  // estoura o cap → kernel SIGKILL antes de exit handlers rodarem. Boot loop
  // observado tem gaps de 53s durante init.
  //
  // - cache_size = -8000 (negativo = KB) → cap 8MB (default better-sqlite3 ~2MB
  //   mas SQLite pode crescer durante big query). Override DB_CACHE_KB.
  // - mmap_size = 0 desliga mmap. Default better-sqlite3 = 0 mas alguns builds
  //   ativam. Mmap em DB 151MB pode mappear pra address space + paginar via OS,
  //   mas RSS vai indicar full mapping. Off é safer pra constrained env.
  // - Override pode ser DB_MMAP_MB > 0 pra reativar quando bot rodar em ambiente
  //   com mais memória.
  const cacheKb = Math.max(1024, parseInt(process.env.DB_CACHE_KB || '8000', 10) || 8000);
  try { db.pragma(`cache_size = -${cacheKb}`); } catch (_) {}
  const mmapMb = parseInt(process.env.DB_MMAP_MB || '0', 10) || 0;
  try { db.pragma(`mmap_size = ${mmapMb * 1024 * 1024}`); } catch (_) {}
  // 2026-05-28 audit-banco P1: auto_vacuum=INCREMENTAL pra reclamar páginas deletadas.
  // Default NONE deixava a freelist crescer indefinido (219MB de 387MB em prod, 56%
  // bloat — amplificava RSS e o risco de container-kill no Railway 512MB).
  // ATENÇÃO: num DB já existente com auto_vacuum=NONE, este PRAGMA só MUDA O MODO
  // após 1 VACUUM full (rodar manual em janela de baixo tráfego: backup -> VACUUM).
  // Depois, PRAGMA incremental_vacuum (no fim de runAuditTablesRetention) reclama
  // gradualmente sem lock longo. Em DB novo (CI/dev) aplica direto. Opt-out DB_AUTO_VACUUM=off.
  if (!/^(0|false|off|no)$/i.test(String(process.env.DB_AUTO_VACUUM || ''))) {
    try { db.pragma('auto_vacuum = INCREMENTAL'); } catch (_) {}
  }
  // Log effective settings pra audit no /admin/boot-diag pós-fix
  try {
    const effCache = db.pragma('cache_size', { simple: true });
    const effMmap = db.pragma('mmap_size', { simple: true });
    console.log(`[db] memory caps: cache_size=${effCache} (req ${-cacheKb}) mmap_size=${effMmap} (req ${mmapMb * 1024 * 1024})`);
  } catch (_) {}
  ...
  // ── Initial schema DDL (v1 baseline) + applyMigrations ──────────────────
  // Architecture (dual-source, 2026-05-15 Sprint 3 #2 documented):
  //   1. db.exec() abaixo cria tables em v1 baseline (snapshot do schema
  //      original quando o bot foi escrito). USA IF NOT EXISTS — re-execução
  //      em DBs existentes é no-op.
  //   2. applyMigrations(db) (linha ~369) aplica 109+ migrations idempotentes
  //      que evoluem v1 → schema atual (adds columns, indexes, new tables).
  //
  // Por que dual-source: refactor pra source-of-truth única seria refactor
  // grande (~600 LoC de DDL + 110 migs). Atualmente arquitetura é estável
  // e testada via tests/test-schema-boot-integrity.js (catches regression
  // se mig depender de coluna não criada pelo DDL inicial).
  //
  // Adicionando NEW tables: PREFERIR migration (não modificar este DDL).
  // Migrations rodam em fresh + existing DBs igualmente; DDL inicial só
  // ajuda fresh boot, mas migration via CREATE TABLE IF NOT EXISTS é
  // equivalente — sem necessidade de touchar este bloco.
  db.exec(`
    ...
    -- 2026-05-07 (audit P2): PK (match_id, game) cobre lookup por match_id+game,
    -- mas N queries filtram POR game primeiro: WHERE game = 'tennis' / 'football' / etc.
    -- Sem índice em game, são full table scans (millions of rows). Índice
    -- (game, resolved_at DESC) cobre tanto WHERE game = ? AND match_id = ? quanto
    -- queries de "últimas resolved" + COUNT por sport (server.js:8199-8201).
    CREATE INDEX IF NOT EXISTS idx_match_results_game_resolved
      ON match_results(game, resolved_at DESC);
    CREATE INDEX IF NOT EXISTS idx_match_results_game_team1
      ON match_results(game, team1);
  ...
  // 2026-05-06 FIX: catch silenciava erro mesmo o comentário dizendo "não silencia".
  // Mig 090 estava broken (UNIQUE+COALESCE rejeitada por SQLite) e bloqueava 091
  // sem ninguém saber. Agora: log + flag global pra /health expor + re-throw em prod.
  try {
    const migResult = applyMigrations(db);
    if (migResult.applied > 0) {
      console.log(`[DB] Applied ${migResult.applied} migrations`);
    }
  } catch (migErr) {
    console.error('[DB] ERRO em applyMigrations:', migErr.message);
    console.error('[DB] Schema may be incomplete. Monitor /health for alerts.');
    try { global._schemaIncomplete = String(migErr?.message || migErr); } catch (_) {}
    // Em prod com FAIL_ON_MIGRATION_ERROR=true derruba boot pra Railway restartar.
    // Default: segue o boot (DB pode operar com schema 1-2 migs atrás) mas marca incompleto.
    if (process.env.FAIL_ON_MIGRATION_ERROR === 'true') throw migErr;
  }
  ...
  // 2026-05-06 FIX: adicionado basket/cs/cs2/valorant/darts/snooker/tabletennis
  // — antes lazy create via server.js gerava drift no baseline (mig 041/043/044
  // recomputam baseline = SUM(initial_banca); se sport não existe no momento,
  // baseline pula de 1000→1100 sem changelog quando lazy-criado depois).
  for (const s of ['esports', 'mma', 'tennis', 'football', 'lol', 'dota2',
                   'basket', 'cs', 'cs2', 'valorant', 'darts', 'snooker', 'tabletennis']) {
  ...
  const stmts = {
    ...
    insertTip: db.prepare(`
      ...
      VALUES (@sport, @matchId, @eventName, @p1, @p2, @tipParticipant, @odds,
        ...
        @code_sha, @gate_state, @tip_context_json)
    `),
    // 2026-05-10: filtra archived. Mig 096 marca duplicates como archived=2;
    // sem esse filtro tips zumbi ficavam no settle loop pra sempre — /settle
    // filtra archived=0 → não settla → counter local infla mas DB inalterado
    // → próximo ciclo pega de novo. Caso visto: 47 tips tennis archived=2
    // result=null re-iteradas a cada 10min, log "tennis: N tips liquidadas"
    // misleading.
    getUnsettledTips: db.prepare(`SELECT * FROM tips WHERE sport = ? AND result IS NULL AND (archived IS NULL OR archived = 0) AND sent_at > datetime('now', ?)`),
    ...
    // direto via t.id.
    // 2026-05-10 P0 audit: clv_captured_at timestamp + guard NULL (mig 096 add col)
    // bloqueia race condition (2 captures concurrent escrevendo). updateTipOpenOdds
    // mantido sem guard pq open_odds é set-once.
    updateTipCLV: db.prepare(`UPDATE tips SET clv_odds = ?, clv_captured_at = datetime('now')
                              WHERE match_id = ? AND sport = ? AND tip_participant = ?
                              AND clv_captured_at IS NULL`),
    // 2026-05-28: near-regime (bot.js:30999 "última fica como close") re-captura a
    // cada ciclo até o match start — precisa SOBRESCREVER clv_odds. O guard
    // clv_captured_at IS NULL acima (mig 096, anti-race) silenciava esse overwrite,
    // travando o CLV no 1º odd da janela (ou no open-proxy far-future) em vez do
    // fechamento. Variante sem guard: last-writer-wins, correto pra near (converge
    // pro close). Race não é problema aqui — captures sequenciais ~6min, o mais
    // tardio (mais perto do close) deve vencer. Scoped por match+sport+side.
    updateTipCLVNear: db.prepare(`UPDATE tips SET clv_odds = ?, clv_captured_at = datetime('now')
                                  WHERE match_id = ? AND sport = ? AND tip_participant = ?`),
    updateTipCLVById: db.prepare(`UPDATE tips SET clv_odds = ?, clv_captured_at = datetime('now')
                                  WHERE id = ? AND clv_captured_at IS NULL`),
    ...
    `),
    // Mig 109 (FASE 1 dual-source observability): logs cada upsert pra detectar
    // mismatches winner cross-source. Reconcile cron 24h compara últimos 7d.
    // FASE 2 futura: settle só após confirmation.
    // 2026-05-17 EMERGENCY: INSERT OR IGNORE pattern + mig 114 UNIQUE index
    // em (match_id, game, source). Antes plain INSERT crescia 500k/dia por
    // dup cycle. DB 510MB hit Railway 512MB cap. Truncate liberou 357MB.
    // OR IGNORE garante 1 row per (match_id, game, source) — primeira call
    // do source registra, subsequentes no-op silenciosamente.
    insertMatchResultSource: db.prepare(`
      INSERT OR IGNORE INTO match_result_sources (match_id, game, source, team1, team2, winner, final_score)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `),

...
module.exports = function initDatabase(dbPath = 'sportsedge.db') {
  ...
  // ── Slow query log (opt-in via DB_SLOW_QUERY_MS) ──
  ...
  // call (Date.now + comparison). Default OFF (=0) — ative em prod só
  // quando investigando latência específica.
  const _slowMs = parseInt(process.env.DB_SLOW_QUERY_MS || '0', 10);
  ...
  const stmts = {
    ...
    settleTip: db.prepare(`UPDATE tips SET result = ?, settled_at = datetime('now'), is_live = 0 WHERE match_id = ? AND sport = ? AND result IS NULL`),
    // Per-tip settle por id — usar dentro de loops que processam múltiplas
    // tips do mesmo match individualmente (settleTip por match_id contamina
    // tips de lados opostos quando ambas estão pending: a 1ª iteração marca
    // todas com seu result e as iterações seguintes não acham nada por
    // result IS NULL guard). Quando match tem só 1 tip ML, comportamento
    // idêntico ao settleTip; para múltiplas tips do mesmo match (raro mas
    // possível em audit fixes), garante consistência result vs profit_reais.

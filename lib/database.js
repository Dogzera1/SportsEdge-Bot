...
module.exports = function initDatabase(dbPath = 'sportsedge.db') {
  ...
  // 'lol' e 'dota2' são os buckets pós-split (Abr/2026). Herda metade da banca
  // 'esports' quando esta já existe (divisão LoL/Dota 50/50). Fresh install → R$100.
  // Usuário pode re-dividir depois via /split-bankroll admin cmd.
  const existingEsports = db.prepare('SELECT initial_banca, current_banca FROM bankroll WHERE sport=?').get('esports');
  const lolExists = db.prepare('SELECT 1 FROM bankroll WHERE sport=?').get('lol');
  const dotaExists = db.prepare('SELECT 1 FROM bankroll WHERE sport=?').get('dota2');
  const lolInit = parseFloat(process.env.LOL_INITIAL_BANCA || '') || null;
  const dotaInit = parseFloat(process.env.DOTA2_INITIAL_BANCA || '') || null;
  if (!lolExists) {
    const init = lolInit != null ? lolInit :
                 (existingEsports?.initial_banca > 0 ? existingEsports.initial_banca / 2 : 100);
    const curr = lolInit != null ? lolInit :
                 (existingEsports?.current_banca > 0 ? existingEsports.current_banca / 2 : 100);
    db.prepare('INSERT INTO bankroll (sport, initial_banca, current_banca) VALUES (?, ?, ?)').run('lol', init, curr);
  }
  if (!dotaExists) {
    const init = dotaInit != null ? dotaInit :
                 (existingEsports?.initial_banca > 0 ? existingEsports.initial_banca / 2 : 100);
    const curr = dotaInit != null ? dotaInit :
                 (existingEsports?.current_banca > 0 ? existingEsports.current_banca / 2 : 100);
    db.prepare('INSERT INTO bankroll (sport, initial_banca, current_banca) VALUES (?, ?, ?)').run('dota2', init, curr);
  }
  ...
  const stmts = {
    ...
    voidTipById: db.prepare(`UPDATE tips SET result = 'void', settled_at = datetime('now'), profit_reais = 0 WHERE id = ? AND sport = ? AND result IS NULL`),
    voidTipByMatch: db.prepare(`UPDATE tips SET result = 'void', settled_at = datetime('now'), profit_reais = 0 WHERE match_id = ? AND sport = ? AND result IS NULL`),

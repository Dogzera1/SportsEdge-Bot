...
module.exports = function initDatabase(dbPath = 'sportsedge.db') {
  ...
  // Migração: se bankroll ainda usa schema antigo (id INTEGER), converte para per-sport
  try {
    const cols = db.prepare('PRAGMA table_info(bankroll)').all();
    const hasSport = cols.some(c => c.name === 'sport');
    if (!hasSport) {
      let initialBanca = 100.0, currentBanca = 100.0;
      try {
        const old = db.prepare('SELECT * FROM bankroll WHERE id = 1').get();
        if (old) { initialBanca = old.initial_banca; currentBanca = old.current_banca; }
      } catch(_) {}
      ...
      db.prepare("INSERT OR IGNORE INTO bankroll (sport, initial_banca, current_banca) VALUES ('esports', ?, ?)").run(initialBanca, currentBanca);
    }
  } catch(_) {}

  // Seed da banca inicial (R$100) por sport se ainda não existir
  for (const s of ['esports', 'mma', 'tennis', 'football']) {
    db.prepare('INSERT OR IGNORE INTO bankroll (sport, initial_banca, current_banca) VALUES (?, 100.0, 100.0)').run(s);
  }
  ...
  try { db.exec('ALTER TABLE pro_champ_stats ADD COLUMN first_dragon_wr REAL'); } catch (_) {}
  try { db.exec('ALTER TABLE pro_champ_stats ADD COLUMN first_baron_wr REAL'); } catch (_) {}
  try { db.exec("ALTER TABLE tips ADD COLUMN market_type TEXT DEFAULT 'ML'"); } catch (_) {}
  // Colunas para reanálise de tips em andamento (dashboard/telegram)
  try { db.exec('ALTER TABLE tips ADD COLUMN current_odds REAL'); } catch (_) {}
  try { db.exec('ALTER TABLE tips ADD COLUMN current_ev REAL'); } catch (_) {}
  try { db.exec('ALTER TABLE tips ADD COLUMN current_confidence TEXT'); } catch (_) {}
  try { db.exec('ALTER TABLE tips ADD COLUMN current_updated_at TEXT'); } catch (_) {}
  // Colunas para armazenar probabilidade do modelo (calibração precisa)
  try { db.exec('ALTER TABLE tips ADD COLUMN model_p1 REAL'); } catch (_) {}
  try { db.exec('ALTER TABLE tips ADD COLUMN model_p2 REAL'); } catch (_) {}
  try { db.exec('ALTER TABLE tips ADD COLUMN model_p_pick REAL'); } catch (_) {}
  try { db.exec('ALTER TABLE tips ADD COLUMN model_label TEXT'); } catch (_) {}
  // Texto curto do "porquê" da tip
  try { db.exec('ALTER TABLE tips ADD COLUMN tip_reason TEXT'); } catch (_) {}
  ...
  const stmts = {
    ...
    insertTip: db.prepare(`
      INSERT INTO tips (sport, match_id, event_name, participant1, participant2,
        tip_participant, odds, ev, stake, confidence, is_live, bot_token, market_type,
        model_p1, model_p2, model_p_pick, model_label, tip_reason)
      VALUES (@sport, @matchId, @eventName, @p1, @p2, @tipParticipant, @odds,
        @ev, @stake, @confidence, @isLive, @botToken, @market_type,
        @model_p1, @model_p2, @model_p_pick, @model_label, @tip_reason)
    `),
    ...
    updateTipCurrent: db.prepare(`
      UPDATE tips
      SET current_odds = ?, current_ev = ?, current_confidence = ?, current_updated_at = datetime('now')
    ...
    getFactorAccuracyLast30d: db.prepare(`SELECT factor, SUM(CASE WHEN predicted_dir = actual_winner THEN 1 ELSE 0 END) as wins, COUNT(*) as total FROM tip_factor_log WHERE actual_winner IS NOT NULL AND logged_at >= datetime('now', '-30 days') GROUP BY factor`)
  };

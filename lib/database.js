...
module.exports = function initDatabase(dbPath = 'sportsedge.db') {
  ...
  const stmts = {
    ...
    getTeamForm: db.prepare(`SELECT * FROM match_results WHERE (lower(team1) = lower(?) OR lower(team2) = lower(?)) AND game = ? AND resolved_at >= datetime('now', '-30 days') ORDER BY resolved_at DESC LIMIT 10`),
    getTeamFormFuzzy: db.prepare(`SELECT * FROM match_results WHERE (lower(team1) LIKE lower(?) OR lower(team2) LIKE lower(?)) AND game = ? AND resolved_at >= datetime('now', '-30 days') ORDER BY resolved_at DESC LIMIT 10`),
    getH2H: db.prepare(`SELECT * FROM match_results WHERE ((lower(team1) = lower(?) AND lower(team2) = lower(?)) OR (lower(team1) = lower(?) AND lower(team2) = lower(?))) AND game = ? AND resolved_at >= datetime('now', '-30 days') ORDER BY resolved_at DESC LIMIT 10`),
    getH2HFuzzy: db.prepare(`SELECT * FROM match_results WHERE ((lower(team1) LIKE lower(?) AND lower(team2) LIKE lower(?)) OR (lower(team1) LIKE lower(?) AND lower(team2) LIKE lower(?))) AND game = ? AND resolved_at >= datetime('now', '-30 days') ORDER BY resolved_at DESC LIMIT 10`),

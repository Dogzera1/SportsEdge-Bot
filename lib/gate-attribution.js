...
function runGateAttribution(db, opts = {}) {
  ...
  // 2026-05-08: source=shadow|real|all (default real preserva P2-original semantic).
  // shadow=true mistura tips is_shadow=1 (ML shadow) + market_tips_shadow (MT).
  // Útil pra contrafactual completo (passo 4 framework user 2026-05-08).
  const source = ['shadow', 'real', 'all'].includes(opts.source) ? opts.source : 'real';

  // 2026-05-28: ML stake_reais/profit_reais (BRL) convertidos pra units inline.
  // MT shadow já está em units (aliased AS stake_reais/profit_reais abaixo). UNION
  // downstream agora soma escala consistente. Fallback ao valor original quando
  // stake_reais NULL/0 (legacy tips: uv=R$1.00 → BRL == units numericamente).
  const _stakeUnitsSql = `CAST(REPLACE(REPLACE(stake, 'u', ''), 'U', '') AS REAL)`;
  const _stakeUnitsExpr = `
    CASE
      WHEN COALESCE(stake_reais, 0) > 0 AND ${_stakeUnitsSql} > 0
      THEN ${_stakeUnitsSql}
      ELSE COALESCE(stake_reais, 0)
    END
  `;
  const _profitUnitsExpr = `
    CASE
      WHEN COALESCE(stake_reais, 0) > 0 AND ${_stakeUnitsSql} > 0
      THEN COALESCE(profit_reais, 0) * ${_stakeUnitsSql} / stake_reais
  ...
  if (source === 'real' || source === 'all') {
    tips = tips.concat(db.prepare(`
      SELECT id, sport, event_name, odds, ev,
             ${_stakeUnitsExpr} AS stake_reais,
             ${_profitUnitsExpr} AS profit_reais,
             result, confidence, model_p_pick, 'real_ml' AS _src
      FROM tips
  ...
  if (source === 'shadow' || source === 'all') {
    // ML shadow: tips is_shadow=1
    tips = tips.concat(db.prepare(`
      SELECT id, sport, event_name, odds, ev,
             ${_stakeUnitsExpr} AS stake_reais,
             ${_profitUnitsExpr} AS profit_reais,
             result, confidence, model_p_pick, 'shadow_ml' AS _src
      FROM tips
      WHERE is_shadow = 1
        AND COALESCE(archived, 0) = 0
        AND result IN ('win','loss')
        ...
        AND settled_at >= datetime('now', '-' || ? || ' days')
    `).all(days));
    // MT shadow: market_tips_shadow — mapeia colunas pra mesmo shape de tips.
    // ev está em ev_pct (REAL); model_p_pick → p_model. Stake unidade (1u flat).
    // Aplica preço aproximado via odd column (já existe).
    const mtShadow = db.prepare(`
      SELECT id, sport, league AS event_name, odd AS odds, ev_pct AS ev,
             COALESCE(stake_units, 1) AS stake_reais,
             COALESCE(profit_units, 0) AS profit_reais,
             result, NULL AS confidence, p_model AS model_p_pick,
             'shadow_mt' AS _src
...
/**
 * Helper público pra plumbing de would_be_rejected_by per-tip em logShadowTip.
 * Aceita shape similar a tip MT shadow: { sport, league, odd, ev_pct, p_model }.
 *
 * @returns {string|null} JSON array de reasons OR null se passa todos gates
 */
function evaluateGatesForShadow({ sport, league, odd, ev_pct, p_model, confidence }) {
  const tipShape = {
    sport: String(sport || 'unknown').toLowerCase(),
    event_name: league || '',
    odds: odd,
    ev: ev_pct,
    model_p_pick: p_model,
    confidence: confidence || null,
  };
  const { passed, reasons } = _applyGates(tipShape);
  if (passed || !reasons.length) return null;
  return JSON.stringify(reasons);
}

module.exports = { runGateAttribution, evaluateGatesForShadow };

...
function buildExplainPrompt({ gameProfile, draft, teams, probPct, label }) {
  ...
  if (gp.phases) {
    ...
    if (late) lines.push(`- LATE (scaling do sistema — SINAL FRACO, pode estar errado): ${_side(late.winner)}`);
  }
  ...
  lines.push('');
  lines.push('Você é um analista de LoL. Use Elo, early (medido) e odd justa como ÂNCORA (dados sólidos). Para o LATE e as sinergias/counters/win-conditions do draft, USE O SEU CONHECIMENTO dos campeões — o "scaling" que o sistema estima é SINAL FRACO, então você PODE e DEVE discordar dele quando os campeões disserem o contrário (ex.: quem realmente escala melhor). Escreva em PT-BR.');
  lines.push('Responda APENAS um JSON compacto, nada fora dele:');
  lines.push('{"early":"…","mid":"…","late":"…","winCondition":"…","keyMatchup":"…","verdict":"…"}');
  lines.push('Regras: cada campo 1-2 frases; o Elo/odd ancora o verdict (não invente probabilidade); o early é medido (gold/xp @15) — respeite-o; para late e draft use o seu conhecimento e, se discordar do scaling do sistema, diga isso brevemente; NÃO recomende stake nem diga "aposte".');
  return lines.join('\n');
}

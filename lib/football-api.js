...
async function getUpcomingFixturesCached() {
  ...
  if (combined.length) {
    ...
    console.info(`[FOOTBALL-API] Batch cache: ${combined.length} fixtures (${year}: ${curr.length}, ${year-1}: ${prev.length})`);
  } else {
    console.warn(`[FOOTBALL-API] Batch cache vazio — possível quota esgotada ou season ${year}/${year-1} sem fixtures NS/TBD nas ligas configuradas`);
  }
  return combined;
}

...
async function _httpJson(url, { ttlMs = 5 * 60 * 1000 } = {}) {
  // 2026-05-14: throttle global per-host (5× sofascore-* paralelos burst-violavam 200ms).
  ...
  if (!r) { try { require('./utils').logSofaNon200(url, null, 'SOFA-DARTS'); } catch (_) {} return null; }
  if (r.status !== 200) { try { require('./utils').logSofaNon200(url, r.status, 'SOFA-DARTS'); } catch (_) {} return null; }
...
/**
 * Placar ao vivo (sets/legs + pontos no leg atual).
 * Retorna null se Sofascore não responder.
 *
 * @param {number|string} eventId
 * @returns {Promise<null | { isLive, isFinished, setsHome, setsAway, pointsHome, pointsAway, status }>}
 */
async function getLiveScore(eventId) {
  const j = await _fetchFirstJson([`/event/${eventId}`]);
  const ev = j?.event || j;
  if (!ev) return null;
  const statusType = ev?.status?.type || 'unknown';
  const isLive = statusType === 'inprogress' || statusType === 'live';
  const isFinished = statusType === 'finished';
  const setsHome = ev?.homeScore?.current ?? ev?.homeScore?.display ?? null;
  const setsAway = ev?.awayScore?.current ?? ev?.awayScore?.display ?? null;
  // `point` contém os pontos restantes (501 decrescente) no leg corrente — string no Sofascore.
  const pointsHome = ev?.homeScore?.point != null ? parseInt(String(ev.homeScore.point), 10) : null;
  const pointsAway = ev?.awayScore?.point != null ? parseInt(String(ev.awayScore.point), 10) : null;
  return {
    isLive,
    isFinished,
    setsHome: Number.isFinite(setsHome) ? setsHome : null,
    setsAway: Number.isFinite(setsAway) ? setsAway : null,
    pointsHome: Number.isFinite(pointsHome) ? pointsHome : null,
    pointsAway: Number.isFinite(pointsAway) ? pointsAway : null,
    status: statusType,
  };
}

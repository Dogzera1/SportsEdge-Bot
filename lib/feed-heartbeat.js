 ...
 *   odds-api:   15min
 *
 * Sport-aware overrides (P1 — granularidade) 2026-05-09:
 *   pinnacle|basket: 10min  (cron bot:basket idle 1800s, scanner cycle ~10min)
 *   pinnacle|mma:    15min  (cron bot:mma idle 900s)
 *   pinnacle|darts:  15min  (cron bot:darts idle 900s)
 *   pinnacle|snooker:15min  (cron bot:snooker idle 900s)
 *   pinnacle|tt:     15min  (cron bot:tt idle 900s)
 * Sem sport-aware, basket reportava `stale ratio 11x esperado 2min` quando
 * idle (cycle real 30min). Falsos positivos consistentes em /admin/p2-status alerts.
 *
 * Override via FEED_INTERVAL_<SOURCE>_<SPORT>=<minutes> ou FEED_INTERVAL_<SOURCE>.
...
const DEFAULT_INTERVALS_MIN = {
  ...
  'sx-bet': 10,
  'football-data': 60,
  // Sport-aware overrides (key: 'source|sport') — P1 granularidade.
  // Pinnacle base é 2min (assumindo poll constante de LoL/CS/Tennis), mas
  // sports com idle longo precisam intervalo maior senão geram falso stale.
  'pinnacle|basket': 10,
  'pinnacle|mma': 15,
  'pinnacle|darts': 15,
  'pinnacle|snooker': 15,
...
function _expectedIntervalMs(source, sport) {
  const src = String(source || '').toLowerCase().replace(/[^a-z0-9-]/g, '_');
  const sp = String(sport || '').toLowerCase().replace(/[^a-z0-9-]/g, '_');
  // 1. Env per-(source,sport): FEED_INTERVAL_PINNACLE_BASKET=10
  if (sp && sp !== 'all') {
    const envKeySport = `FEED_INTERVAL_${src.toUpperCase().replace(/-/g, '_')}_${sp.toUpperCase().replace(/-/g, '_')}`;
    const envValSport = parseFloat(process.env[envKeySport]);
    if (Number.isFinite(envValSport) && envValSport > 0) return envValSport * 60_000;
  }
  // 2. Env per-source legacy: FEED_INTERVAL_PINNACLE=2
  ...
  // 3. Default per-(source,sport): 'pinnacle|basket' → 10
  if (sp && sp !== 'all') {
    const defSport = DEFAULT_INTERVALS_MIN[`${src}|${sp}`];
    if (Number.isFinite(defSport)) return defSport * 60_000;
  }
  // 4. Default per-source: 'pinnacle' → 2
...
function checkFeedHealth(opts = {}) {
  ...
  for (const [k, s] of _state) {
    ...
    const expected = _expectedIntervalMs(s.source, s.sport);
...
/**
 * 2026-05-11: detecta outage cluster onde Pinnacle retorna 0 partidas em
 * múltiplos sports simultaneamente. Caso observado em prod: 10:09 UTC todos
 * (cs/football/tennis/val/mma/dota/lol) zeraram juntos — bot caiu pra cache
 * stale sem WARN visível. Não é stale (markFeedSuccess foi chamado com count=0)
 * então checkFeedHealth não pega. Esta função é específica pra esse pattern.
 *
 * @param {object} opts
 * @param {number} [opts.windowMin=5] — janela em min pra considerar count recente
 * @param {number} [opts.minSports=4] — mínimo de sports zerados pra cluster
 * @returns {{ isOutage, zeroSports: string[], minSports, windowMin }}
 */
function detectPinnacleOutage(opts = {}) {
  const windowMin = Math.max(1, opts.windowMin || 5);
  const minSports = Math.max(2, opts.minSports || 4);
  const windowMs = windowMin * 60_000;
  const now = Date.now();
  const zeroSports = [];
  for (const [, s] of _state) {
    if (String(s.source || '').toLowerCase() !== 'pinnacle') continue;
    if (!s.lastSuccessMs) continue;
    if (now - s.lastSuccessMs > windowMs) continue;
    if ((s.lastCount || 0) !== 0) continue;
    zeroSports.push(String(s.sport || '?'));
  }
  return { isOutage: zeroSports.length >= minSports, zeroSports, minSports, windowMin };
}

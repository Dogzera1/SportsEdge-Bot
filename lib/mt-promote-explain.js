...
// Espelha _getMtPermanentDisable em bot.js
const PERMANENT_DISABLE_DEFAULT = 'tennis|totalGames|over,tennis|totalGames|under,lol|total,cs|total|under,lol|total_kills_map2|under';

function _getPermanentDisableSet() {
  const raw = String(process.env.MT_PERMANENT_DISABLE_LIST ?? PERMANENT_DISABLE_DEFAULT).trim();
  const set = new Set();
...
function explainMtPromoteGates(args = {}) {
  ...
  // 6. MT_PERMANENT_DISABLE_LIST
  const permSet = _getPermanentDisableSet();
  const permKeys = [

...
function _checkRuntimeDisable(db, sport, market, side, league) {
  try {
    // 2026-05-26: LOWER(market) ambos lados pra cobrir entries legacy camelCase
    // (`handicapGames`/`totalGames`) que o probe lowercased não bate via `=`.
    // Mesmo bug class do a657c29 mas em path DB-direct (mt-promote-explain).
    const rows = db.prepare(`
      SELECT sport, market, side, league, tier, source, reason
      FROM market_tips_runtime_state
      WHERE disabled = 1 AND LOWER(sport) = LOWER(?) AND LOWER(market) = LOWER(?)

...
test('SQL CASE classifica market real prod corretamente', () => {
  const db = setup();
  ...
  for (const r of rows) {
    ...
    assert.equal(r.segment, expected[key], `${key} → expected ${expected[key]} got ${r.segment}`);
  }
  db.close();
});

test('GROUP BY (sport, segment) agrega contagens corretas', () => {
  const db = setup();
  const rows = db.prepare(`
